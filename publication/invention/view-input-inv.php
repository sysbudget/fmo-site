        ...
        <body>
                ...
                <?php
                        ...
                        // number of results to show per page
										...
										if ($usercu == !'') {
										$query = "SELECT year_cover, tbl_cu.short_name, unit, inv_invention, inv_sub_inventor, inv_inventor, inv_applied, inv_patented, inv_commercial, inv_adopted, inv_patent_num, inv_date_issue, inv_development, inv_service, inv_end_product, inv_product_name, inv_award, inv_keyword, user_name, invid FROM tbl_invention_main_raw INNER JOIN tbl_cu ON tbl_invention_main_raw.cu = tbl_cu.id WHERE user_name =  '$username' ORDER BY invid DESC" ;
}
else {
										$query = "SELECT year_cover, tbl_cu.short_name, unit, inv_invention, inv_sub_inventor, inv_inventor, inv_applied, inv_patented, inv_commercial, inv_adopted, inv_patent_num, inv_date_issue, inv_development, inv_service, inv_end_product, inv_product_name, inv_award, inv_keyword, user_name, invid FROM tbl_invention_main_raw INNER JOIN tbl_cu ON tbl_invention_main_raw.cu = tbl_cu.id ORDER BY invid DESC" ;
}
										if ($result = $mysqli->query($query))
                                        {
                                                if ($result->num_rows != 0)
                                                {
														...
														echo "<p><a href='input-inv.php'>Add New Record</a>";
                                                        echo "</div>";
