...
class caudittrail {
	...
	function caudittrail() {
		...
		// id
		$this->id = new cField('audittrail', 'audittrail', 'x_id', 'id', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] =& $this->id;

		// datetime
		$this->datetime = new cField('audittrail', 'audittrail', 'x_datetime', 'datetime', '`datetime`', 135, 6, FALSE, '`datetime`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->datetime->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['datetime'] =& $this->datetime;

		// script
		$this->script = new cField('audittrail', 'audittrail', 'x_script', 'script', '`script`', 200, -1, FALSE, '`script`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['script'] =& $this->script;

		// user
		$this->user = new cField('audittrail', 'audittrail', 'x_user', 'user', '`user`', 200, -1, FALSE, '`user`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['user'] =& $this->user;

		// action
		$this->action = new cField('audittrail', 'audittrail', 'x_action', 'action', '`action`', 200, -1, FALSE, '`action`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['action'] =& $this->action;

		// table
		$this->ztable = new cField('audittrail', 'audittrail', 'x_ztable', 'table', '`table`', 200, -1, FALSE, '`table`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['table'] =& $this->ztable;

		// field
		$this->zfield = new cField('audittrail', 'audittrail', 'x_zfield', 'field', '`field`', 200, -1, FALSE, '`field`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['field'] =& $this->zfield;

		// keyvalue
		$this->keyvalue = new cField('audittrail', 'audittrail', 'x_keyvalue', 'keyvalue', '`keyvalue`', 201, -1, FALSE, '`keyvalue`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['keyvalue'] =& $this->keyvalue;

		// oldvalue
		$this->oldvalue = new cField('audittrail', 'audittrail', 'x_oldvalue', 'oldvalue', '`oldvalue`', 201, -1, FALSE, '`oldvalue`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['oldvalue'] =& $this->oldvalue;

		// newvalue
		$this->newvalue = new cField('audittrail', 'audittrail', 'x_newvalue', 'newvalue', '`newvalue`', 201, -1, FALSE, '`newvalue`', FALSE, FALSE, 'FORMATTED TEXT');
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->ztable->setDbValue($rs->fields('table'));
   ...
   // Common render codes
		...
		$this->datetime->ViewValue = up_FormatDateTime($this->datetime->ViewValue, 6);
		...
		// table
		$this->ztable->ViewValue = $this->ztable->CurrentValue;
		$this->ztable->ViewCustomAttributes = "";
		...
		// table
		$this->ztable->LinkCustomAttributes = "";
		$this->ztable->HrefValue = "";
		$this->ztable->TooltipValue = "";

		// field
		$this->zfield->LinkCustomAttributes = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('datetime', $this->datetime->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('script', $this->script->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('user', $this->user->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('action', $this->action->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('ztable', $this->ztable->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('zfield', $this->zfield->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('keyvalue', $this->keyvalue->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('oldvalue', $this->oldvalue->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('newvalue', $this->newvalue->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('id', $this->id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('datetime', $this->datetime->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('script', $this->script->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->ztable);
				$Doc->ExportCaption($this->zfield);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->ztable);
					$Doc->ExportField($this->zfield);
