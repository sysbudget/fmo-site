...
tbl_users_add.ValidateForm = function(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !up_CheckInteger(elm.value))
			return up_OnError(this, elm, "<?php echo up_JsEncode2($tbl_users->unitID->FldErrMsg()) ?>");
...
<?php } else { ?>
<span id="as_x_unitID" style="white-space: nowrap; z-index: 8970">
	<input type="text" name="sv_x_unitID" id="sv_x_unitID" value="<?php echo $tbl_users->unitID->EditValue ?>" size="30"<?php echo $tbl_users->unitID->EditAttributes() ?>>&nbsp;<span id="em_x_unitID" class="upMessage" style="display: none"><?php echo str_replace("%f", "phpimages/", $Language->Phrase("UnmatchedValue")) ?></span>
	<div id="sc_x_unitID" style="z-index: 8970"></div>
</span>
<input type="hidden" name="x_unitID" id="x_unitID" value="<?php echo $tbl_users->unitID->CurrentValue ?>">
<?php
$sSqlWrk = "SELECT `id`, `unit_bo` FROM `tbl_units`";
$sWhereWrk = "`unit_bo`  LIKE '{query_value}%'";
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
$sSqlWrk .= " LIMIT " . UP_AUTO_SUGGEST_MAX_ENTRIES;
	$sSqlWrk = TEAencrypt($sSqlWrk, UP_RANDOM_KEY);
?>
<input type="hidden" name="s_x_unitID" id="s_x_unitID" value="<?php echo $sSqlWrk ?>">
<script type="text/javascript">
<!--
var oas_x_unitID = new up_AutoSuggest("sv_x_unitID", "sc_x_unitID", "s_x_unitID", "em_x_unitID", "x_unitID", "", false, UP_AUTO_SUGGEST_MAX_ENTRIES);
oas_x_unitID.formatResult = function(ar) {	
	var df1 = ar[1];
	return df1;
};
oas_x_unitID.ac.typeAhead = false;
...
class ctbl_users_add {
	...
	function RenderRow() {
		...
		if ($tbl_users->RowType == UP_ROWTYPE_VIEW) { // View row
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$tbl_users->unitID->EditValue = $rswrk->fields('unit_bo');
					$rswrk->Close();
				} else {
					$tbl_users->unitID->EditValue = $tbl_users->unitID->CurrentValue;
				}
			...
			$tbl_users->unitID->EditValue = up_HtmlEncode($tbl_users->unitID->CurrentValue);
			if (strval($tbl_users->unitID->CurrentValue) <> "") {
				$sFilterWrk = "`id` = " . up_AdjustSql($tbl_users->unitID->CurrentValue) . "";
			$sSqlWrk = "SELECT `unit_bo` FROM `tbl_units`";
			$sWhereWrk = "";
			...
			$arwrk[] = array("3", $tbl_users->users_userLevel->FldTagCaption(4) <> "" ? $tbl_users->users_userLevel->FldTagCaption(4) : "3");
	...
	function ValidateForm() {
		...
		if (!UP_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!up_CheckInteger($tbl_users->unitID->FormValue)) {
			up_AddMessage($gsFormError, $tbl_users->unitID->FldErrMsg());
		}
	...
	function AddRow($rsold = NULL) {
		...
		// users_nameLast
		$tbl_users->users_nameLast->SetDbValueDef($rsnew, $tbl_users->users_nameLast->CurrentValue, "", FALSE);

		// users_nameFirst
		$tbl_users->users_nameFirst->SetDbValueDef($rsnew, $tbl_users->users_nameFirst->CurrentValue, "", FALSE);
