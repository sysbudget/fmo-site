...
// Create page object
$tbl_collectionperiod_edit = new ctbl_collectionperiod_edit();
$Page =& $tbl_collectionperiod_edit;

// Page init
$tbl_collectionperiod_edit->Page_Init();

// Page main
$tbl_collectionperiod_edit->Page_Main();
?>
...
// Create page object
var tbl_collectionperiod_edit = new up_Page("tbl_collectionperiod_edit");

// page properties
tbl_collectionperiod_edit.PageID = "edit"; // page ID
tbl_collectionperiod_edit.FormID = "ftbl_collectionperiodedit"; // form ID
var UP_PAGE_ID = tbl_collectionperiod_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
tbl_collectionperiod_edit.ValidateForm = function(fobj) {
...
tbl_collectionperiod_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
tbl_collectionperiod_edit.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
tbl_collectionperiod_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
tbl_collectionperiod_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $tbl_collectionperiod->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $tbl_collectionperiod->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $tbl_collectionperiod_edit->ShowPageHeader(); ?>
<?php
$tbl_collectionperiod_edit->ShowMessage();
?>
<form name="ftbl_collectionperiodedit" id="ftbl_collectionperiodedit" action="<?php echo up_CurrentPage() ?>" method="post" onsubmit="return tbl_collectionperiod_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="tbl_collectionperiod">
...
<input type="hidden" name="x_collectionPeriod_ID" id="x_collectionPeriod_ID" value="<?php echo up_HtmlEncode($tbl_collectionperiod->collectionPeriod_ID->CurrentValue) ?>">
<p>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo up_BtnCaption($Language->Phrase("EditBtn")) ?>">
</form>
<?php
$tbl_collectionperiod_edit->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$tbl_collectionperiod_edit->Page_Terminate();
?>
<?php
...
// Page class
//
class ctbl_collectionperiod_edit {
	...
	// Page object name
	var $PageObjName = 'tbl_collectionperiod_edit';
	...
	//
	function ctbl_collectionperiod_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$tbl_collectionperiod->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$tbl_collectionperiod->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$tbl_collectionperiod->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$tbl_collectionperiod->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($tbl_collectionperiod->collectionPeriod_ID->CurrentValue == "")
			$this->Page_Terminate("tbl_collectionperiodlist.php"); // Invalid key, return to list
		switch ($tbl_collectionperiod->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$tbl_collectionperiod->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$tbl_collectionperiod->RowType = UP_ROWTYPE_EDIT; // Render as Edit
	...
	function LoadFormValues() {
		...
		if (!$tbl_collectionperiod->collectionPeriod_ID->FldIsDetailKey)
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_cu
			$tbl_collectionperiod->id_cu->SetDbValueDef($rsnew, $tbl_collectionperiod->id_cu->CurrentValue, NULL, $tbl_collectionperiod->id_cu->ReadOnly);

			// cu_name
			$tbl_collectionperiod->cu_name->SetDbValueDef($rsnew, $tbl_collectionperiod->cu_name->CurrentValue, NULL, $tbl_collectionperiod->cu_name->ReadOnly);
			...
			// remarks_cutOffDate
			$tbl_collectionperiod->remarks_cutOffDate->SetDbValueDef($rsnew, up_UnFormatDateTime($tbl_collectionperiod->remarks_cutOffDate->CurrentValue, 6), NULL, $tbl_collectionperiod->remarks_cutOffDate->ReadOnly);
