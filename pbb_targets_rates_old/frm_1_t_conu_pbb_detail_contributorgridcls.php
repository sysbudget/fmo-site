...
// Page class
//
class cfrm_1_t_conu_pbb_detail_contributor_grid {
	...
	// Page object name
	var $PageObjName = 'frm_1_t_conu_pbb_detail_contributor_grid';
	...
	//
	function cfrm_1_t_conu_pbb_detail_contributor_grid() {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			if ($frm_1_t_conu_pbb_detail_contributor->AllowAddDeleteRow) {
	...
	function ClearInlineMode() {
		global $frm_1_t_conu_pbb_detail_contributor;
		$frm_1_t_conu_pbb_detail_contributor->LastAction = $frm_1_t_conu_pbb_detail_contributor->CurrentAction; // Save last action
		$frm_1_t_conu_pbb_detail_contributor->CurrentAction = ""; // Clear action
	...
	function GridUpdate() {
		global $conn, $Language, $objForm, $gsFormError, $frm_1_t_conu_pbb_detail_contributor;
		$bGridUpdate = TRUE;

		// Get old recordset
		$frm_1_t_conu_pbb_detail_contributor->CurrentFilter = $this->BuildKeyFilter();
				...
				// Validate form and insert/update/delete record
					...
					if ($rowaction == "delete") {
						...
						if ($rowaction == "insert") {
							...
							if ($rowkey <> "") {
								$frm_1_t_conu_pbb_detail_contributor->SendEmail = FALSE; // Do not send email on update success
		...
		if ($bGridUpdate) {
			...
			$frm_1_t_conu_pbb_detail_contributor->EventCancelled = TRUE; // Set event cancelled
			$frm_1_t_conu_pbb_detail_contributor->CurrentAction = "gridedit"; // Stay in Grid Edit mode
		}
		return $bGridUpdate;
	}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GridInsert() {
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				$addcnt++;
				$frm_1_t_conu_pbb_detail_contributor->SendEmail = FALSE; // Do not send email on insert success
				...
				if ($bGridInsert) {
					...
					$sKey .= $frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->CurrentValue;
		...
		if ($bGridInsert) {
			...
			// Get new recordset
			$frm_1_t_conu_pbb_detail_contributor->CurrentFilter = $sWrkFilter;
			$sSql = $frm_1_t_conu_pbb_detail_contributor->SQL();
			if ($rs = $conn->Execute($sSql)) {
				$rsnew = $rs->GetRows();
				$rs->Close();
			...
			$frm_1_t_conu_pbb_detail_contributor->EventCancelled = TRUE; // Set event cancelled
			$frm_1_t_conu_pbb_detail_contributor->CurrentAction = "gridadd"; // Stay in gridadd mode
		}
		return $bGridInsert;
	}
	...
	// Check if empty row
	function EmptyRow() {
		global $frm_1_t_conu_pbb_detail_contributor, $objForm;
		if ($objForm->HasValue("x_Delivery_Unit") && $objForm->HasValue("o_Delivery_Unit") && $frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Contributing_Unit") && $objForm->HasValue("o_Contributing_Unit") && $frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_MFO") && $objForm->HasValue("o_MFO") && $frm_1_t_conu_pbb_detail_contributor->MFO->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->MFO->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Question") && $objForm->HasValue("o_Question") && $frm_1_t_conu_pbb_detail_contributor->Question->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Question->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Applicable") && $objForm->HasValue("o_Applicable") && $frm_1_t_conu_pbb_detail_contributor->Applicable->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Applicable->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Target") && $objForm->HasValue("o_Target") && $frm_1_t_conu_pbb_detail_contributor->Target->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Target->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Target_Remarks") && $objForm->HasValue("o_Target_Remarks") && $frm_1_t_conu_pbb_detail_contributor->Target_Remarks->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Target_Remarks->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Target_Cut2DOff_Date") && $objForm->HasValue("o_Target_Cut2DOff_Date") && $frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Target_Message") && $objForm->HasValue("o_Target_Message") && $frm_1_t_conu_pbb_detail_contributor->Target_Message->CurrentValue <> $frm_1_t_conu_pbb_detail_contributor->Target_Message->OldValue)
			return FALSE;
		return TRUE;
	}
		...
		if ($frm_1_t_conu_pbb_detail_contributor->AllowAddDeleteRow) {
			if ($frm_1_t_conu_pbb_detail_contributor->CurrentMode == "add" || $frm_1_t_conu_pbb_detail_contributor->CurrentMode == "copy" || $frm_1_t_conu_pbb_detail_contributor->CurrentMode == "edit") {
				...
				if (is_numeric($this->RowIndex) && ($this->RowAction == "" || $this->RowAction == "edit")) { // Do not allow delete existing record
					...
					$oListOpt->Body = "<a class=\"upGridLink\" href=\"javascript:void(0);\" onclick=\"up_DeleteGridRow(this, frm_1_t_conu_pbb_detail_contributor_grid, " . $this->RowIndex . ");\">" . $Language->Phrase("DeleteLink") . "</a>";
				}
			}
		}
		if ($frm_1_t_conu_pbb_detail_contributor->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		global $frm_1_t_conu_pbb_detail_contributor;
		$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->OldValue = $frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->OldValue = $frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->MFO->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->MFO->OldValue = $frm_1_t_conu_pbb_detail_contributor->MFO->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Question->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Question->OldValue = $frm_1_t_conu_pbb_detail_contributor->Question->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Applicable->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Applicable->OldValue = $frm_1_t_conu_pbb_detail_contributor->Applicable->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Target->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Target->OldValue = $frm_1_t_conu_pbb_detail_contributor->Target->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Target_Remarks->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Target_Remarks->OldValue = $frm_1_t_conu_pbb_detail_contributor->Target_Remarks->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->OldValue = $frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->CurrentValue;
		$frm_1_t_conu_pbb_detail_contributor->Target_Message->CurrentValue = NULL;
		$frm_1_t_conu_pbb_detail_contributor->Target_Message->OldValue = $frm_1_t_conu_pbb_detail_contributor->Target_Message->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->setOldValue($objForm->GetValue("o_Delivery_Unit"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->setOldValue($objForm->GetValue("o_Contributing_Unit"));
		...
		$frm_1_t_conu_pbb_detail_contributor->MFO->setOldValue($objForm->GetValue("o_MFO"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Question->setOldValue($objForm->GetValue("o_Question"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Applicable->setOldValue($objForm->GetValue("o_Applicable"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Target->setOldValue($objForm->GetValue("o_Target"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Target_Remarks->setOldValue($objForm->GetValue("o_Target_Remarks"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->setOldValue($objForm->GetValue("o_Target_Cut2DOff_Date"));
		...
		$frm_1_t_conu_pbb_detail_contributor->Target_Message->setOldValue($objForm->GetValue("o_Target_Message"));
		if (!$frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->FldIsDetailKey && $frm_1_t_conu_pbb_detail_contributor->CurrentAction <> "gridadd" && $frm_1_t_conu_pbb_detail_contributor->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		global $objForm, $frm_1_t_conu_pbb_detail_contributor;
		if ($frm_1_t_conu_pbb_detail_contributor->CurrentAction <> "gridadd" && $frm_1_t_conu_pbb_detail_contributor->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->CurrentValue = strval($arKeys[0]); // pbb_contributor_id
			else
				$bValidKey = FALSE;
		}
	...
	function RenderRow() {
		...
		if ($frm_1_t_conu_pbb_detail_contributor->RowType == UP_ROWTYPE_VIEW) { // View row
		...
		} elseif ($frm_1_t_conu_pbb_detail_contributor->RowType == UP_ROWTYPE_ADD) { // Add row

			// Delivery Unit
			$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->EditCustomAttributes = "";
			$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->CurrentValue);

			// Contributing Unit
			$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->EditCustomAttributes = "";
			$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->CurrentValue);

			// MFO
			$frm_1_t_conu_pbb_detail_contributor->MFO->EditCustomAttributes = "";
			$frm_1_t_conu_pbb_detail_contributor->MFO->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->MFO->CurrentValue);

			// Question
			$frm_1_t_conu_pbb_detail_contributor->Question->EditCustomAttributes = "";
			$frm_1_t_conu_pbb_detail_contributor->Question->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->Question->CurrentValue);
			...
			$frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->CurrentValue);

			// Target Message
			$frm_1_t_conu_pbb_detail_contributor->Target_Message->EditCustomAttributes = "";
			$frm_1_t_conu_pbb_detail_contributor->Target_Message->EditValue = up_HtmlEncode($frm_1_t_conu_pbb_detail_contributor->Target_Message->CurrentValue);
	...
	//
	function DeleteRows() {
		global $conn, $Language, $Security, $frm_1_t_conu_pbb_detail_contributor;
		$DeleteRows = TRUE;
		$sSql = $frm_1_t_conu_pbb_detail_contributor->SQL();
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $frm_1_t_conu_pbb_detail_contributor->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$conn->raiseErrorFn = 'up_ErrorFn';
				$DeleteRows = $conn->Execute($frm_1_t_conu_pbb_detail_contributor->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$frm_1_t_conu_pbb_detail_contributor->Row_Deleted($row);
			}
		}
		return $DeleteRows;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sFilter = $frm_1_t_conu_pbb_detail_contributor->SqlMasterFilter_frm_1_t_conu_contributor_pi_status();
			if (strval($frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->CurrentValue) <> "" &&
				$frm_1_t_conu_pbb_detail_contributor->getCurrentMasterTable() == "frm_1_t_conu_contributor_pi_status") {
				$sFilter = str_replace("@unit_contributor_id@", up_AdjustSql($frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if (strval($frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->CurrentValue) <> "" &&
				$frm_1_t_conu_pbb_detail_contributor->getCurrentMasterTable() == "frm_1_t_conu_contributor_pi_status") {
				$sFilter = str_replace("@MFO_SEQ@", up_AdjustSql($frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
		...
		// Set up foreign key field value from Session
			if ($frm_1_t_conu_pbb_detail_contributor->getCurrentMasterTable() == "frm_1_t_conu_contributor_pi_status") {
				$frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->CurrentValue = $frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->getSessionValue();
				$frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->CurrentValue = $frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->getSessionValue();
			}
		$rsnew = array();

		// Delivery Unit
		$frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Delivery_Unit->CurrentValue, NULL, FALSE);

		// Contributing Unit
		$frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Contributing_Unit->CurrentValue, NULL, FALSE);

		// MFO
		$frm_1_t_conu_pbb_detail_contributor->MFO->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->MFO->CurrentValue, NULL, FALSE);

		// Question
		$frm_1_t_conu_pbb_detail_contributor->Question->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Question->CurrentValue, NULL, FALSE);

		// Applicable
		$frm_1_t_conu_pbb_detail_contributor->Applicable->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Applicable->CurrentValue, NULL, FALSE);

		// Target
		$frm_1_t_conu_pbb_detail_contributor->Target->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Target->CurrentValue, NULL, FALSE);

		// Target Remarks
		$frm_1_t_conu_pbb_detail_contributor->Target_Remarks->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Target_Remarks->CurrentValue, NULL, FALSE);

		// Target Cut-Off Date
		$frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Target_Cut2DOff_Date->CurrentValue, NULL, FALSE);

		// Target Message
		$frm_1_t_conu_pbb_detail_contributor->Target_Message->SetDbValueDef($rsnew, $frm_1_t_conu_pbb_detail_contributor->Target_Message->CurrentValue, NULL, FALSE);

		// unit_contributor_id
		if ($frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->getSessionValue() <> "") {
			$rsnew['unit_contributor_id'] = $frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->getSessionValue();
		} elseif (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin
			$rsnew['unit_contributor_id'] = CurrentUserID();
		}

		// mfo_question_online_insert_question_mfo
		if ($frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->getSessionValue() <> "") {
			$rsnew['mfo_question_online_insert_question_mfo'] = $frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->getSessionValue();
		}

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $frm_1_t_conu_pbb_detail_contributor->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'up_ErrorFn';
			$AddRow = $conn->Execute($frm_1_t_conu_pbb_detail_contributor->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
			if ($frm_1_t_conu_pbb_detail_contributor->CancelMessage <> "") {
				$this->setFailureMessage($frm_1_t_conu_pbb_detail_contributor->CancelMessage);
		...
		if ($AddRow) {
			$frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->setDbValue($conn->Insert_ID());
			$rsnew['pbb_contributor_id'] = $frm_1_t_conu_pbb_detail_contributor->pbb_contributor_id->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$frm_1_t_conu_pbb_detail_contributor->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
	...
	function SetUpMasterParms() {
		...
		// Hide foreign keys
		$sMasterTblVar = $frm_1_t_conu_pbb_detail_contributor->getCurrentMasterTable();
		if ($sMasterTblVar == "frm_1_t_conu_contributor_pi_status") {
			$frm_1_t_conu_pbb_detail_contributor->unit_contributor_id->Visible = FALSE;
			if ($GLOBALS["frm_1_t_conu_contributor_pi_status"]->EventCancelled) $frm_1_t_conu_pbb_detail_contributor->EventCancelled = TRUE;
			$frm_1_t_conu_pbb_detail_contributor->mfo_question_online_insert_question_mfo->Visible = FALSE;
			if ($GLOBALS["frm_1_t_conu_contributor_pi_status"]->EventCancelled) $frm_1_t_conu_pbb_detail_contributor->EventCancelled = TRUE;
		}
