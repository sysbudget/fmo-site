...
//
// Table class for tbl_temp_cu
//
class ctbl_temp_cu {
	var $TableVar = 'tbl_temp_cu';
	...
	//
	function ctbl_temp_cu() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// temp_id
		$this->temp_id = new cField('tbl_temp_cu', 'tbl_temp_cu', 'x_temp_id', 'temp_id', '`temp_id`', 3, -1, FALSE, '`temp_id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->temp_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['temp_id'] =& $this->temp_id;

		// temp_name
		$this->temp_name = new cField('tbl_temp_cu', 'tbl_temp_cu', 'x_temp_name', 'temp_name', '`temp_name`', 200, -1, FALSE, '`temp_name`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `tbl_temp_cu` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `tbl_temp_cu` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `tbl_temp_cu` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tbl_temp_cuview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tbl_temp_cuadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_cuedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_cuadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("tbl_temp_cudelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tbl_temp_cu" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
