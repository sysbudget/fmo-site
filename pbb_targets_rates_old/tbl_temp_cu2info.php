...
//
// Table class for tbl_temp_cu2
//
class ctbl_temp_cu2 {
	var $TableVar = 'tbl_temp_cu2';
	...
	//
	function ctbl_temp_cu2() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// temp_id
		$this->temp_id = new cField('tbl_temp_cu2', 'tbl_temp_cu2', 'x_temp_id', 'temp_id', '`temp_id`', 3, -1, FALSE, '`temp_id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->temp_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['temp_id'] =& $this->temp_id;

		// temp_name
		$this->temp_name = new cField('tbl_temp_cu2', 'tbl_temp_cu2', 'x_temp_name', 'temp_name', '`temp_name`', 200, -1, FALSE, '`temp_name`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `tbl_temp_cu2` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `tbl_temp_cu2` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `tbl_temp_cu2` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tbl_temp_cu2view.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tbl_temp_cu2add.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_cu2edit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_cu2add.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("tbl_temp_cu2delete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tbl_temp_cu2" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
