...
class cfrm_u_pbb_detail_target {
	...
	function cfrm_u_pbb_detail_target() {
		...
		// form_a_1_sequence
		$this->form_a_1_sequence = new cField('frm_u_pbb_detail_target', 'frm_u_pbb_detail_target', 'x_form_a_1_sequence', 'form_a_1_sequence', '`form_a_1_sequence`', 3, -1, FALSE, '`form_a_1_sequence`', FALSE, FALSE, 'FORMATTED TEXT');
		...
		// t_startDate
		$this->t_startDate = new cField('frm_u_pbb_detail_target', 'frm_u_pbb_detail_target', 'x_t_startDate', 't_startDate', '`t_startDate`', 135, 6, FALSE, '`t_startDate`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->t_startDate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateMDY"));
		$this->fields['t_startDate'] =& $this->t_startDate;

		// a_startDate
		$this->a_startDate = new cField('frm_u_pbb_detail_target', 'frm_u_pbb_detail_target', 'x_a_startDate', 'a_startDate', '`a_startDate`', 135, 6, FALSE, '`a_startDate`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "frm_u_rep_t_completion_status_unit_mfo") {
	...
	// Master filter
	function SqlMasterFilter_frm_u_rep_t_completion_status_unit_mfo() {
		return "`units_id`=@units_id@ AND `focal_person_id`=@focal_person_id@ AND `mfo`=@mfo@";
	}

	// Detail filter
	function SqlDetailFilter_frm_u_rep_t_completion_status_unit_mfo() {
		return "`units_id`=@units_id@ AND `focal_person_id`=@focal_person_id@ AND `mfo`=@mfo@";
	}
...
//			$AddUrl .= "?" . $sUrlParm;
   ...
   // Common render codes
	...
	function AddMasterUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "frm_u_rep_t_completion_status_unit_mfo") {
			$sFilterWrk = $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->AddUserIDFilter($sFilterWrk);
		}
		return $sFilterWrk;
	}
	...
	function AddDetailUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "frm_u_rep_t_completion_status_unit_mfo") {
			$sSubqueryWrk = $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->GetUserIDSubquery($this->units_id, $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->units_id);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
			...
			$sSubqueryWrk = $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->GetUserIDSubquery($this->focal_person_id, $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->focal_person_id);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
					$sFilterWrk = "($sFilterWrk) AND ($sSubqueryWrk)";
				} else {
			...
			$sSubqueryWrk = $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->GetUserIDSubquery($this->mfo, $GLOBALS["frm_u_rep_t_completion_status_unit_mfo"]->mfo);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
					$sFilterWrk = "($sFilterWrk) AND ($sSubqueryWrk)";
				} else {
