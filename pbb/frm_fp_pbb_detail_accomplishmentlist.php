...
<?php
$gsMasterReturnUrl = "frm_fp_rep_a_eligible_status_unit_mfolist.php";
if ($frm_fp_pbb_detail_accomplishment_list->DbMasterFilter <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_a_eligible_status_unit_mfo") {
	if ($frm_fp_pbb_detail_accomplishment_list->MasterRecordExists) {
		if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == $frm_fp_pbb_detail_accomplishment->TableVar) $gsMasterReturnUrl .= "?" . UP_TABLE_SHOW_MASTER . "=";
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $frm_fp_rep_a_eligible_status_unit_mfo->TableCaption() ?>
...
<?php } ?>
<?php include_once "frm_fp_rep_a_eligible_status_unit_mfomaster.php" ?>
<?php
	}
}
?>
<?php
$gsMasterReturnUrl = "frm_sam_rep_a_monitor_above_100list.php";
if ($frm_fp_pbb_detail_accomplishment_list->DbMasterFilter <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
	if ($frm_fp_pbb_detail_accomplishment_list->MasterRecordExists) {
		if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == $frm_fp_pbb_detail_accomplishment->TableVar) $gsMasterReturnUrl .= "?" . UP_TABLE_SHOW_MASTER . "=";
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $frm_sam_rep_a_monitor_above_100->TableCaption() ?>
&nbsp;&nbsp;<?php $frm_fp_pbb_detail_accomplishment_list->ExportOptions->Render("body"); ?>
</p>
<?php if ($frm_fp_pbb_detail_accomplishment->Export == "") { ?>
<p class="upbudgetofficeclass"><a href="<?php echo $gsMasterReturnUrl ?>"><?php echo $Language->Phrase("BackToMasterPage") ?></a></p>
<?php } ?>
<?php include_once "frm_sam_rep_a_monitor_above_100master.php" ?>
<?php
	}
}
?>
...
if ($frm_fp_pbb_detail_accomplishment_list->DbMasterFilter <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
	if ($frm_fp_pbb_detail_accomplishment_list->MasterRecordExists) {
		if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == $frm_fp_pbb_detail_accomplishment->TableVar) $gsMasterReturnUrl .= "?" . UP_TABLE_SHOW_MASTER . "=";
?>
...
<?php if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "") { ?>
&nbsp;&nbsp;<?php $frm_fp_pbb_detail_accomplishment_list->ExportOptions->Render("body"); ?>
<?php } ?>
</p>
...
class cfrm_fp_pbb_detail_accomplishment_list {
	...
	function cfrm_fp_pbb_detail_accomplishment_list() {
		...
		if (!isset($GLOBALS['frm_sam_rep_a_monitor_above_100'])) $GLOBALS['frm_sam_rep_a_monitor_above_100'] = new cfrm_sam_rep_a_monitor_above_100();
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_fp_pbb_detail_accomplishment->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_fp_pbb_detail_accomplishment->Export = $_POST["exporttype"];
		} else {
			$frm_fp_pbb_detail_accomplishment->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_fp_pbb_detail_accomplishment->Export; // Get export parameter, used in header
		$gsExportFile = $frm_fp_pbb_detail_accomplishment->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_fp_pbb_detail_accomplishment->Export == "csv") {
	...
	function Page_Main() {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			...
			if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100")
				$this->DbMasterFilter = $frm_fp_pbb_detail_accomplishment->AddMasterUserIDFilter($this->DbMasterFilter, "frm_sam_rep_a_monitor_above_100"); // Add master User ID filter
			if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu")
				$this->DbMasterFilter = $frm_fp_pbb_detail_accomplishment->AddMasterUserIDFilter($this->DbMasterFilter, "frm_fp_rep_ta_form_a_cu"); // Add master User ID filter
		}
		...
		// Load master record
		if ($frm_fp_pbb_detail_accomplishment->getMasterFilter() <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
			global $frm_sam_rep_a_monitor_above_100;
			$rsmaster = $frm_sam_rep_a_monitor_above_100->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate($frm_fp_pbb_detail_accomplishment->getReturnUrl()); // Return to caller
			} else {
				$frm_sam_rep_a_monitor_above_100->LoadListRowValues($rsmaster);
				$frm_sam_rep_a_monitor_above_100->RowType = UP_ROWTYPE_MASTER; // Master row
				$frm_sam_rep_a_monitor_above_100->RenderListRow();
				$rsmaster->Close();
			}
		}

		// Load master record
		if ($frm_fp_pbb_detail_accomplishment->getMasterFilter() <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
		...
		// Export data only
		if (in_array($frm_fp_pbb_detail_accomplishment->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_fp_pbb_detail_accomplishment->Export <> "email")
	...
	function ResetCmd() {
		...
		if (@$_GET["cmd"] <> "") {
			...
			if (strtolower($sCmd) == "resetall") {
				...
				$frm_fp_pbb_detail_accomplishment->units_id->setSessionValue("");
				$frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue("");
				$frm_fp_pbb_detail_accomplishment->focal_person_id->setSessionValue("");
				$frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue("");
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_frm_fp_pbb_detail_accomplishment\" id=\"emf_frm_fp_pbb_detail_accomplishment\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_fp_pbb_detail_accomplishment',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_fp_pbb_detail_accomplishmentlist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_fp_pbb_detail_accomplishment->Export <> "" ||
			$frm_fp_pbb_detail_accomplishment->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_fp_pbb_detail_accomplishment->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_fp_pbb_detail_accomplishment->ExportAll) {
		...
		if ($frm_fp_pbb_detail_accomplishment->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_fp_pbb_detail_accomplishment, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_fp_pbb_detail_accomplishment->getMasterFilter() <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_a_eligible_status_unit_mfo") {
			global $frm_fp_rep_a_eligible_status_unit_mfo;
			$rsmaster = $frm_fp_rep_a_eligible_status_unit_mfo->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_fp_pbb_detail_accomplishment->Export == "xml") {
					$ParentTable = "frm_fp_rep_a_eligible_status_unit_mfo";
					$frm_fp_rep_a_eligible_status_unit_mfo->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_fp_pbb_detail_accomplishment->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_fp_rep_a_eligible_status_unit_mfo->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_fp_pbb_detail_accomplishment->getMasterFilter() <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
			global $frm_sam_rep_a_monitor_above_100;
			$rsmaster = $frm_sam_rep_a_monitor_above_100->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_fp_pbb_detail_accomplishment->Export == "xml") {
					$ParentTable = "frm_sam_rep_a_monitor_above_100";
					$frm_sam_rep_a_monitor_above_100->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_fp_pbb_detail_accomplishment->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_sam_rep_a_monitor_above_100->ExportDocument($ExportDoc, $rsmaster, 1, 1);
						$ExportDoc->ExportEmptyLine();
					}
					$ExportDoc->ChangeStyle($ExportStyle); // Restore
				}
		...
		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_fp_pbb_detail_accomplishment->getMasterFilter() <> "" && $frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
			global $frm_fp_rep_ta_form_a_cu;
		...
		if ($frm_fp_pbb_detail_accomplishment->Export == "xml") {
			$frm_fp_pbb_detail_accomplishment->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_fp_pbb_detail_accomplishment->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_fp_pbb_detail_accomplishment->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_fp_pbb_detail_accomplishment->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_fp_pbb_detail_accomplishment->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_fp_pbb_detail_accomplishment->ExportReturnUrl());
		} elseif ($frm_fp_pbb_detail_accomplishment->Export == "pdf") {
	...
	function SetUpMasterParms() {
		...
		if (@$_GET[UP_TABLE_SHOW_MASTER] <> "") {
			...
			if ($sMasterTblVar == "frm_sam_rep_a_monitor_above_100") {
				$bValidMaster = TRUE;
				if (@$_GET["units_id"] <> "") {
					$GLOBALS["frm_sam_rep_a_monitor_above_100"]->units_id->setQueryStringValue($_GET["units_id"]);
					$frm_fp_pbb_detail_accomplishment->units_id->setQueryStringValue($GLOBALS["frm_sam_rep_a_monitor_above_100"]->units_id->QueryStringValue);
					$frm_fp_pbb_detail_accomplishment->units_id->setSessionValue($frm_fp_pbb_detail_accomplishment->units_id->QueryStringValue);
					if (!is_numeric($GLOBALS["frm_sam_rep_a_monitor_above_100"]->units_id->QueryStringValue)) $bValidMaster = FALSE;
				} else {
					$bValidMaster = FALSE;
				}
				if (@$_GET["MFO"] <> "") {
					$GLOBALS["frm_sam_rep_a_monitor_above_100"]->MFO->setQueryStringValue($_GET["MFO"]);
					$frm_fp_pbb_detail_accomplishment->mfo_sequence->setQueryStringValue($GLOBALS["frm_sam_rep_a_monitor_above_100"]->MFO->QueryStringValue);
					$frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue($frm_fp_pbb_detail_accomplishment->mfo_sequence->QueryStringValue);
				} else {
					$bValidMaster = FALSE;
				}
			}
			if ($sMasterTblVar == "frm_fp_rep_ta_form_a_cu") {
				...
				if (@$_GET["focal_person_id"] <> "") {
					...
					$frm_fp_pbb_detail_accomplishment->focal_person_id->setQueryStringValue($GLOBALS["frm_fp_rep_ta_form_a_cu"]->focal_person_id->QueryStringValue);
					$frm_fp_pbb_detail_accomplishment->focal_person_id->setSessionValue($frm_fp_pbb_detail_accomplishment->focal_person_id->QueryStringValue);
				...
				if (@$_GET["Sequence"] <> "") {
					...
					$frm_fp_pbb_detail_accomplishment->mfo_sequence->setQueryStringValue($GLOBALS["frm_fp_rep_ta_form_a_cu"]->Sequence->QueryStringValue);
					$frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue($frm_fp_pbb_detail_accomplishment->mfo_sequence->QueryStringValue);
		...
		if ($bValidMaster) {
			...
			if ($sMasterTblVar <> "frm_sam_rep_a_monitor_above_100") {
				if ($frm_fp_pbb_detail_accomplishment->units_id->QueryStringValue == "") $frm_fp_pbb_detail_accomplishment->units_id->setSessionValue("");
				if ($frm_fp_pbb_detail_accomplishment->mfo_sequence->QueryStringValue == "") $frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue("");
			}
			if ($sMasterTblVar <> "frm_fp_rep_ta_form_a_cu") {
				if ($frm_fp_pbb_detail_accomplishment->focal_person_id->QueryStringValue == "") $frm_fp_pbb_detail_accomplishment->focal_person_id->setSessionValue("");
				if ($frm_fp_pbb_detail_accomplishment->mfo_sequence->QueryStringValue == "") $frm_fp_pbb_detail_accomplishment->mfo_sequence->setSessionValue("");
			}
		}
