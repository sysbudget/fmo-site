...
class cfrm_fp_units_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_fp_units->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_fp_units->Export = $_POST["exporttype"];
		} else {
			$frm_fp_units->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_fp_units->Export; // Get export parameter, used in header
		$gsExportFile = $frm_fp_units->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_fp_units->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_fp_units->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_fp_units->Export <> "email")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_frm_fp_units\" id=\"emf_frm_fp_units\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_fp_units',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_fp_unitslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_fp_units->Export <> "" ||
			$frm_fp_units->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_fp_units->SelectRecordCount();
		} else {
		...
		if ($frm_fp_units->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_fp_units, "h");
		}
		$ParentTable = "";
		if ($bSelectLimit) {
			$StartRec = 1;
		...
		if ($frm_fp_units->Export == "xml") {
			$frm_fp_units->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_fp_units->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_fp_units->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_fp_units->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_fp_units->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_fp_units->ExportReturnUrl());
		} elseif ($frm_fp_units->Export == "pdf") {
