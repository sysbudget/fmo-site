...
class cfrm_fp_rep_ta_form_a_cu_list {
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList('frm_fp_pbb_detail_accomplishment') && $this->ShowOptionLink()) {
			...
			$oListOpt->Body = "<a class=\"upRowLink\" href=\"frm_fp_pbb_detail_accomplishmentlist.php?" . UP_TABLE_SHOW_MASTER . "=frm_fp_rep_ta_form_a_cu&focal_person_id=" . urlencode(strval($frm_fp_rep_ta_form_a_cu->focal_person_id->CurrentValue)) . "&Sequence=" . urlencode(strval($frm_fp_rep_ta_form_a_cu->Sequence->CurrentValue)) . "\">" . $oListOpt->Body . "</a>";
			$links = "";
			if ($links <> "") $oListOpt->Body .= "<br>" . $links;
		}
	...
	function RenderListOptionsExt() {
		...
		$links = "";
		if ($links <> "") $oListOpt->Body .= "<br>" . $links;
		$sSqlWrk = "`focal_person_id`=" . up_AdjustSql($frm_fp_rep_ta_form_a_cu->focal_person_id->CurrentValue) . "";
		$sSqlWrk = $sSqlWrk . " AND " . "`mfo_sequence`=" . up_AdjustSql($frm_fp_rep_ta_form_a_cu->Sequence->CurrentValue) . "";
		$sSqlWrk = TEAencrypt($sSqlWrk, UP_RANDOM_KEY);
		$sSqlWrk = str_replace("'", "\'", $sSqlWrk);
		$sHyperLinkParm = " href=\"frm_fp_pbb_detail_accomplishmentlist.php?" . UP_TABLE_SHOW_MASTER . "=frm_fp_rep_ta_form_a_cu&focal_person_id=" . urlencode(strval($frm_fp_rep_ta_form_a_cu->focal_person_id->CurrentValue)) . "&Sequence=" . urlencode(strval($frm_fp_rep_ta_form_a_cu->Sequence->CurrentValue)) . "\"";
		$oListOpt =& $this->ListOptions->Items["detail_frm_fp_pbb_detail_accomplishment"];
		$oListOpt->Body = $Language->TablePhrase("frm_fp_pbb_detail_accomplishment", "TblCaption");
		$oListOpt->Body = "<a class=\"upRowLink\"" . $sHyperLinkParm . ">" . $oListOpt->Body . "</a>";
		$sHyperLinkParm = " href=\"javascript:void(0);\" name=\"dl%i_frm_fp_rep_ta_form_a_cu_frm_fp_pbb_detail_accomplishment\" id=\"dl%i_frm_fp_rep_ta_form_a_cu_frm_fp_pbb_detail_accomplishment\" onclick=\"up_AjaxShowDetails2(event, this, 'frm_fp_pbb_detail_accomplishmentpreview.php?f=%s')\"";		
		$sHyperLinkParm = str_replace("%i", $this->RowCnt, $sHyperLinkParm);
		$sHyperLinkParm = str_replace("%s", $sSqlWrk, $sHyperLinkParm);
		$oListOpt->Body = "<a" . $sHyperLinkParm . "><img class=\"upPreviewRowImage\" src=\"phpimages/expand.gif\" width=\"9\" height=\"9\" border=\"0\"></a>&nbsp;" . $oListOpt->Body;
		$links = "";
