...
// Create page object
$frm_fp_units_delete = new cfrm_fp_units_delete();
$Page =& $frm_fp_units_delete;

// Page init
$frm_fp_units_delete->Page_Init();

// Page main
$frm_fp_units_delete->Page_Main();
?>
...
// Create page object
var frm_fp_units_delete = new up_Page("frm_fp_units_delete");

// page properties
frm_fp_units_delete.PageID = "delete"; // page ID
frm_fp_units_delete.FormID = "ffrm_fp_unitsdelete"; // form ID
var UP_PAGE_ID = frm_fp_units_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
frm_fp_units_delete.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
frm_fp_units_delete.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
frm_fp_units_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
frm_fp_units_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if ($frm_fp_units_delete->Recordset = $frm_fp_units_delete->LoadRecordset())
	$frm_fp_units_deleteTotalRecs = $frm_fp_units_delete->Recordset->RecordCount(); // Get record count
if ($frm_fp_units_deleteTotalRecs <= 0) { // No record found, exit
	if ($frm_fp_units_delete->Recordset)
		$frm_fp_units_delete->Recordset->Close();
	$frm_fp_units_delete->Page_Terminate("frm_fp_unitslist.php"); // Return to list
}
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $frm_fp_units->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $frm_fp_units->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $frm_fp_units_delete->ShowPageHeader(); ?>
<?php
$frm_fp_units_delete->ShowMessage();
?>
<form action="<?php echo up_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="t" id="t" value="frm_fp_units">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($frm_fp_units_delete->RecKeys as $key) { ?>
...
<?php echo $frm_fp_units->TableCustomInnerHtml ?>
	...
	<tr class="upTableHeader">
		<td valign="top"><?php echo $frm_fp_units->cu_unit_name->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->personnel_count->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->mfo_1->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->mfo_2->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->mfo_3->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->mfo_4->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->mfo_5->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->sto->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->gass->FldCaption() ?></td>
		<td valign="top"><?php echo $frm_fp_units->t_cutOffDate_remarks->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$frm_fp_units_delete->RecCnt = 0;
$i = 0;
while (!$frm_fp_units_delete->Recordset->EOF) {
	$frm_fp_units_delete->RecCnt++;

	// Set row properties
	$frm_fp_units->ResetAttrs();
	$frm_fp_units->RowType = UP_ROWTYPE_VIEW; // View

	// Get the field contents
	$frm_fp_units_delete->LoadRowValues($frm_fp_units_delete->Recordset);

	// Render row
	$frm_fp_units_delete->RenderRow();
?>
	<tr<?php echo $frm_fp_units->RowAttributes() ?>>
		<td<?php echo $frm_fp_units->cu_unit_name->CellAttributes() ?>>
<div<?php echo $frm_fp_units->cu_unit_name->ViewAttributes() ?>><?php echo $frm_fp_units->cu_unit_name->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->personnel_count->CellAttributes() ?>>
<div<?php echo $frm_fp_units->personnel_count->ViewAttributes() ?>><?php echo $frm_fp_units->personnel_count->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->mfo_1->CellAttributes() ?>>
<div<?php echo $frm_fp_units->mfo_1->ViewAttributes() ?>><?php echo $frm_fp_units->mfo_1->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->mfo_2->CellAttributes() ?>>
<div<?php echo $frm_fp_units->mfo_2->ViewAttributes() ?>><?php echo $frm_fp_units->mfo_2->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->mfo_3->CellAttributes() ?>>
<div<?php echo $frm_fp_units->mfo_3->ViewAttributes() ?>><?php echo $frm_fp_units->mfo_3->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->mfo_4->CellAttributes() ?>>
<div<?php echo $frm_fp_units->mfo_4->ViewAttributes() ?>><?php echo $frm_fp_units->mfo_4->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->mfo_5->CellAttributes() ?>>
<div<?php echo $frm_fp_units->mfo_5->ViewAttributes() ?>><?php echo $frm_fp_units->mfo_5->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->sto->CellAttributes() ?>>
<div<?php echo $frm_fp_units->sto->ViewAttributes() ?>><?php echo $frm_fp_units->sto->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->gass->CellAttributes() ?>>
<div<?php echo $frm_fp_units->gass->ViewAttributes() ?>><?php echo $frm_fp_units->gass->ListViewValue() ?></div></td>
		<td<?php echo $frm_fp_units->t_cutOffDate_remarks->CellAttributes() ?>>
<div<?php echo $frm_fp_units->t_cutOffDate_remarks->ViewAttributes() ?>><?php echo $frm_fp_units->t_cutOffDate_remarks->ListViewValue() ?></div></td>
	</tr>
<?php
	$frm_fp_units_delete->Recordset->MoveNext();
}
$frm_fp_units_delete->Recordset->Close();
?>
</tbody>
...
<?php
$frm_fp_units_delete->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$frm_fp_units_delete->Page_Terminate();
?>
<?php
...
class cfrm_fp_units_delete {
	...
	// Page object name
	var $PageObjName = 'frm_fp_units_delete';
	...
	//
	function Page_Main() {
		global $Language, $frm_fp_units;

		// Load key parameters
		$this->RecKeys = $frm_fp_units->GetRecordKeys(); // Load record keys
		$sFilter = $frm_fp_units->GetKeyFilter();
		if ($sFilter == "")
			$this->Page_Terminate("frm_fp_unitslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in frm_fp_units class, frm_fp_unitsinfo.php

		$frm_fp_units->CurrentFilter = $sFilter;

		// Get action
		if (@$_POST["a_delete"] <> "") {
			$frm_fp_units->CurrentAction = $_POST["a_delete"];
		} else {
			$frm_fp_units->CurrentAction = "I"; // Display record
		}
		switch ($frm_fp_units->CurrentAction) {
			case "D": // Delete
				$frm_fp_units->SendEmail = TRUE; // Send email on delete success
				if ($this->DeleteRows()) { // delete rows
					$this->setSuccessMessage($Language->Phrase("DeleteSuccess")); // Set up success message
					$this->Page_Terminate($frm_fp_units->getReturnUrl()); // Return to caller
				}
		}
	}
...
// No functions
	...
	function RenderRow() {
		...
		// t_startDate
		$frm_fp_units->t_startDate->CellCssStyle = "white-space: nowrap;";
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $frm_fp_units->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$conn->raiseErrorFn = 'up_ErrorFn';
				$DeleteRows = $conn->Execute($frm_fp_units->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
