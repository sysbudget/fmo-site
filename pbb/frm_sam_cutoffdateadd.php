...
// Create page object
$frm_sam_cutoffdate_add = new cfrm_sam_cutoffdate_add();
$Page =& $frm_sam_cutoffdate_add;

// Page init
$frm_sam_cutoffdate_add->Page_Init();

// Page main
$frm_sam_cutoffdate_add->Page_Main();
?>
...
// Create page object
var frm_sam_cutoffdate_add = new up_Page("frm_sam_cutoffdate_add");

// page properties
frm_sam_cutoffdate_add.PageID = "add"; // page ID
frm_sam_cutoffdate_add.FormID = "ffrm_sam_cutoffdateadd"; // form ID
var UP_PAGE_ID = frm_sam_cutoffdate_add.PageID; // for backward compatibility

// extend page with ValidateForm function
frm_sam_cutoffdate_add.ValidateForm = function(fobj) {
...
frm_sam_cutoffdate_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
frm_sam_cutoffdate_add.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
frm_sam_cutoffdate_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
frm_sam_cutoffdate_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $frm_sam_cutoffdate->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $frm_sam_cutoffdate->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $frm_sam_cutoffdate_add->ShowPageHeader(); ?>
<?php
$frm_sam_cutoffdate_add->ShowMessage();
?>
<form name="ffrm_sam_cutoffdateadd" id="ffrm_sam_cutoffdateadd" action="<?php echo up_CurrentPage() ?>" method="post" onsubmit="return frm_sam_cutoffdate_add.ValidateForm(this);">
<p>
...
<?php if ($frm_sam_cutoffdate->focal_person_office->Visible) { // focal_person_office ?>
	<tr id="r_focal_person_office"<?php echo $frm_sam_cutoffdate->RowAttributes() ?>>
		<td class="upTableHeader"><?php echo $frm_sam_cutoffdate->focal_person_office->FldCaption() ?></td>
		<td<?php echo $frm_sam_cutoffdate->focal_person_office->CellAttributes() ?>><span id="el_focal_person_office">
<input type="text" name="x_focal_person_office" id="x_focal_person_office" size="30" maxlength="255" value="<?php echo $frm_sam_cutoffdate->focal_person_office->EditValue ?>"<?php echo $frm_sam_cutoffdate->focal_person_office->EditAttributes() ?>>
</span><?php echo $frm_sam_cutoffdate->focal_person_office->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$frm_sam_cutoffdate_add->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$frm_sam_cutoffdate_add->Page_Terminate();
?>
<?php
...
class cfrm_sam_cutoffdate_add {
	...
	// Page object name
	var $PageObjName = 'frm_sam_cutoffdate_add';
	...
	//
	function cfrm_sam_cutoffdate_add() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$frm_sam_cutoffdate->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$frm_sam_cutoffdate->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["cutOffDate_id"] != "") {
				...
				$frm_sam_cutoffdate->setKey("cutOffDate_id", $frm_sam_cutoffdate->cutOffDate_id->CurrentValue); // Set up key
			} else {
				$frm_sam_cutoffdate->setKey("cutOffDate_id", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$frm_sam_cutoffdate->CurrentAction = "C"; // Copy record
			} else {
				$frm_sam_cutoffdate->CurrentAction = "I"; // Display blank record
		...
		switch ($frm_sam_cutoffdate->CurrentAction) {
			...
			case "A": // ' Add new record
				$frm_sam_cutoffdate->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $frm_sam_cutoffdate->getReturnUrl();
					if (up_GetPageName($sReturnUrl) == "frm_sam_cutoffdateview.php")
						$sReturnUrl = $frm_sam_cutoffdate->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$frm_sam_cutoffdate->RowType = UP_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $frm_sam_cutoffdate;
		$frm_sam_cutoffdate->focal_person_id->CurrentValue = NULL;
		$frm_sam_cutoffdate->focal_person_id->OldValue = $frm_sam_cutoffdate->focal_person_id->CurrentValue;
		$frm_sam_cutoffdate->focal_person_office->CurrentValue = NULL;
		$frm_sam_cutoffdate->focal_person_office->OldValue = $frm_sam_cutoffdate->focal_person_office->CurrentValue;
		$frm_sam_cutoffdate->t_startDate->CurrentValue = NULL;
		$frm_sam_cutoffdate->t_startDate->OldValue = $frm_sam_cutoffdate->t_startDate->CurrentValue;
		$frm_sam_cutoffdate->t_cutOffDate->CurrentValue = NULL;
		$frm_sam_cutoffdate->t_cutOffDate->OldValue = $frm_sam_cutoffdate->t_cutOffDate->CurrentValue;
		$frm_sam_cutoffdate->t_cutOffDate_fp->CurrentValue = NULL;
		$frm_sam_cutoffdate->t_cutOffDate_fp->OldValue = $frm_sam_cutoffdate->t_cutOffDate_fp->CurrentValue;
		$frm_sam_cutoffdate->t_cutOffDate_remarks->CurrentValue = NULL;
		$frm_sam_cutoffdate->t_cutOffDate_remarks->OldValue = $frm_sam_cutoffdate->t_cutOffDate_remarks->CurrentValue;
		$frm_sam_cutoffdate->a_startDate->CurrentValue = NULL;
		$frm_sam_cutoffdate->a_startDate->OldValue = $frm_sam_cutoffdate->a_startDate->CurrentValue;
		$frm_sam_cutoffdate->a_cutOffDate->CurrentValue = NULL;
		$frm_sam_cutoffdate->a_cutOffDate->OldValue = $frm_sam_cutoffdate->a_cutOffDate->CurrentValue;
		$frm_sam_cutoffdate->a_cutOffDate_fp->CurrentValue = NULL;
		$frm_sam_cutoffdate->a_cutOffDate_fp->OldValue = $frm_sam_cutoffdate->a_cutOffDate_fp->CurrentValue;
		$frm_sam_cutoffdate->a_cutOffDate_remarks->CurrentValue = NULL;
		$frm_sam_cutoffdate->a_cutOffDate_remarks->OldValue = $frm_sam_cutoffdate->a_cutOffDate_remarks->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		if (!$frm_sam_cutoffdate->focal_person_office->FldIsDetailKey) {
			$frm_sam_cutoffdate->focal_person_office->setFormValue($objForm->GetValue("x_focal_person_office"));
		}
	...
	function RestoreFormValues() {
		...
		$frm_sam_cutoffdate->focal_person_office->CurrentValue = $frm_sam_cutoffdate->focal_person_office->FormValue;
	...
	function RenderRow() {
		...
		if ($frm_sam_cutoffdate->RowType == UP_ROWTYPE_VIEW) { // View row
		...
		} elseif ($frm_sam_cutoffdate->RowType == UP_ROWTYPE_ADD) { // Add row
			...
			// focal_person_office
			$frm_sam_cutoffdate->focal_person_office->EditCustomAttributes = "";
			$frm_sam_cutoffdate->focal_person_office->EditValue = up_HtmlEncode($frm_sam_cutoffdate->focal_person_office->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// focal_person_id
		$frm_sam_cutoffdate->focal_person_id->SetDbValueDef($rsnew, $frm_sam_cutoffdate->focal_person_id->CurrentValue, NULL, FALSE);

		// focal_person_office
		$frm_sam_cutoffdate->focal_person_office->SetDbValueDef($rsnew, $frm_sam_cutoffdate->focal_person_office->CurrentValue, NULL, FALSE);

		// t_startDate
		$frm_sam_cutoffdate->t_startDate->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->t_startDate->CurrentValue, 6), NULL, FALSE);

		// t_cutOffDate
		$frm_sam_cutoffdate->t_cutOffDate->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->t_cutOffDate->CurrentValue, 6), NULL, FALSE);

		// t_cutOffDate_fp
		$frm_sam_cutoffdate->t_cutOffDate_fp->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->t_cutOffDate_fp->CurrentValue, 6), NULL, FALSE);

		// t_cutOffDate_remarks
		$frm_sam_cutoffdate->t_cutOffDate_remarks->SetDbValueDef($rsnew, $frm_sam_cutoffdate->t_cutOffDate_remarks->CurrentValue, NULL, FALSE);

		// a_startDate
		$frm_sam_cutoffdate->a_startDate->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->a_startDate->CurrentValue, 6), NULL, FALSE);

		// a_cutOffDate
		$frm_sam_cutoffdate->a_cutOffDate->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->a_cutOffDate->CurrentValue, 6), NULL, FALSE);

		// a_cutOffDate_fp
		$frm_sam_cutoffdate->a_cutOffDate_fp->SetDbValueDef($rsnew, up_UnFormatDateTime($frm_sam_cutoffdate->a_cutOffDate_fp->CurrentValue, 6), NULL, FALSE);

		// a_cutOffDate_remarks
		$frm_sam_cutoffdate->a_cutOffDate_remarks->SetDbValueDef($rsnew, $frm_sam_cutoffdate->a_cutOffDate_remarks->CurrentValue, NULL, FALSE);

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $frm_sam_cutoffdate->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'up_ErrorFn';
			$AddRow = $conn->Execute($frm_sam_cutoffdate->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$frm_sam_cutoffdate->cutOffDate_id->setDbValue($conn->Insert_ID());
			$rsnew['cutOffDate_id'] = $frm_sam_cutoffdate->cutOffDate_id->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$frm_sam_cutoffdate->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
