...
// Create page object
$frm_sam_units_add = new cfrm_sam_units_add();
$Page =& $frm_sam_units_add;

// Page init
$frm_sam_units_add->Page_Init();

// Page main
$frm_sam_units_add->Page_Main();
?>
...
// Create page object
var frm_sam_units_add = new up_Page("frm_sam_units_add");

// page properties
frm_sam_units_add.PageID = "add"; // page ID
frm_sam_units_add.FormID = "ffrm_sam_unitsadd"; // form ID
var UP_PAGE_ID = frm_sam_units_add.PageID; // for backward compatibility

// extend page with ValidateForm function
frm_sam_units_add.ValidateForm = function(fobj) {
...
frm_sam_units_add.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
frm_sam_units_add.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
frm_sam_units_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
frm_sam_units_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $frm_sam_units->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $frm_sam_units->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $frm_sam_units_add->ShowPageHeader(); ?>
<?php
$frm_sam_units_add->ShowMessage();
?>
<form name="ffrm_sam_unitsadd" id="ffrm_sam_unitsadd" action="<?php echo up_CurrentPage() ?>" method="post" onsubmit="return frm_sam_units_add.ValidateForm(this);">
<p>
...
<?php if (strval($frm_sam_units->focal_person_id->getSessionValue()) <> "") { ?>
<input type="hidden" name="x_focal_person_id" id="x_focal_person_id" value="<?php echo up_HtmlEncode(strval($frm_sam_units->focal_person_id->getSessionValue())) ?>">
<?php } ?>
<p>
<input type="submit" name="btnAction" id="btnAction" value="<?php echo up_BtnCaption($Language->Phrase("AddBtn")) ?>">
</form>
<?php
$frm_sam_units_add->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$frm_sam_units_add->Page_Terminate();
?>
<?php
...
class cfrm_sam_units_add {
	...
	// Page object name
	var $PageObjName = 'frm_sam_units_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			$frm_sam_units->CurrentAction = $_POST["a_add"]; // Get form action
			$this->CopyRecord = $this->LoadOldRecord(); // Load old recordset
			$this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$frm_sam_units->CurrentAction = "I"; // Form error, reset action
			...
			if (@$_GET["units_id"] != "") {
				...
				$frm_sam_units->setKey("units_id", $frm_sam_units->units_id->CurrentValue); // Set up key
			} else {
				$frm_sam_units->setKey("units_id", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
				$frm_sam_units->CurrentAction = "C"; // Copy record
			} else {
				$frm_sam_units->CurrentAction = "I"; // Display blank record
		...
		switch ($frm_sam_units->CurrentAction) {
			...
			case "A": // ' Add new record
				$frm_sam_units->SendEmail = TRUE; // Send email on add success
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					$this->setSuccessMessage($Language->Phrase("AddSuccess")); // Set up success message
					$sReturnUrl = $frm_sam_units->getReturnUrl();
					if (up_GetPageName($sReturnUrl) == "frm_sam_unitsview.php")
						$sReturnUrl = $frm_sam_units->ViewUrl(); // View paging, return to view page with keyurl directly
		...
		$frm_sam_units->RowType = UP_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $frm_sam_units;
		$frm_sam_units->unit_name->CurrentValue = NULL;
		$frm_sam_units->unit_name->OldValue = $frm_sam_units->unit_name->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// unit_name
		$frm_sam_units->unit_name->SetDbValueDef($rsnew, $frm_sam_units->unit_name->CurrentValue, NULL, FALSE);
