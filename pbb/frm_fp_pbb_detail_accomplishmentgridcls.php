...
class cfrm_fp_pbb_detail_accomplishment_grid {
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			...
			$sFilter = $frm_fp_pbb_detail_accomplishment->SqlMasterFilter_frm_sam_rep_a_monitor_above_100();
			if (strval($frm_fp_pbb_detail_accomplishment->units_id->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
				$sFilter = str_replace("@units_id@", up_AdjustSql($frm_fp_pbb_detail_accomplishment->units_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if (strval($frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
				$sFilter = str_replace("@MFO@", up_AdjustSql($frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
				$rsmaster = $GLOBALS["frm_sam_rep_a_monitor_above_100"]->LoadRs($sFilter);
				$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->MasterRecordExists) {
					$sMasterUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedMasterUserID"));
					$sMasterUserIdMsg = str_replace("%f", $sFilter, $sMasterUserIdMsg);
			...
			$sFilter = $frm_fp_pbb_detail_accomplishment->SqlMasterFilter_frm_fp_rep_ta_form_a_cu();
			if (strval($frm_fp_pbb_detail_accomplishment->focal_person_id->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
				$sFilter = str_replace("@focal_person_id@", up_AdjustSql($frm_fp_pbb_detail_accomplishment->focal_person_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if (strval($frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
				$sFilter = str_replace("@Sequence@", up_AdjustSql($frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
		...
		// Set up foreign key field value from Session
			...
			if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_sam_rep_a_monitor_above_100") {
				$frm_fp_pbb_detail_accomplishment->units_id->CurrentValue = $frm_fp_pbb_detail_accomplishment->units_id->getSessionValue();
				$frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue = $frm_fp_pbb_detail_accomplishment->mfo_sequence->getSessionValue();
			}
			if ($frm_fp_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_fp_rep_ta_form_a_cu") {
				$frm_fp_pbb_detail_accomplishment->focal_person_id->CurrentValue = $frm_fp_pbb_detail_accomplishment->focal_person_id->getSessionValue();
				$frm_fp_pbb_detail_accomplishment->mfo_sequence->CurrentValue = $frm_fp_pbb_detail_accomplishment->mfo_sequence->getSessionValue();
			}
		$rsnew = array();
		...
		// focal_person_id
		if ($frm_fp_pbb_detail_accomplishment->focal_person_id->getSessionValue() <> "") {
			$rsnew['focal_person_id'] = $frm_fp_pbb_detail_accomplishment->focal_person_id->getSessionValue();
		} elseif (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin
			$rsnew['focal_person_id'] = CurrentUserID();
		}

		// mfo_sequence
		if ($frm_fp_pbb_detail_accomplishment->mfo_sequence->getSessionValue() <> "") {
			$rsnew['mfo_sequence'] = $frm_fp_pbb_detail_accomplishment->mfo_sequence->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "frm_sam_rep_a_monitor_above_100") {
			$frm_fp_pbb_detail_accomplishment->units_id->Visible = FALSE;
			if ($GLOBALS["frm_sam_rep_a_monitor_above_100"]->EventCancelled) $frm_fp_pbb_detail_accomplishment->EventCancelled = TRUE;
			$frm_fp_pbb_detail_accomplishment->mfo_sequence->Visible = FALSE;
			if ($GLOBALS["frm_sam_rep_a_monitor_above_100"]->EventCancelled) $frm_fp_pbb_detail_accomplishment->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "frm_fp_rep_ta_form_a_cu") {
			$frm_fp_pbb_detail_accomplishment->focal_person_id->Visible = FALSE;
			if ($GLOBALS["frm_fp_rep_ta_form_a_cu"]->EventCancelled) $frm_fp_pbb_detail_accomplishment->EventCancelled = TRUE;
			$frm_fp_pbb_detail_accomplishment->mfo_sequence->Visible = FALSE;
			if ($GLOBALS["frm_fp_rep_ta_form_a_cu"]->EventCancelled) $frm_fp_pbb_detail_accomplishment->EventCancelled = TRUE;
		}
