...
class cfrm_1_t_fi_pbb_detail_contributor_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_1_t_fi_pbb_detail_contributor->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_1_t_fi_pbb_detail_contributor->Export = $_POST["exporttype"];
		} else {
			$frm_1_t_fi_pbb_detail_contributor->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_1_t_fi_pbb_detail_contributor->Export; // Get export parameter, used in header
		$gsExportFile = $frm_1_t_fi_pbb_detail_contributor->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_1_t_fi_pbb_detail_contributor->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_1_t_fi_pbb_detail_contributor->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_1_t_fi_pbb_detail_contributor->Export <> "email")
	...
	function SetupExportOptions() {
		global $Language, $frm_1_t_fi_pbb_detail_contributor;
		...
		$item->Body = "<a name=\"emf_frm_1_t_fi_pbb_detail_contributor\" id=\"emf_frm_1_t_fi_pbb_detail_contributor\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_1_t_fi_pbb_detail_contributor',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_1_t_fi_pbb_detail_contributorlist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_1_t_fi_pbb_detail_contributor->Export <> "" ||
			$frm_1_t_fi_pbb_detail_contributor->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_1_t_fi_pbb_detail_contributor->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_1_t_fi_pbb_detail_contributor->ExportAll) {
		...
		if ($frm_1_t_fi_pbb_detail_contributor->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_1_t_fi_pbb_detail_contributor, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_1_t_fi_pbb_detail_contributor->getMasterFilter() <> "" && $frm_1_t_fi_pbb_detail_contributor->getCurrentMasterTable() == "frm_1_t_fi_pbb_detail_delivery") {
			global $frm_1_t_fi_pbb_detail_delivery;
			$rsmaster = $frm_1_t_fi_pbb_detail_delivery->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_1_t_fi_pbb_detail_contributor->Export == "xml") {
					$ParentTable = "frm_1_t_fi_pbb_detail_delivery";
					$frm_1_t_fi_pbb_detail_delivery->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_1_t_fi_pbb_detail_contributor->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_1_t_fi_pbb_detail_delivery->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_1_t_fi_pbb_detail_contributor->getMasterFilter() <> "" && $frm_1_t_fi_pbb_detail_contributor->getCurrentMasterTable() == "frm_1_t_fi_contributor_pi_status") {
			global $frm_1_t_fi_contributor_pi_status;
			$rsmaster = $frm_1_t_fi_contributor_pi_status->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_1_t_fi_pbb_detail_contributor->Export == "xml") {
					$ParentTable = "frm_1_t_fi_contributor_pi_status";
					$frm_1_t_fi_contributor_pi_status->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_1_t_fi_pbb_detail_contributor->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_1_t_fi_contributor_pi_status->ExportDocument($ExportDoc, $rsmaster, 1, 1);
						$ExportDoc->ExportEmptyLine();
					}
					$ExportDoc->ChangeStyle($ExportStyle); // Restore
				}
		...
		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_1_t_fi_pbb_detail_contributor->getMasterFilter() <> "" && $frm_1_t_fi_pbb_detail_contributor->getCurrentMasterTable() == "frm_2_a_fi_pbb_detail_delivery") {
			global $frm_2_a_fi_pbb_detail_delivery;
		...
		if ($frm_1_t_fi_pbb_detail_contributor->Export == "xml") {
			$frm_1_t_fi_pbb_detail_contributor->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_1_t_fi_pbb_detail_contributor->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_1_t_fi_pbb_detail_contributor->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_1_t_fi_pbb_detail_contributor->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_1_t_fi_pbb_detail_contributor->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_1_t_fi_pbb_detail_contributor->ExportReturnUrl());
		} elseif ($frm_1_t_fi_pbb_detail_contributor->Export == "pdf") {
