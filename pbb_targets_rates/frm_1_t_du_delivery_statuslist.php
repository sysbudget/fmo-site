...
class cfrm_1_t_du_delivery_status_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_1_t_du_delivery_status->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_1_t_du_delivery_status->Export = $_POST["exporttype"];
		} else {
			$frm_1_t_du_delivery_status->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_1_t_du_delivery_status->Export; // Get export parameter, used in header
		$gsExportFile = $frm_1_t_du_delivery_status->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_1_t_du_delivery_status->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_1_t_du_delivery_status->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_1_t_du_delivery_status->Export <> "email")
	...
	function SetupExportOptions() {
		global $Language, $frm_1_t_du_delivery_status;
		...
		$item->Body = "<a name=\"emf_frm_1_t_du_delivery_status\" id=\"emf_frm_1_t_du_delivery_status\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_1_t_du_delivery_status',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_1_t_du_delivery_statuslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_1_t_du_delivery_status->Export <> "" ||
			$frm_1_t_du_delivery_status->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_1_t_du_delivery_status->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_1_t_du_delivery_status->ExportAll) {
		...
		if ($frm_1_t_du_delivery_status->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_1_t_du_delivery_status, "h");
		}
		$ParentTable = "";
		if ($bSelectLimit) {
			$StartRec = 1;
		...
		if ($frm_1_t_du_delivery_status->Export == "xml") {
			$frm_1_t_du_delivery_status->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_1_t_du_delivery_status->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_1_t_du_delivery_status->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_1_t_du_delivery_status->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_1_t_du_delivery_status->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_1_t_du_delivery_status->ExportReturnUrl());
		} elseif ($frm_1_t_du_delivery_status->Export == "pdf") {
