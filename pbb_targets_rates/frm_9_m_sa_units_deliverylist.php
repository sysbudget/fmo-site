...
class cfrm_9_m_sa_units_delivery_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_9_m_sa_units_delivery->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_9_m_sa_units_delivery->Export = $_POST["exporttype"];
		} else {
			$frm_9_m_sa_units_delivery->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_9_m_sa_units_delivery->Export; // Get export parameter, used in header
		$gsExportFile = $frm_9_m_sa_units_delivery->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_9_m_sa_units_delivery->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_9_m_sa_units_delivery->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_9_m_sa_units_delivery->Export <> "email")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_frm_9_m_sa_units_delivery\" id=\"emf_frm_9_m_sa_units_delivery\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_9_m_sa_units_delivery',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_9_m_sa_units_deliverylist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_9_m_sa_units_delivery->Export <> "" ||
			$frm_9_m_sa_units_delivery->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_9_m_sa_units_delivery->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_9_m_sa_units_delivery->ExportAll) {
		...
		if ($frm_9_m_sa_units_delivery->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_9_m_sa_units_delivery, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_9_m_sa_units_delivery->getMasterFilter() <> "" && $frm_9_m_sa_units_delivery->getCurrentMasterTable() == "frm_9_m_sa_units_cu") {
			global $frm_9_m_sa_units_cu;
			$rsmaster = $frm_9_m_sa_units_cu->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_9_m_sa_units_delivery->Export == "xml") {
					$ParentTable = "frm_9_m_sa_units_cu";
					$frm_9_m_sa_units_cu->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_9_m_sa_units_delivery->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_9_m_sa_units_cu->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		if ($frm_9_m_sa_units_delivery->Export == "xml") {
			$frm_9_m_sa_units_delivery->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_9_m_sa_units_delivery->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_9_m_sa_units_delivery->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_9_m_sa_units_delivery->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_9_m_sa_units_delivery->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_9_m_sa_units_delivery->ExportReturnUrl());
		} elseif ($frm_9_m_sa_units_delivery->Export == "pdf") {
