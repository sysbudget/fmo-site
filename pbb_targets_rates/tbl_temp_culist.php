...
class ctbl_temp_cu_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$tbl_temp_cu->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$tbl_temp_cu->Export = $_POST["exporttype"];
		} else {
			$tbl_temp_cu->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $tbl_temp_cu->Export; // Get export parameter, used in header
		$gsExportFile = $tbl_temp_cu->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($tbl_temp_cu->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($tbl_temp_cu->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($tbl_temp_cu->Export <> "email")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_tbl_temp_cu\" id=\"emf_tbl_temp_cu\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_tbl_temp_cu',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ftbl_temp_culist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($tbl_temp_cu->Export <> "" ||
			$tbl_temp_cu->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $tbl_temp_cu->SelectRecordCount();
		} else {
		...
		if ($tbl_temp_cu->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($tbl_temp_cu, "h");
		}
		$ParentTable = "";
		if ($bSelectLimit) {
			$StartRec = 1;
		...
		if ($tbl_temp_cu->Export == "xml") {
			$tbl_temp_cu->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$tbl_temp_cu->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($tbl_temp_cu->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($tbl_temp_cu->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($tbl_temp_cu->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($tbl_temp_cu->ExportReturnUrl());
		} elseif ($tbl_temp_cu->Export == "pdf") {
