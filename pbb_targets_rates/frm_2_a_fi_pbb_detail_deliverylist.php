...
class cfrm_2_a_fi_pbb_detail_delivery_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_2_a_fi_pbb_detail_delivery->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_2_a_fi_pbb_detail_delivery->Export = $_POST["exporttype"];
		} else {
			$frm_2_a_fi_pbb_detail_delivery->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_2_a_fi_pbb_detail_delivery->Export; // Get export parameter, used in header
		$gsExportFile = $frm_2_a_fi_pbb_detail_delivery->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_2_a_fi_pbb_detail_delivery->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_2_a_fi_pbb_detail_delivery->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_2_a_fi_pbb_detail_delivery->Export <> "email")
	...
	function SetupExportOptions() {
		global $Language, $frm_2_a_fi_pbb_detail_delivery;
		...
		$item->Body = "<a name=\"emf_frm_2_a_fi_pbb_detail_delivery\" id=\"emf_frm_2_a_fi_pbb_detail_delivery\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_2_a_fi_pbb_detail_delivery',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_2_a_fi_pbb_detail_deliverylist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_2_a_fi_pbb_detail_delivery->Export <> "" ||
			$frm_2_a_fi_pbb_detail_delivery->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_2_a_fi_pbb_detail_delivery->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_2_a_fi_pbb_detail_delivery->ExportAll) {
		...
		if ($frm_2_a_fi_pbb_detail_delivery->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_2_a_fi_pbb_detail_delivery, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_2_a_fi_pbb_detail_delivery->getMasterFilter() <> "" && $frm_2_a_fi_pbb_detail_delivery->getCurrentMasterTable() == "frm_2_a_fi_rep_form_a_raw_data_cu") {
			global $frm_2_a_fi_rep_form_a_raw_data_cu;
			$rsmaster = $frm_2_a_fi_rep_form_a_raw_data_cu->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_2_a_fi_pbb_detail_delivery->Export == "xml") {
					$ParentTable = "frm_2_a_fi_rep_form_a_raw_data_cu";
					$frm_2_a_fi_rep_form_a_raw_data_cu->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_2_a_fi_pbb_detail_delivery->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_2_a_fi_rep_form_a_raw_data_cu->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		if ($frm_2_a_fi_pbb_detail_delivery->Export == "xml") {
			$frm_2_a_fi_pbb_detail_delivery->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_2_a_fi_pbb_detail_delivery->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_2_a_fi_pbb_detail_delivery->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_2_a_fi_pbb_detail_delivery->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_2_a_fi_pbb_detail_delivery->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_2_a_fi_pbb_detail_delivery->ExportReturnUrl());
		} elseif ($frm_2_a_fi_pbb_detail_delivery->Export == "pdf") {
