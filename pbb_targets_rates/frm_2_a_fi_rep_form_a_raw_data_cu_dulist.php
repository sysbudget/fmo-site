...
class cfrm_2_a_fi_rep_form_a_raw_data_cu_du_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_2_a_fi_rep_form_a_raw_data_cu_du->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_2_a_fi_rep_form_a_raw_data_cu_du->Export = $_POST["exporttype"];
		} else {
			$frm_2_a_fi_rep_form_a_raw_data_cu_du->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_2_a_fi_rep_form_a_raw_data_cu_du->Export; // Get export parameter, used in header
		$gsExportFile = $frm_2_a_fi_rep_form_a_raw_data_cu_du->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export <> "email")
	...
	function SetupExportOptions() {
		global $Language, $frm_2_a_fi_rep_form_a_raw_data_cu_du;
		...
		$item->Body = "<a name=\"emf_frm_2_a_fi_rep_form_a_raw_data_cu_du\" id=\"emf_frm_2_a_fi_rep_form_a_raw_data_cu_du\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_2_a_fi_rep_form_a_raw_data_cu_du',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_2_a_fi_rep_form_a_raw_data_cu_dulist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export <> "" ||
			$frm_2_a_fi_rep_form_a_raw_data_cu_du->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_2_a_fi_rep_form_a_raw_data_cu_du->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->ExportAll) {
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_2_a_fi_rep_form_a_raw_data_cu_du, "h");
		}
		$ParentTable = "";
		if ($bSelectLimit) {
			$StartRec = 1;
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "xml") {
			$frm_2_a_fi_rep_form_a_raw_data_cu_du->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_2_a_fi_rep_form_a_raw_data_cu_du->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_2_a_fi_rep_form_a_raw_data_cu_du->ExportReturnUrl());
		} elseif ($frm_2_a_fi_rep_form_a_raw_data_cu_du->Export == "pdf") {
