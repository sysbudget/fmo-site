...
//
// Table class for tbl_temp_du
//
class ctbl_temp_du {
	var $TableVar = 'tbl_temp_du';
	...
	//
	function ctbl_temp_du() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// temp_id
		$this->temp_id = new cField('tbl_temp_du', 'tbl_temp_du', 'x_temp_id', 'temp_id', '`temp_id`', 3, -1, FALSE, '`temp_id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->temp_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['temp_id'] =& $this->temp_id;

		// temp_name
		$this->temp_name = new cField('tbl_temp_du', 'tbl_temp_du', 'x_temp_name', 'temp_name', '`temp_name`', 200, -1, FALSE, '`temp_name`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `tbl_temp_du` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `tbl_temp_du` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `tbl_temp_du` WHERE ";
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tbl_temp_duview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tbl_temp_duadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_duedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("tbl_temp_duadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("tbl_temp_dudelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tbl_temp_du" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
