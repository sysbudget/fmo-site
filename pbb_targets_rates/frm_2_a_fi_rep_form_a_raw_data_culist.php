...
class cfrm_2_a_fi_rep_form_a_raw_data_cu_list {
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_2_a_fi_rep_form_a_raw_data_cu->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_2_a_fi_rep_form_a_raw_data_cu->Export = $_POST["exporttype"];
		} else {
			$frm_2_a_fi_rep_form_a_raw_data_cu->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_2_a_fi_rep_form_a_raw_data_cu->Export; // Get export parameter, used in header
		$gsExportFile = $frm_2_a_fi_rep_form_a_raw_data_cu->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "csv") {
	...
	function Page_Main() {
		...
		// Export data only
		if (in_array($frm_2_a_fi_rep_form_a_raw_data_cu->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export <> "email")
	...
	function SetupExportOptions() {
		global $Language, $frm_2_a_fi_rep_form_a_raw_data_cu;
		...
		$item->Body = "<a name=\"emf_frm_2_a_fi_rep_form_a_raw_data_cu\" id=\"emf_frm_2_a_fi_rep_form_a_raw_data_cu\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_2_a_fi_rep_form_a_raw_data_cu',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_2_a_fi_rep_form_a_raw_data_culist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export <> "" ||
			$frm_2_a_fi_rep_form_a_raw_data_cu->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_2_a_fi_rep_form_a_raw_data_cu->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->ExportAll) {
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_2_a_fi_rep_form_a_raw_data_cu, "h");
		}
		$ParentTable = "";
		if ($bSelectLimit) {
			$StartRec = 1;
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "xml") {
			$frm_2_a_fi_rep_form_a_raw_data_cu->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_2_a_fi_rep_form_a_raw_data_cu->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_2_a_fi_rep_form_a_raw_data_cu->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_2_a_fi_rep_form_a_raw_data_cu->ExportReturnUrl());
		} elseif ($frm_2_a_fi_rep_form_a_raw_data_cu->Export == "pdf") {
