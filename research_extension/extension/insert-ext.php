...
if ( $npriv_fund < 0 ) {
  print "<p>Error! National Private Fund is too low or is not a number: $npriv_fund.</p>";
  exit();
}
...
if ( $ipriv_fund < 0 ) {
  print "<p>Error! International Private Fund is too low or is not a number: $ipriv_fund.</p>";
  exit();
}
...
$query = "INSERT INTO tbl_extension_main_raw(created, user_name, year_cover, cu, unit, extension_title, prep_edate, sign_edate, imple_sdate, imple_edate, close_edate,num_session, hrs_session, objective, out_imp, interest_ext, prog_class, activity_type, client_type, client_count, sponsor, curr_gen_fund, gen_fund, curr_revolv_fund, revolv_fund, curr_oth_fund, other_fund, curr_npriv_fund, npriv_fund, curr_ipriv_fund, ipriv_fund, faculty, reps, other_staff, adj_ladder) VALUES (NOW(), '$user_name', '$year_cover', '$cu', '$unit', '$extension_title',  STR_TO_DATE('$_POST[prep_edate]','%m/%d/%Y'), STR_TO_DATE('$_POST[sign_edate]','%m/%d/%Y'), STR_TO_DATE('$_POST[imple_sdate]','%m/%d/%Y'), STR_TO_DATE('$_POST[imple_edate]','%m/%d/%Y'), STR_TO_DATE('$_POST[close_edate]','%m/%d/%Y'),'$num_session','$hrs_session', '$objective', '$out_imp', '$interest_ext', '$prog_class', '$activity_type', '$client_type', '$client_count', '$sponsor', '$curr_gen_fund', $gen_fund, '$curr_revolv_fund', $revolv_fund, '$curr_oth_fund', $other_fund, '$curr_npriv_fund', $npriv_fund, '$curr_ipriv_fund', $ipriv_fund, '$faculty', '$reps', '$other_staff', '$adj_ladder')";
...
echo "1 record added";
header("Location: ../extension/view-input-ext.php");
