...
//
// Table class for ref_tenurecode
//
class cref_tenurecode {
	var $TableVar = 'ref_tenurecode';
	var $TableName = 'ref_tenurecode';
	var $TableType = 'TABLE';
	var $tenureCode_ID;
	var $tenureCode_description;
	var $fields = array();
	...
	//
	function cref_tenurecode() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// tenureCode_ID
		$this->tenureCode_ID = new cField('ref_tenurecode', 'ref_tenurecode', 'x_tenureCode_ID', 'tenureCode_ID', '`tenureCode_ID`', 3, -1, FALSE, '`tenureCode_ID`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->tenureCode_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['tenureCode_ID'] =& $this->tenureCode_ID;

		// tenureCode_description
		$this->tenureCode_description = new cField('ref_tenurecode', 'ref_tenurecode', 'x_tenureCode_description', 'tenureCode_description', '`tenureCode_description`', 200, -1, FALSE, '`tenureCode_description`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['tenureCode_description'] =& $this->tenureCode_description;
	}
	...
	function HighlightName() {
		return "ref_tenurecode_Highlight";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `ref_tenurecode` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `ref_tenurecode` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `ref_tenurecode` WHERE ";
		$SQL .= up_QuotedName('tenureCode_ID') . '=' . up_QuotedValue($rs['tenureCode_ID'], $this->tenureCode_ID->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`tenureCode_ID` = @tenureCode_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->tenureCode_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@tenureCode_ID@", up_AdjustSql($this->tenureCode_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("ref_tenurecodeview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "ref_tenurecodeadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("ref_tenurecodeedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("ref_tenurecodeadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("ref_tenurecodedelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->tenureCode_ID->CurrentValue)) {
			$sUrl .= "tenureCode_ID=" . urlencode($this->tenureCode_ID->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=ref_tenurecode" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["tenureCode_ID"]; // tenureCode_ID
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->tenureCode_ID->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->tenureCode_ID->setDbValue($rs->fields('tenureCode_ID'));
		$this->tenureCode_description->setDbValue($rs->fields('tenureCode_description'));
	}
   ...
   // Common render codes
		...
		$this->tenureCode_ID->ViewValue = $this->tenureCode_ID->CurrentValue;
		$this->tenureCode_ID->ViewCustomAttributes = "";

		// tenureCode_description
		$this->tenureCode_description->ViewValue = $this->tenureCode_description->CurrentValue;
		$this->tenureCode_description->ViewCustomAttributes = "";

		// tenureCode_ID
		$this->tenureCode_ID->LinkCustomAttributes = "";
		$this->tenureCode_ID->HrefValue = "";
		$this->tenureCode_ID->TooltipValue = "";

		// tenureCode_description
		$this->tenureCode_description->LinkCustomAttributes = "";
		$this->tenureCode_description->HrefValue = "";
		$this->tenureCode_description->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					$XmlDoc->AddField('tenureCode_ID', $this->tenureCode_ID->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('tenureCode_description', $this->tenureCode_description->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('tenureCode_ID', $this->tenureCode_ID->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('tenureCode_description', $this->tenureCode_description->ExportValue($this->Export, $this->ExportOriginalValue));
				}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			$Doc->BeginExportRow();
			if ($ExportPageType == "view") {
				$Doc->ExportCaption($this->tenureCode_ID);
				$Doc->ExportCaption($this->tenureCode_description);
			} else {
				$Doc->ExportCaption($this->tenureCode_ID);
				$Doc->ExportCaption($this->tenureCode_description);
			}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					$Doc->ExportField($this->tenureCode_ID);
					$Doc->ExportField($this->tenureCode_description);
				} else {
					$Doc->ExportField($this->tenureCode_ID);
					$Doc->ExportField($this->tenureCode_description);
				}
