...
$tbl_uporgchart_location = NULL;

//
// Table class for tbl_uporgchart_location
//
class ctbl_uporgchart_location {
	var $TableVar = 'tbl_uporgchart_location';
	...
	//
	function ctbl_uporgchart_location() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// id
		$this->id = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_id', 'id', '`id`', 200, -1, FALSE, '`id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['id'] =& $this->id;

		// name
		$this->name = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_name', 'name', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['name'] =& $this->name;

		// address
		$this->address = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_address', 'address', '`address`', 200, -1, FALSE, '`address`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['address'] =& $this->address;

		// region_id
		$this->region_id = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_region_id', 'region_id', '`region_id`', 200, -1, FALSE, '`region_id`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['region_id'] =& $this->region_id;

		// zip_code
		$this->zip_code = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_zip_code', 'zip_code', '`zip_code`', 200, -1, FALSE, '`zip_code`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['zip_code'] =& $this->zip_code;

		// inst_code
		$this->inst_code = new cField('tbl_uporgchart_location', 'tbl_uporgchart_location', 'x_inst_code', 'inst_code', '`inst_code`', 200, -1, FALSE, '`inst_code`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['inst_code'] =& $this->inst_code;
	}
	...
	function HighlightName() {
		return "tbl_uporgchart_location_Highlight";
	}
	...
	function SqlFrom() { // From
		return "`tbl_uporgchart_location`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `tbl_uporgchart_location` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `tbl_uporgchart_location` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `tbl_uporgchart_location` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "tbl_uporgchart_locationlist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("tbl_uporgchart_locationview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "tbl_uporgchart_locationadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("tbl_uporgchart_locationedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("tbl_uporgchart_locationadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("tbl_uporgchart_locationdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=tbl_uporgchart_location" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->region_id->setDbValue($rs->fields('region_id'));
		$this->zip_code->setDbValue($rs->fields('zip_code'));
		$this->inst_code->setDbValue($rs->fields('inst_code'));
	}
   ...
   // Common render codes
		...
		// region_id
		$this->region_id->CellCssStyle = "white-space: nowrap;";

		// zip_code
		$this->zip_code->CellCssStyle = "white-space: nowrap;";

		// inst_code
		$this->inst_code->CellCssStyle = "white-space: nowrap;";
		...
		// region_id
		$this->region_id->ViewValue = $this->region_id->CurrentValue;
		$this->region_id->ViewCustomAttributes = "";
		...
		// inst_code
		$this->inst_code->ViewValue = $this->inst_code->CurrentValue;
		$this->inst_code->ViewCustomAttributes = "";
		...
		// region_id
		$this->region_id->LinkCustomAttributes = "";
		$this->region_id->HrefValue = "";
		$this->region_id->TooltipValue = "";
		...
		// inst_code
		$this->inst_code->LinkCustomAttributes = "";
		$this->inst_code->HrefValue = "";
		$this->inst_code->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('region_id', $this->region_id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('zip_code', $this->zip_code->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('inst_code', $this->inst_code->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
					$XmlDoc->AddField('id', $this->id->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('name', $this->name->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('address', $this->address->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->region_id);
				$Doc->ExportCaption($this->zip_code);
				$Doc->ExportCaption($this->inst_code);
			} else {
				$Doc->ExportCaption($this->id);
				$Doc->ExportCaption($this->name);
				$Doc->ExportCaption($this->address);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->region_id);
					$Doc->ExportField($this->zip_code);
					$Doc->ExportField($this->inst_code);
				} else {
					$Doc->ExportField($this->id);
					$Doc->ExportField($this->name);
					$Doc->ExportField($this->address);
