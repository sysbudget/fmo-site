...
$ref_degreelevel_faculty = NULL;

//
// Table class for ref_degreelevel_faculty
//
class cref_degreelevel_faculty {
	var $TableVar = 'ref_degreelevel_faculty';
	var $TableName = 'ref_degreelevel_faculty';
	var $TableType = 'TABLE';
	var $degreelevelFaculty_ID;
	var $degreelevelFaculty_name;
	var $fields = array();
	...
	//
	function cref_degreelevel_faculty() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// degreelevelFaculty_ID
		$this->degreelevelFaculty_ID = new cField('ref_degreelevel_faculty', 'ref_degreelevel_faculty', 'x_degreelevelFaculty_ID', 'degreelevelFaculty_ID', '`degreelevelFaculty_ID`', 3, -1, FALSE, '`degreelevelFaculty_ID`', FALSE, FALSE, 'FORMATTED TEXT');
		...
		$this->degreelevelFaculty_name = new cField('ref_degreelevel_faculty', 'ref_degreelevel_faculty', 'x_degreelevelFaculty_name', 'degreelevelFaculty_name', '`degreelevelFaculty_name`', 200, -1, FALSE, '`degreelevelFaculty_name`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['degreelevelFaculty_name'] =& $this->degreelevelFaculty_name;
	}
	...
	function HighlightName() {
		return "ref_degreelevel_faculty_Highlight";
	}
	...
	function SqlFrom() { // From
		return "`ref_degreelevel_faculty`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `ref_degreelevel_faculty` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `ref_degreelevel_faculty` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `ref_degreelevel_faculty` WHERE ";
		$SQL .= up_QuotedName('degreelevelFaculty_ID') . '=' . up_QuotedValue($rs['degreelevelFaculty_ID'], $this->degreelevelFaculty_ID->FldDataType) . ' AND ';
	...
	function SqlKeyFilter() {
		return "`degreelevelFaculty_ID` = @degreelevelFaculty_ID@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->degreelevelFaculty_ID->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@degreelevelFaculty_ID@", up_AdjustSql($this->degreelevelFaculty_ID->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "ref_degreelevel_facultylist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("ref_degreelevel_facultyview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "ref_degreelevel_facultyadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("ref_degreelevel_facultyedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("ref_degreelevel_facultyadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("ref_degreelevel_facultydelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->degreelevelFaculty_ID->CurrentValue)) {
			$sUrl .= "degreelevelFaculty_ID=" . urlencode($this->degreelevelFaculty_ID->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=ref_degreelevel_faculty" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["degreelevelFaculty_ID"]; // degreelevelFaculty_ID
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->degreelevelFaculty_ID->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->degreelevelFaculty_name->setDbValue($rs->fields('degreelevelFaculty_name'));
	}
   ...
   // Common render codes
		...
		// degreelevelFaculty_name
		$this->degreelevelFaculty_name->ViewValue = $this->degreelevelFaculty_name->CurrentValue;
		$this->degreelevelFaculty_name->ViewCustomAttributes = "";
		...
		// degreelevelFaculty_name
		$this->degreelevelFaculty_name->LinkCustomAttributes = "";
		$this->degreelevelFaculty_name->HrefValue = "";
		$this->degreelevelFaculty_name->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('degreelevelFaculty_name', $this->degreelevelFaculty_name->ExportValue($this->Export, $this->ExportOriginalValue));
				} else {
				}
			}
			$Recordset->MoveNext();
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->degreelevelFaculty_name);
			} else {
			}
			if ($this->Export == "pdf") {
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->degreelevelFaculty_name);
				} else {
				}
				$Doc->EndExportRow();
			}
