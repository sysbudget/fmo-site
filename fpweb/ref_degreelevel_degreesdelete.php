...
// Create page object
$ref_degreelevel_degrees_delete = new cref_degreelevel_degrees_delete();
$Page =& $ref_degreelevel_degrees_delete;

// Page init
$ref_degreelevel_degrees_delete->Page_Init();

// Page main
$ref_degreelevel_degrees_delete->Page_Main();
?>
...
// Create page object
var ref_degreelevel_degrees_delete = new up_Page("ref_degreelevel_degrees_delete");

// page properties
ref_degreelevel_degrees_delete.PageID = "delete"; // page ID
ref_degreelevel_degrees_delete.FormID = "fref_degreelevel_degreesdelete"; // form ID
var UP_PAGE_ID = ref_degreelevel_degrees_delete.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
ref_degreelevel_degrees_delete.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
ref_degreelevel_degrees_delete.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
ref_degreelevel_degrees_delete.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
ref_degreelevel_degrees_delete.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
if ($ref_degreelevel_degrees_delete->Recordset = $ref_degreelevel_degrees_delete->LoadRecordset())
	$ref_degreelevel_degrees_deleteTotalRecs = $ref_degreelevel_degrees_delete->Recordset->RecordCount(); // Get record count
if ($ref_degreelevel_degrees_deleteTotalRecs <= 0) { // No record found, exit
	if ($ref_degreelevel_degrees_delete->Recordset)
		$ref_degreelevel_degrees_delete->Recordset->Close();
	$ref_degreelevel_degrees_delete->Page_Terminate("ref_degreelevel_degreeslist.php"); // Return to list
}
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ref_degreelevel_degrees->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $ref_degreelevel_degrees->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $ref_degreelevel_degrees_delete->ShowPageHeader(); ?>
<?php
$ref_degreelevel_degrees_delete->ShowMessage();
?>
<form action="<?php echo up_CurrentPage() ?>" method="post">
<p>
<input type="hidden" name="t" id="t" value="ref_degreelevel_degrees">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($ref_degreelevel_degrees_delete->RecKeys as $key) { ?>
...
<?php echo $ref_degreelevel_degrees->TableCustomInnerHtml ?>
	...
	<tr class="upTableHeader">
		<td valign="top"><?php echo $ref_degreelevel_degrees->degreeLevel_ID->FldCaption() ?></td>
		<td valign="top"><?php echo $ref_degreelevel_degrees->degreeLevel_name->FldCaption() ?></td>
		<td valign="top"><?php echo $ref_degreelevel_degrees->degreeLevel_educationLevel->FldCaption() ?></td>
	</tr>
	</thead>
	<tbody>
<?php
$ref_degreelevel_degrees_delete->RecCnt = 0;
$i = 0;
while (!$ref_degreelevel_degrees_delete->Recordset->EOF) {
	$ref_degreelevel_degrees_delete->RecCnt++;

	// Set row properties
	$ref_degreelevel_degrees->ResetAttrs();
	$ref_degreelevel_degrees->RowType = UP_ROWTYPE_VIEW; // View

	// Get the field contents
	$ref_degreelevel_degrees_delete->LoadRowValues($ref_degreelevel_degrees_delete->Recordset);

	// Render row
	$ref_degreelevel_degrees_delete->RenderRow();
?>
	<tr<?php echo $ref_degreelevel_degrees->RowAttributes() ?>>
		<td<?php echo $ref_degreelevel_degrees->degreeLevel_ID->CellAttributes() ?>>
<div<?php echo $ref_degreelevel_degrees->degreeLevel_ID->ViewAttributes() ?>><?php echo $ref_degreelevel_degrees->degreeLevel_ID->ListViewValue() ?></div></td>
		<td<?php echo $ref_degreelevel_degrees->degreeLevel_name->CellAttributes() ?>>
<div<?php echo $ref_degreelevel_degrees->degreeLevel_name->ViewAttributes() ?>><?php echo $ref_degreelevel_degrees->degreeLevel_name->ListViewValue() ?></div></td>
		<td<?php echo $ref_degreelevel_degrees->degreeLevel_educationLevel->CellAttributes() ?>>
<div<?php echo $ref_degreelevel_degrees->degreeLevel_educationLevel->ViewAttributes() ?>><?php echo $ref_degreelevel_degrees->degreeLevel_educationLevel->ListViewValue() ?></div></td>
	</tr>
<?php
	$ref_degreelevel_degrees_delete->Recordset->MoveNext();
}
$ref_degreelevel_degrees_delete->Recordset->Close();
?>
</tbody>
...
<?php
$ref_degreelevel_degrees_delete->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$ref_degreelevel_degrees_delete->Page_Terminate();
?>
<?php
...
// Page class
//
class cref_degreelevel_degrees_delete {
	...
	// Page object name
	var $PageObjName = 'ref_degreelevel_degrees_delete';
	...
	//
	function cref_degreelevel_degrees_delete() {
	...
	//
	function Page_Main() {
		global $Language, $ref_degreelevel_degrees;

		// Load key parameters
		$this->RecKeys = $ref_degreelevel_degrees->GetRecordKeys(); // Load record keys
		$sFilter = $ref_degreelevel_degrees->GetKeyFilter();
		if ($sFilter == "")
			$this->Page_Terminate("ref_degreelevel_degreeslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in ref_degreelevel_degrees class, ref_degreelevel_degreesinfo.php

		$ref_degreelevel_degrees->CurrentFilter = $sFilter;

		// Get action
		if (@$_POST["a_delete"] <> "") {
			$ref_degreelevel_degrees->CurrentAction = $_POST["a_delete"];
		} else {
			$ref_degreelevel_degrees->CurrentAction = "I"; // Display record
		}
		switch ($ref_degreelevel_degrees->CurrentAction) {
			case "D": // Delete
				$ref_degreelevel_degrees->SendEmail = TRUE; // Send email on delete success
				if ($this->DeleteRows()) { // delete rows
					$this->setSuccessMessage($Language->Phrase("DeleteSuccess")); // Set up success message
					$this->Page_Terminate($ref_degreelevel_degrees->getReturnUrl()); // Return to caller
				}
		}
	}
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$DeleteRows = $ref_degreelevel_degrees->Row_Deleting($row);
				if (!$DeleteRows) break;
			}
		}
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['degreeLevel_ID'];
				$conn->raiseErrorFn = 'up_ErrorFn';
				$DeleteRows = $conn->Execute($ref_degreelevel_degrees->DeleteSQL($row)); // Delete
				$conn->raiseErrorFn = '';
				if ($DeleteRows === FALSE)
					break;
				if ($sKey <> "") $sKey .= ", ";
		...
		if ($DeleteRows) {
			foreach ($rsold as $row) {
				$ref_degreelevel_degrees->Row_Deleted($row);
			}
		}
		return $DeleteRows;
	}
	...
	function WriteAuditTrailOnDelete(&$rs) {
	  ...
	  $curUser = CurrentUserID();
		foreach (array_keys($rs) as $fldname) {
			if (array_key_exists($fldname, $ref_degreelevel_degrees->fields) && $ref_degreelevel_degrees->fields[$fldname]->FldDataType <> UP_DATATYPE_BLOB) { // Ignore BLOB fields
