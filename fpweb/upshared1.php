...
//
// Shared code for UPFacultyProfile and UP Faculty Profile Report
//
...
function up_ConvertToUtf8($str) {
	return up_Convert(UP_ENCODING, "UTF-8", $str);
}

function up_ConvertFromUtf8($str) {
	return up_Convert("UTF-8", UP_ENCODING, $str);
}

function up_Convert($from, $to, $str) {
...
class cLanguage {
	var $LanguageId;
	var $Phrases = NULL;
	var $LanguageFolder = UP_LANGUAGE_FOLDER;
	...
	function cLanguage($langfolder = "", $langid = "") {
		...
		if ($langid <> "") { // Set up language id
			...
			$_SESSION[UP_SESSION_LANGUAGE_ID] = $this->LanguageId;
		} elseif (@$_GET["language"] <> "") {
			$this->LanguageId = $_GET["language"];
			$_SESSION[UP_SESSION_LANGUAGE_ID] = $this->LanguageId;
		} elseif (@$_SESSION[UP_SESSION_LANGUAGE_ID] <> "") {
			$this->LanguageId = $_SESSION[UP_SESSION_LANGUAGE_ID];
		} else {
			$this->LanguageId = UP_LANGUAGE_DEFAULT_ID;
		}
	...
	function LoadFileList() {
		global $UP_LANGUAGE_FILE;
		if (is_array($UP_LANGUAGE_FILE)) {
			$cnt = count($UP_LANGUAGE_FILE);
			for ($i = 0; $i < $cnt; $i++)
				$UP_LANGUAGE_FILE[$i][1] = $this->LoadFileDesc($this->LanguageFolder . $UP_LANGUAGE_FILE[$i][2]);
		}
	}
	...
	function LoadFileDesc($File) {
		if (UP_USE_DOM_XML) {
			...
			$ar =	up_Xml2Array(substr(file_get_contents($File), 0, 512)); // Just read the first part
			return (is_array($ar)) ? @$ar['up-language']['attr']['desc'] : "";
		}
	}
	...
	function Load($id) {
		...
		if ($sFileName == "")
			$sFileName = $this->GetFileName(UP_LANGUAGE_DEFAULT_ID);
		if ($sFileName == "")
			return;
		if (UP_USE_DOM_XML) {
			$this->Phrases = new cXMLDocument();
			...
			if (is_array(@$_SESSION[UP_PROJECT_NAME . "_" . $sFileName])) {
				$this->Phrases = $_SESSION[UP_PROJECT_NAME . "_" . $sFileName];
			} else {
				$this->Phrases = up_Xml2Array(file_get_contents($sFileName));
			}
		}
	}
	...
	function GetFileName($Id) {
		...
		if (is_array($UP_LANGUAGE_FILE)) {
			$cnt = count($UP_LANGUAGE_FILE);
			for ($i = 0; $i < $cnt; $i++)
				if ($UP_LANGUAGE_FILE[$i][0] == $Id) {
					return $this->LanguageFolder . $UP_LANGUAGE_FILE[$i][2];
				}
		}
		return "";
	}
	...
	function GetNodeAtt($Nodes, $Att) {
		...
		//return up_ConvertFromUtf8($value);
		return $value;
	}
	...
	function Phrase($Id) {
		if (is_object($this->Phrases)) {
			...
			return up_ConvertFromUtf8(@$this->Phrases['up-language']['global']['phrase'][strtolower($Id)]['attr']['value']);
		}
	}
	...
	function setPhrase($Id, $Value) {
		if (is_array($this->Phrases)) {
			$this->Phrases['up-language']['global']['phrase'][strtolower($Id)]['attr']['value'] = $Value;
		}
	}
	...
	function ProjectPhrase($Id) {
		if (is_object($this->Phrases)) {
			...
			return up_ConvertFromUtf8(@$this->Phrases['up-language']['project']['phrase'][strtolower($Id)]['attr']['value']);
		}
	}
	...
	function setProjectPhrase($Id, $Value) {
		if (is_array($this->Phrases)) {
			$this->Phrases['up-language']['project']['phrase'][strtolower($Id)]['attr']['value'] = $Value;
		}
	}
	...
	function MenuPhrase($MenuId, $Id) {
		if (is_object($this->Phrases)) {
			...
			return up_ConvertFromUtf8(@$this->Phrases['up-language']['project']['menu'][$MenuId]['phrase'][strtolower($Id)]['attr']['value']);
		}
	}
	...
	function setMenuPhrase($MenuId, $Id, $Value) {
		if (is_array($this->Phrases)) {
			$this->Phrases['up-language']['project']['menu'][$MenuId]['phrase'][strtolower($Id)]['attr']['value'] = $Value;
		}
	}
	...
	function TablePhrase($TblVar, $Id) {
		if (is_object($this->Phrases)) {
			...
			return up_ConvertFromUtf8(@$this->Phrases['up-language']['project']['table'][strtolower($TblVar)]['phrase'][strtolower($Id)]['attr']['value']);
		}
	}
	...
	function setTablePhrase($TblVar, $Id, $Value) {
		if (is_array($this->Phrases)) {
			$this->Phrases['up-language']['project']['table'][strtolower($TblVar)]['phrase'][strtolower($Id)]['attr']['value'] = $Value;
		}
	}
	...
	function FieldPhrase($TblVar, $FldVar, $Id) {
		if (is_object($this->Phrases)) {
			...
			return up_ConvertFromUtf8(@$this->Phrases['up-language']['project']['table'][strtolower($TblVar)]['field'][strtolower($FldVar)]['phrase'][strtolower($Id)]['attr']['value']);
		}
	}
	...
	function setFieldPhrase($TblVar, $FldVar, $Id, $Value) {
		if (is_array($this->Phrases)) {
			$this->Phrases['up-language']['project']['table'][strtolower($TblVar)]['field'][strtolower($FldVar)]['phrase'][strtolower($Id)]['attr']['value'] = $Value;
		}
	}
	...
	function XmlToJSON($XPath) {
		...
		foreach ($NodeList as $Node) {
			...
			$Str .= "\"" . up_JsEncode2($Id) . "\":\"" . up_JsEncode2($Value) . "\",";
		}
	...
	function ArrayToJSON($client) {
		$ar = @$this->Phrases['up-language']['global']['phrase'];
		$Str = "{";
		if (is_array($ar)) {
			foreach ($ar as $id => $node) {
				$is_client = @$node['attr']['client'] == '1';
				$value = up_ConvertFromUtf8(@$node['attr']['value']);
	...
	function AllToJSON() {
		if (is_object($this->Phrases)) {
			return "var upLanguage = new up_Language(" . $this->XmlToJSON("//global/phrase") . ");";
		} elseif (is_array($this->Phrases)) {
			return "var upLanguage = new up_Language(" . $this->ArrayToJSON(FALSE) . ");";
		}
	}
	...
	function ToJSON() {
		if (is_object($this->Phrases)) {
			return "var upLanguage = new up_Language(" . $this->XmlToJSON("//global/phrase[@client='1']") . ");";
		} elseif (is_array($this->Phrases)) {
			return "var upLanguage = new up_Language(" . $this->ArrayToJSON(TRUE) . ");";
		}
	}
}
...
class cXMLDocument {
	...
	function GetAttribute($element, $name) {
		return ($element) ? up_ConvertFromUtf8($element->getAttribute($name)) : "";
	}
	...
	function AddRoot($roottagname = 'table') {
		$this->RootTagName = up_XmlTagName($roottagname);
	...
	function AddRow($tabletagname = '', $rowtagname = 'row') {
		$this->RowTagName = up_XmlTagName($rowtagname);
		...
		if ($tabletagname == '') {
			...
			if ($this->SubTblName == '') {
				$this->SubTblName = up_XmlTagName($tabletagname);
	...
	function AddField($name, $value) {
		...
		$value = up_ConvertToUtf8($value); // Convert to UTF-8
		$xmlfld = $this->XmlDoc->createElement(up_XmlTagName($name));
...
class cMenu {
	...
	function Render($ret = FALSE) {
		...
		$str .= " class=\"" . (($this->IsRoot) ? UP_MENUBAR_CLASSNAME : UP_MENU_CLASSNAME) . "\">";
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				$aclass = ($this->IsRoot) ? UP_MENUBAR_ITEM_LABEL_CLASSNAME : UP_MENU_ITEM_LABEL_CLASSNAME;
				$liclass = ($this->IsRoot) ? UP_MENUBAR_ITEM_CLASSNAME : UP_MENU_ITEM_CLASSNAME;
				if ($item->GroupTitle && UP_MENU_ITEM_CLASSNAME <> "") { // Group title
					$gcnt++;
					if ($i > 1 && $gopen) {
						$str .= "</ul>\n"; // End last group
						$gopen = FALSE;
...
class cMenuItem {
	...
	function Render($aclass = "", $liclass = "") {
		...
		$innerhtml = up_HtmlElement("a", $attrs, $this->Text);
		if (!is_null($this->SubMenu))
			$innerhtml .= $this->SubMenu->Render(TRUE);

		// Create <LI>
		return up_HtmlElement("li", array("class" => $liclass), $innerhtml);
	}
}

// Output SCRIPT tag
function up_AddClientScript($src, $attrs = NULL) {
	$atts = array("type"=>"text/javascript", "src"=>$src);
	if (is_array($attrs))
		$atts = array_merge($atts, $attrs);
	echo up_HtmlElement("script", $atts, "") . "\n";
}

// Output LINK tag
function up_AddStylesheet($href, $attrs = NULL) {
	$atts = array("rel"=>"stylesheet", "type"=>"text/css", "href"=>$href);
	if (is_array($attrs))
		$atts = array_merge($atts, $attrs);
	echo up_HtmlElement("link", $atts, "", FALSE) . "\n";
}

// Build HTML element
function up_HtmlElement($tagname, $attrs, $innerhtml = "", $endtag = TRUE) {
	$html = "<" . $tagname;
	if (is_array($attrs)) {
		foreach ($attrs as $name => $attr) {
			if (strval($attr) <> "")
				$html .= " " . $name . "=\"" . up_HtmlEncode($attr) . "\"";
		}
	}
	$html .= ">";
	if (strval($innerhtml) <> "")
...
// Encode html
function up_HtmlEncode($exp) {
	return htmlspecialchars(strval($exp));
}

// XML tag name
function up_XmlTagName($name) {
...
function up_Xml2Array($contents) {
	...
	foreach ($xml_values as $data) {
		...
		if ($type == "open") { // The starting of the tag '<tag>'
			...
			if (!is_array($current) || !in_array($tag, array_keys($current))) { // Insert New tag
				if ($tag <> 'up-language' && @$result['attr']['id'] <> '') { // 
...
function up_ArrayToJsonAttr($ar) {
	$Str = "{";
	foreach ($ar as $key => $val)
		$Str .= $key . ":'" . up_JsEncode3($val) . "',";
	if (substr($Str, -1) == ",") $Str = substr($Str, 0, strlen($Str)-1);
	$Str .= "}";
	return $Str;
}
...
function up_CurrentPage() {
	return up_GetPageName(up_ScriptName());
}

// Get page name
function up_GetPageName($url) {
	$PageName = "";
	if ($url <> "") {
		$PageName = $url;
		$p = strpos($PageName, "?");
...
function up_BtnCaption($Caption) {
	$Min = 10;
...
function CurrentUserLevels() {
	...
	if (isset($Security)) {
		...
		if (isset($_SESSION[UP_SESSION_USER_LEVEL_ID])) {
			return array($_SESSION[UP_SESSION_USER_LEVEL_ID]);
		} else {
			return array();
		}
	}
...
function AllowListMenu($TableName) {
	...
	if (IsLoggedIn()) {
		if (in_array("-1", $userlevels)) {
			...
			$priv = 0;
			if (is_array(@$_SESSION[UP_SESSION_AR_USER_LEVEL_PRIV])) {
				foreach ($_SESSION[UP_SESSION_AR_USER_LEVEL_PRIV] as $row) {
...
// Check if HTTPS
function up_IsHttps() {
	return (up_ServerVar("HTTPS") <> "" && up_ServerVar("HTTPS") <> "off");
}

// Get domain URL
function up_DomainUrl() {
	$sUrl = "http";
	$bSSL = up_IsHttps();
	$sPort = strval(up_ServerVar("SERVER_PORT"));
	$defPort = ($bSSL) ? "443" : "80";
	$sPort = ($sPort == $defPort) ? "" : ":$sPort";
	$sUrl .= ($bSSL) ? "s" : "";
	$sUrl .= "://";
	$sUrl .= up_ServerVar("SERVER_NAME") . $sPort;
	return $sUrl;
}

// Get full URL
function up_FullUrl() {
	return up_DomainUrl() . up_ScriptName();
}

// Get current URL
function up_CurrentUrl() {
	$s = up_ScriptName();
...
// Convert to full URL
function up_ConvertFullUrl($url) {
	if ($url == "") return "";
	$sUrl = up_FullUrl();
