...
$view_tbl_uporgchart_cu_users = NULL;

//
// Table class for view_tbl_uporgchart_cu_users
//
class cview_tbl_uporgchart_cu_users {
	var $TableVar = 'view_tbl_uporgchart_cu_users';
	...
	//
	function cview_tbl_uporgchart_cu_users() {
		global $Language;
		$this->AllowAddDeleteRow = up_AllowAddDeleteRow(); // Allow add/delete row

		// ID
		$this->ID = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_ID', 'ID', '`ID`', 3, -1, FALSE, '`ID`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ID'] =& $this->ID;

		// code
		$this->code = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_code', 'code', '`code`', 200, -1, FALSE, '`code`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['code'] =& $this->code;

		// shortName
		$this->shortName = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_shortName', 'shortName', '`shortName`', 200, -1, FALSE, '`shortName`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['shortName'] =& $this->shortName;

		// name
		$this->name = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_name', 'name', '`name`', 200, -1, FALSE, '`name`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['name'] =& $this->name;

		// acronym
		$this->acronym = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_acronym', 'acronym', '`acronym`', 200, -1, FALSE, '`acronym`', FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['acronym'] =& $this->acronym;

		// address
		$this->address = new cField('view_tbl_uporgchart_cu_users', 'view_tbl_uporgchart_cu_users', 'x_address', 'address', '`address`', 200, -1, FALSE, '`address`', FALSE, FALSE, 'FORMATTED TEXT');
	...
	function HighlightName() {
		return "view_tbl_uporgchart_cu_users_Highlight";
	}
	...
	function getDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "tbl_users") {
			$sDetailUrl = $GLOBALS["tbl_users"]->ListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&cu=" . $this->code->CurrentValue;
		}
		return $sDetailUrl;
	}
	...
	function SqlFrom() { // From
		return "`view_tbl_uporgchart_cu_users`";
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `view_tbl_uporgchart_cu_users` ($names) VALUES ($values)";
	}

	// UPDATE statement
	function UpdateSQL(&$rs) {
		global $conn;
		$SQL = "UPDATE `view_tbl_uporgchart_cu_users` SET ";
		foreach ($rs as $name => $value) {
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `view_tbl_uporgchart_cu_users` WHERE ";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "view_tbl_uporgchart_cu_userslist.php";
		}
	}
	...
	// View URL
	function ViewUrl() {
		return $this->KeyUrl("view_tbl_uporgchart_cu_usersview.php", $this->UrlParm());
	}

	// Add URL
	function AddUrl() {
		$AddUrl = "view_tbl_uporgchart_cu_usersadd.php";
...
//			$AddUrl .= "?" . $sUrlParm;
	...
	function EditUrl($parm = "") {
		return $this->KeyUrl("view_tbl_uporgchart_cu_usersedit.php", $this->UrlParm($parm));
	}
	...
	function CopyUrl($parm = "") {
		return $this->KeyUrl("view_tbl_uporgchart_cu_usersadd.php", $this->UrlParm($parm));
	}
	...
	// Delete URL
	function DeleteUrl() {
		return $this->KeyUrl("view_tbl_uporgchart_cu_usersdelete.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=view_tbl_uporgchart_cu_users" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->acronym->setDbValue($rs->fields('acronym'));
   ...
   // Common render codes
		...
		// shortName
		$this->shortName->CellCssStyle = "white-space: nowrap;";

		// name
		$this->name->CellCssStyle = "white-space: nowrap;";

		// acronym
		$this->acronym->CellCssStyle = "white-space: nowrap;";
		...
		// acronym
		$this->acronym->ViewValue = $this->acronym->CurrentValue;
		$this->acronym->ViewCustomAttributes = "";
		...
		// acronym
		$this->acronym->LinkCustomAttributes = "";
		$this->acronym->HrefValue = "";
		$this->acronym->TooltipValue = "";
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$XmlDoc->AddField('code', $this->code->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('shortName', $this->shortName->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('name', $this->name->ExportValue($this->Export, $this->ExportOriginalValue));
					$XmlDoc->AddField('acronym', $this->acronym->ExportValue($this->Export, $this->ExportOriginalValue));
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->acronym);
