...
// Create page object
$ref_facultyrank_edit = new cref_facultyrank_edit();
$Page =& $ref_facultyrank_edit;

// Page init
$ref_facultyrank_edit->Page_Init();

// Page main
$ref_facultyrank_edit->Page_Main();
?>
...
// Create page object
var ref_facultyrank_edit = new up_Page("ref_facultyrank_edit");

// page properties
ref_facultyrank_edit.PageID = "edit"; // page ID
ref_facultyrank_edit.FormID = "fref_facultyrankedit"; // form ID
var UP_PAGE_ID = ref_facultyrank_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
ref_facultyrank_edit.ValidateForm = function(fobj) {
...
ref_facultyrank_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
ref_facultyrank_edit.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
ref_facultyrank_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
ref_facultyrank_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $ref_facultyrank->TableCaption() ?></p>
<p class="upbudgetofficeclass"><a href="<?php echo $ref_facultyrank->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $ref_facultyrank_edit->ShowPageHeader(); ?>
<?php
$ref_facultyrank_edit->ShowMessage();
?>
<form name="fref_facultyrankedit" id="fref_facultyrankedit" action="<?php echo up_CurrentPage() ?>" method="post" onsubmit="return ref_facultyrank_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="ref_facultyrank">
...
<div<?php echo $ref_facultyrank->facultyRank_ID->ViewAttributes() ?>><?php echo $ref_facultyrank->facultyRank_ID->EditValue ?></div>
<input type="hidden" name="x_facultyRank_ID" id="x_facultyRank_ID" value="<?php echo up_HtmlEncode($ref_facultyrank->facultyRank_ID->CurrentValue) ?>">
...
<?php
$ref_facultyrank_edit->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<script language="JavaScript" type="text/javascript">
...
<?php
$ref_facultyrank_edit->Page_Terminate();
?>
<?php
...
class cref_facultyrank_edit {
	...
	// Page object name
	var $PageObjName = 'ref_facultyrank_edit';
	...
	//
	function cref_facultyrank_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$ref_facultyrank->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$ref_facultyrank->CurrentAction = ""; // Form error, reset action
				$this->setFailureMessage($gsFormError);
				$ref_facultyrank->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$ref_facultyrank->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($ref_facultyrank->facultyRank_ID->CurrentValue == "")
			$this->Page_Terminate("ref_facultyranklist.php"); // Invalid key, return to list
		switch ($ref_facultyrank->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$ref_facultyrank->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$ref_facultyrank->RowType = UP_ROWTYPE_EDIT; // Render as Edit
	...
	function RenderRow() {
		...
		if ($ref_facultyrank->RowType == UP_ROWTYPE_VIEW) { // View row
		...
		} elseif ($ref_facultyrank->RowType == UP_ROWTYPE_EDIT) { // Edit row
			...
			$ref_facultyrank->facultyRank_ID->EditValue = $ref_facultyrank->facultyRank_ID->CurrentValue;
			$ref_facultyrank->facultyRank_ID->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// facultyRank_facultyRank2012
			$ref_facultyrank->facultyRank_facultyRank2012->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_facultyRank2012->CurrentValue, NULL, $ref_facultyrank->facultyRank_facultyRank2012->ReadOnly);

			// facultyRank_numericRank2012
			$ref_facultyrank->facultyRank_numericRank2012->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_numericRank2012->CurrentValue, NULL, $ref_facultyrank->facultyRank_numericRank2012->ReadOnly);

			// facultyRank_ID
			// facultyRank_UPRank

			$ref_facultyrank->facultyRank_UPRank->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_UPRank->CurrentValue, NULL, $ref_facultyrank->facultyRank_UPRank->ReadOnly);

			// facultyRank_rankGroup
			$ref_facultyrank->facultyRank_rankGroup->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_rankGroup->CurrentValue, NULL, $ref_facultyrank->facultyRank_rankGroup->ReadOnly);

			// facultyRank_isRegular
			$ref_facultyrank->facultyRank_isRegular->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_isRegular->CurrentValue, NULL, $ref_facultyrank->facultyRank_isRegular->ReadOnly);

			// facultyRank_category
			$ref_facultyrank->facultyRank_category->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_category->CurrentValue, NULL, $ref_facultyrank->facultyRank_category->ReadOnly);

			// facultyRank_salaryScale
			$ref_facultyrank->facultyRank_salaryScale->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_salaryScale->CurrentValue, NULL, $ref_facultyrank->facultyRank_salaryScale->ReadOnly);

			// facultyRank_reportingDBM
			$ref_facultyrank->facultyRank_reportingDBM->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_reportingDBM->CurrentValue, NULL, $ref_facultyrank->facultyRank_reportingDBM->ReadOnly);

			// facultyRank_reportingCHED
			$ref_facultyrank->facultyRank_reportingCHED->SetDbValueDef($rsnew, $ref_facultyrank->facultyRank_reportingCHED->CurrentValue, NULL, $ref_facultyrank->facultyRank_reportingCHED->ReadOnly);

			// Call Row Updating event
			$bUpdateRow = $ref_facultyrank->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'up_ErrorFn';
				if (count($rsnew) > 0)
					$EditRow = $conn->Execute($ref_facultyrank->UpdateSQL($rsnew));
				else
		...
		if ($EditRow)
			$ref_facultyrank->Row_Updated($rsold, $rsnew);
		if ($EditRow) {
	  ...
	  $usr = CurrentUserID();
		foreach (array_keys($rsnew) as $fldname) {
			if ($ref_facultyrank->fields[$fldname]->FldDataType <> UP_DATATYPE_BLOB) { // Ignore BLOB fields
				if ($ref_facultyrank->fields[$fldname]->FldDataType == UP_DATATYPE_DATE) { // DateTime field
				...
				if ($modified) {
					if ($ref_facultyrank->fields[$fldname]->FldDataType == UP_DATATYPE_MEMO) { // Memo field
					...
					} elseif ($ref_facultyrank->fields[$fldname]->FldDataType == UP_DATATYPE_XML) { // XML field
						$oldvalue = "[XML]";
						$newvalue = "[XML]";
					} else {
						$oldvalue = $rsold[$fldname];
