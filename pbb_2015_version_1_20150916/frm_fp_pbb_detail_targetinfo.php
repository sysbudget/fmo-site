...
class cfrm_fp_pbb_detail_target {
	...
	// Master filter
	function SqlMasterFilter_frm_fp_completion_status() {
		return "`units_id`=@units_id@";
	}

	// Detail filter
	function SqlDetailFilter_frm_fp_completion_status() {
		return "`units_id`=@units_id@";
	}

	// Master filter
	function SqlMasterFilter_frm_fp_forma() {
		return "`Sequence`=@Sequence@";
	}

	// Detail filter
	function SqlDetailFilter_frm_fp_forma() {
		return "`mfo_sequence`=@mfo_sequence@";
	}
...
//			$AddUrl .= "?" . $sUrlParm;
   ...
   // Common render codes
	...
	function AddMasterUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "frm_fp_completion_status") {
			$sFilterWrk = $GLOBALS["frm_fp_completion_status"]->AddUserIDFilter($sFilterWrk);
		}
		if ($sCurrentMasterTable == "frm_fp_forma") {
			$sFilterWrk = $GLOBALS["frm_fp_forma"]->AddUserIDFilter($sFilterWrk);
		}
		return $sFilterWrk;
	}
	...
	function AddDetailUserIDFilter($sFilter, $sCurrentMasterTable) {
		$sFilterWrk = $sFilter;
		if ($sCurrentMasterTable == "frm_fp_completion_status") {
			$sSubqueryWrk = $GLOBALS["frm_fp_completion_status"]->GetUserIDSubquery($this->units_id, $GLOBALS["frm_fp_completion_status"]->units_id);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
		...
		if ($sCurrentMasterTable == "frm_fp_forma") {
			$sSubqueryWrk = $GLOBALS["frm_fp_forma"]->GetUserIDSubquery($this->mfo_sequence, $GLOBALS["frm_fp_forma"]->Sequence);
			if ($sSubqueryWrk <> "") {
				if ($sFilterWrk <> "") {
					$sFilterWrk = "($sFilterWrk) AND ($sSubqueryWrk)";
				} else {
