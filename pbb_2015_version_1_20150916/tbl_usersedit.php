...
class ctbl_users_edit {
	...
	function EditRow() {
		...
		$sFilter = $tbl_users->KeyFilter();
			...
			$sFilterChk = "(`users_password` = '" . up_AdjustSql($tbl_users->users_password->CurrentValue) . "')";
			$sFilterChk .= " AND NOT (" . $sFilter . ")";
			$tbl_users->CurrentFilter = $sFilterChk;
			$sSqlChk = $tbl_users->SQL();
			$conn->raiseErrorFn = 'up_ErrorFn';
		...
		if ($rs->EOF) {
			...
			// cu
			$tbl_users->cu->SetDbValueDef($rsnew, $tbl_users->cu->CurrentValue, NULL, $tbl_users->cu->ReadOnly);

			// units_id
			$tbl_users->units_id->SetDbValueDef($rsnew, $tbl_users->units_id->CurrentValue, NULL, $tbl_users->units_id->ReadOnly);

			// cu_unit_name
			$tbl_users->cu_unit_name->SetDbValueDef($rsnew, $tbl_users->cu_unit_name->CurrentValue, NULL, $tbl_users->cu_unit_name->ReadOnly);

			// users_name
			$tbl_users->users_name->SetDbValueDef($rsnew, $tbl_users->users_name->CurrentValue, NULL, $tbl_users->users_name->ReadOnly);

			// users_nameLast
			$tbl_users->users_nameLast->SetDbValueDef($rsnew, $tbl_users->users_nameLast->CurrentValue, NULL, $tbl_users->users_nameLast->ReadOnly);

			// users_nameFirst
			$tbl_users->users_nameFirst->SetDbValueDef($rsnew, $tbl_users->users_nameFirst->CurrentValue, NULL, $tbl_users->users_nameFirst->ReadOnly);
			...
			// users_userLoginName
			$tbl_users->users_userLoginName->SetDbValueDef($rsnew, $tbl_users->users_userLoginName->CurrentValue, NULL, $tbl_users->users_userLoginName->ReadOnly);

			// users_password
			$tbl_users->users_password->SetDbValueDef($rsnew, $tbl_users->users_password->CurrentValue, NULL, $tbl_users->users_password->ReadOnly || (UP_ENCRYPTED_PASSWORD && $rs->fields('users_password') == $tbl_users->users_password->CurrentValue));
