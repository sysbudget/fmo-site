...
class cfrm_fp_pbb_detail_target_grid {
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sFilter = $frm_fp_pbb_detail_target->SqlMasterFilter_frm_fp_completion_status();
			if (strval($frm_fp_pbb_detail_target->units_id->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_target->getCurrentMasterTable() == "frm_fp_completion_status") {
				$sFilter = str_replace("@units_id@", up_AdjustSql($frm_fp_pbb_detail_target->units_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
				$rsmaster = $GLOBALS["frm_fp_completion_status"]->LoadRs($sFilter);
				$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->MasterRecordExists) {
			...
			$sFilter = $frm_fp_pbb_detail_target->SqlMasterFilter_frm_fp_forma();
			if (strval($frm_fp_pbb_detail_target->mfo_sequence->CurrentValue) <> "" &&
				$frm_fp_pbb_detail_target->getCurrentMasterTable() == "frm_fp_forma") {
				$sFilter = str_replace("@Sequence@", up_AdjustSql($frm_fp_pbb_detail_target->mfo_sequence->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
				$rsmaster = $GLOBALS["frm_fp_forma"]->LoadRs($sFilter);
				$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->MasterRecordExists) {
					$sMasterUserIdMsg = str_replace("%c", CurrentUserID(), $Language->Phrase("UnAuthorizedMasterUserID"));
					$sMasterUserIdMsg = str_replace("%f", $sFilter, $sMasterUserIdMsg);
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "frm_fp_completion_status") {
			...
			if ($GLOBALS["frm_fp_completion_status"]->EventCancelled) $frm_fp_pbb_detail_target->EventCancelled = TRUE;
		}
		if ($sMasterTblVar == "frm_fp_forma") {
			$frm_fp_pbb_detail_target->mfo_sequence->Visible = FALSE;
			if ($GLOBALS["frm_fp_forma"]->EventCancelled) $frm_fp_pbb_detail_target->EventCancelled = TRUE;
		}
