...
// Create page object
$frm_fp_rep_a_eligible_status_unit_mfo_list = new cfrm_fp_rep_a_eligible_status_unit_mfo_list();
$Page =& $frm_fp_rep_a_eligible_status_unit_mfo_list;

// Page init
$frm_fp_rep_a_eligible_status_unit_mfo_list->Page_Init();

// Page main
$frm_fp_rep_a_eligible_status_unit_mfo_list->Page_Main();
?>
...
// Create page object
var frm_fp_rep_a_eligible_status_unit_mfo_list = new up_Page("frm_fp_rep_a_eligible_status_unit_mfo_list");

// page properties
frm_fp_rep_a_eligible_status_unit_mfo_list.PageID = "list"; // page ID
frm_fp_rep_a_eligible_status_unit_mfo_list.FormID = "ffrm_fp_rep_a_eligible_status_unit_mfolist"; // form ID
var UP_PAGE_ID = frm_fp_rep_a_eligible_status_unit_mfo_list.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
frm_fp_rep_a_eligible_status_unit_mfo_list.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
frm_fp_rep_a_eligible_status_unit_mfo_list.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
frm_fp_rep_a_eligible_status_unit_mfo_list.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
frm_fp_rep_a_eligible_status_unit_mfo_list.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php } ?>
<?php if (($frm_fp_rep_a_eligible_status_unit_mfo->Export == "") || (UP_EXPORT_MASTER_RECORD && $frm_fp_rep_a_eligible_status_unit_mfo->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "frm_fp_rep_a_eligible_status_unitlist.php";
if ($frm_fp_rep_a_eligible_status_unit_mfo_list->DbMasterFilter <> "" && $frm_fp_rep_a_eligible_status_unit_mfo->getCurrentMasterTable() == "frm_fp_rep_a_eligible_status_unit") {
	if ($frm_fp_rep_a_eligible_status_unit_mfo_list->MasterRecordExists) {
		if ($frm_fp_rep_a_eligible_status_unit_mfo->getCurrentMasterTable() == $frm_fp_rep_a_eligible_status_unit_mfo->TableVar) $gsMasterReturnUrl .= "?" . UP_TABLE_SHOW_MASTER . "=";
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $frm_fp_rep_a_eligible_status_unit->TableCaption() ?>
&nbsp;&nbsp;<?php $frm_fp_rep_a_eligible_status_unit_mfo_list->ExportOptions->Render("body"); ?>
</p>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "") { ?>
<p class="upbudgetofficeclass"><a href="<?php echo $gsMasterReturnUrl ?>"><?php echo $Language->Phrase("BackToMasterPage") ?></a></p>
<?php } ?>
<?php include_once "frm_fp_rep_a_eligible_status_unitmaster.php" ?>
<?php
	}
}
?>
...
<?php
	...
	if ($bSelectLimit) {
		$frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs = $frm_fp_rep_a_eligible_status_unit_mfo->SelectRecordCount();
	} else {
		if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset = $frm_fp_rep_a_eligible_status_unit_mfo_list->LoadRecordset())
			$frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs = $frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->RecordCount();
	}
	$frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec = 1;
	if ($frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs <= 0 || ($frm_fp_rep_a_eligible_status_unit_mfo->Export <> "" && $frm_fp_rep_a_eligible_status_unit_mfo->ExportAll)) // Display all records
		$frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs = $frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs;
	if (!($frm_fp_rep_a_eligible_status_unit_mfo->Export <> "" && $frm_fp_rep_a_eligible_status_unit_mfo->ExportAll))
		$frm_fp_rep_a_eligible_status_unit_mfo_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset = $frm_fp_rep_a_eligible_status_unit_mfo_list->LoadRecordset($frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec-1, $frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs);
?>
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->getCurrentMasterTable() == "") { ?>
&nbsp;&nbsp;<?php $frm_fp_rep_a_eligible_status_unit_mfo_list->ExportOptions->Render("body"); ?>
<?php } ?>
</p>
<?php if ($Security->CanSearch()) { ?>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "" && $frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction == "") { ?>
<a href="javascript:up_ToggleSearchPanel(frm_fp_rep_a_eligible_status_unit_mfo_list);" style="text-decoration: none;"><img id="frm_fp_rep_a_eligible_status_unit_mfo_list_SearchImage" src="phpimages/collapse.gif" alt="" width="9" height="9" border="0"></a><span class="upbudgetofficeclass">&nbsp;<?php echo $Language->Phrase("Search") ?></span><br>
<div id="frm_fp_rep_a_eligible_status_unit_mfo_list_SearchPanel">
<form name="ffrm_fp_rep_a_eligible_status_unit_mfolistsrch" id="ffrm_fp_rep_a_eligible_status_unit_mfolistsrch" class="upForm" action="<?php echo up_CurrentPage() ?>">
<input type="hidden" id="t" name="t" value="frm_fp_rep_a_eligible_status_unit_mfo">
<div class="upBasicSearch">
<div id="xsr_1" class="upCssTableRow">
	<input type="text" name="<?php echo UP_TABLE_BASIC_SEARCH ?>" id="<?php echo UP_TABLE_BASIC_SEARCH ?>" size="20" value="<?php echo up_HtmlEncode($frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchKeyword()) ?>">
	<input type="Submit" name="Submit" id="Submit" value="<?php echo up_BtnCaption($Language->Phrase("QuickSearchBtn")) ?>">&nbsp;
	<a href="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>&nbsp;
</div>
<div id="xsr_2" class="upCssTableRow">
	<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value=""<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchType() == "") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>&nbsp;&nbsp;<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>&nbsp;&nbsp;<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</form>
</div>
<?php } ?>
<?php } ?>
<?php $frm_fp_rep_a_eligible_status_unit_mfo_list->ShowPageHeader(); ?>
<?php
$frm_fp_rep_a_eligible_status_unit_mfo_list->ShowMessage();
?>
<br>
<table cellspacing="0" class="upGrid"><tr><td class="upGridContent">
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "") { ?>
<div class="upGridUpperPanel">
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction <> "gridadd" && $frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction <> "gridedit") { ?>
<form name="uppagerform" id="uppagerform" class="upForm" action="<?php echo up_CurrentPage() ?>">
<table border="0" cellspacing="0" cellpadding="0" class="upPager">
	<tr>
		<td nowrap>
<?php if (!isset($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager)) $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager = new cPrevNextPager($frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec, $frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs, $frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs) ?>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->RecordCount > 0) { ?>
	<table border="0" cellspacing="0" cellpadding="0"><tr><td><span class="upbudgetofficeclass"><?php echo $Language->Phrase("Page") ?>&nbsp;</span></td>
<!--first page button-->
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->FirstButton->Enabled) { ?>
	<td><a href="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageUrl() ?>start=<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->FirstButton->Start ?>"><img src="phpimages/first.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/firstdisab.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->PrevButton->Enabled) { ?>
	<td><a href="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageUrl() ?>start=<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->PrevButton->Start ?>"><img src="phpimages/prev.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/prevdisab.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--current page number-->
	<td><input type="text" name="<?php echo UP_TABLE_PAGE_NO ?>" id="<?php echo UP_TABLE_PAGE_NO ?>" value="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->CurrentPage ?>" size="4"></td>
<!--next page button-->
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->NextButton->Enabled) { ?>
	<td><a href="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageUrl() ?>start=<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->NextButton->Start ?>"><img src="phpimages/next.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/nextdisab.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->LastButton->Enabled) { ?>
	<td><a href="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageUrl() ?>start=<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->LastButton->Start ?>"><img src="phpimages/last.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/lastdisab.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
	<td><span class="upbudgetofficeclass">&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->PageCount ?></span></td>
	</tr></table>
	</td>	
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td>
	<span class="upbudgetofficeclass"><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->Pager->RecordCount ?></span>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->SearchWhere == "0=101") { ?>
...
<?php } ?>
<form name="ffrm_fp_rep_a_eligible_status_unit_mfolist" id="ffrm_fp_rep_a_eligible_status_unit_mfolist" class="upForm" action="" method="post">
<input type="hidden" name="t" id="t" value="frm_fp_rep_a_eligible_status_unit_mfo">
<div id="gmp_frm_fp_rep_a_eligible_status_unit_mfo" class="upGridMiddlePanel">
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs > 0) { ?>
...
// Render list options
$frm_fp_rep_a_eligible_status_unit_mfo_list->RenderListOptions();

// Render list options (header, left)
$frm_fp_rep_a_eligible_status_unit_mfo_list->ListOptions->Render("header", "left");
?>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->Visible) { // cu_unit_name ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn" style="white-space: nowrap;"><thead><tr><td><?php echo $frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->Visible) { // mfo_name_rep ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn" style="white-space: nowrap;"><thead><tr><td><?php echo $frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Participated_PI->Visible) { // Participated PI ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Participated_PI) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Not_Started_PI->Visible) { // Not Started PI ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Not_Started_PI) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Started_PI->Visible) { // % Not Started PI ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Started_PI) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Status->Visible) { // Status ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Status) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn" style="white-space: nowrap;"><thead><tr><td><?php echo $frm_fp_rep_a_eligible_status_unit_mfo->Status->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Status->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_fp_rep_a_eligible_status_unit_mfo->Status->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Not_Eligible_PI_Count->Visible) { // Not Eligible PI Count ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Not_Eligible_PI_Count) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Eligible_PI_Count->Visible) { // % Not Eligible PI Count ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Eligible_PI_Count) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Eligible_PI_Count->Visible) { // Eligible PI Count ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Eligible_PI_Count) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->z25_Eligible_PI_Count->Visible) { // % Eligible PI Count ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->z25_Eligible_PI_Count) ?>',1);">
...
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Remarks->Visible) { // Remarks ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_fp_rep_a_eligible_status_unit_mfo->SortUrl($frm_fp_rep_a_eligible_status_unit_mfo->Remarks) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn" style="white-space: nowrap;"><thead><tr><td><?php echo $frm_fp_rep_a_eligible_status_unit_mfo->Remarks->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Remarks->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_fp_rep_a_eligible_status_unit_mfo->Remarks->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php
...
$frm_fp_rep_a_eligible_status_unit_mfo_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<?php
if ($frm_fp_rep_a_eligible_status_unit_mfo->ExportAll && $frm_fp_rep_a_eligible_status_unit_mfo->Export <> "") {
	$frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec = $frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs;
} else {

	// Set the last record to display
	if ($frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs > $frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec + $frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs - 1)
		$frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec = $frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec + $frm_fp_rep_a_eligible_status_unit_mfo_list->DisplayRecs - 1;
	else
		$frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec = $frm_fp_rep_a_eligible_status_unit_mfo_list->TotalRecs;
}
$frm_fp_rep_a_eligible_status_unit_mfo_list->RecCnt = $frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec - 1;
if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset && !$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->EOF) {
	$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec > 1)
		$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->Move($frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec - 1);
} elseif (!$frm_fp_rep_a_eligible_status_unit_mfo->AllowAddDeleteRow && $frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec == 0) {
	$frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec = $frm_fp_rep_a_eligible_status_unit_mfo->GridAddRowCount;
}
...
$frm_fp_rep_a_eligible_status_unit_mfo_list->RenderRow();
$frm_fp_rep_a_eligible_status_unit_mfo_list->RowCnt = 0;
while ($frm_fp_rep_a_eligible_status_unit_mfo_list->RecCnt < $frm_fp_rep_a_eligible_status_unit_mfo_list->StopRec) {
	$frm_fp_rep_a_eligible_status_unit_mfo_list->RecCnt++;
	if (intval($frm_fp_rep_a_eligible_status_unit_mfo_list->RecCnt) >= intval($frm_fp_rep_a_eligible_status_unit_mfo_list->StartRec)) {
		$frm_fp_rep_a_eligible_status_unit_mfo_list->RowCnt++;

		// Set up key count
		$frm_fp_rep_a_eligible_status_unit_mfo_list->KeyCount = $frm_fp_rep_a_eligible_status_unit_mfo_list->RowIndex;
		...
		if ($frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction == "gridadd") {
			...
			$frm_fp_rep_a_eligible_status_unit_mfo_list->LoadRowValues($frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset); // Load row values
		}
		$frm_fp_rep_a_eligible_status_unit_mfo->RowType = UP_ROWTYPE_VIEW; // Render view
		$frm_fp_rep_a_eligible_status_unit_mfo->RowAttrs = array('onmouseover'=>'up_MouseOver(event, this);', 'onmouseout'=>'up_MouseOut(event, this);', 'onclick'=>'up_Click(event, this);');
...
$frm_fp_rep_a_eligible_status_unit_mfo_list->ListOptions->Render("body", "left");
?>
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->Visible) { // cu_unit_name ?>
...
<a name="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageObjName . "_row_" . $frm_fp_rep_a_eligible_status_unit_mfo_list->RowCnt ?>" id="<?php echo $frm_fp_rep_a_eligible_status_unit_mfo_list->PageObjName . "_row_" . $frm_fp_rep_a_eligible_status_unit_mfo_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->Visible) { // mfo_name_rep ?>
...
$frm_fp_rep_a_eligible_status_unit_mfo_list->ListOptions->Render("body", "right");
?>
	</tr>
<?php
	}
	if ($frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction <> "gridadd")
		$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
...
// Close recordset
if ($frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset)
	$frm_fp_rep_a_eligible_status_unit_mfo_list->Recordset->Close();
?>
</td></tr></table>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "" && $frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction == "") { ?>
<?php } ?>
<?php
$frm_fp_rep_a_eligible_status_unit_mfo_list->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<?php if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "") { ?>
...
<?php
$frm_fp_rep_a_eligible_status_unit_mfo_list->Page_Terminate();
?>
<?php
...
// Page class
//
class cfrm_fp_rep_a_eligible_status_unit_mfo_list {
	...
	// Page object name
	var $PageObjName = 'frm_fp_rep_a_eligible_status_unit_mfo_list';
	...
	//
	function cfrm_fp_rep_a_eligible_status_unit_mfo_list() {
		...
		$this->AddUrl = "frm_fp_rep_a_eligible_status_unit_mfoadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "frm_fp_rep_a_eligible_status_unit_mfodelete.php";
		$this->MultiUpdateUrl = "frm_fp_rep_a_eligible_status_unit_mfoupdate.php";
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_fp_rep_a_eligible_status_unit_mfo->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_fp_rep_a_eligible_status_unit_mfo->Export = $_POST["exporttype"];
		} else {
			$frm_fp_rep_a_eligible_status_unit_mfo->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_fp_rep_a_eligible_status_unit_mfo->Export; // Get export parameter, used in header
		$gsExportFile = $frm_fp_rep_a_eligible_status_unit_mfo->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "csv") {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			$frm_fp_rep_a_eligible_status_unit_mfo->Recordset_SearchValidated();
		...
		$frm_fp_rep_a_eligible_status_unit_mfo->Recordset_Searching($this->SearchWhere);

		// Save search criteria
		if ($this->SearchWhere <> "") {
			if ($sSrchBasic == "")
				$this->ResetBasicSearchParms();
			$frm_fp_rep_a_eligible_status_unit_mfo->setSearchWhere($this->SearchWhere); // Save to Session
			if (!$this->RestoreSearch) {
				$this->StartRec = 1; // Reset start record counter
				$frm_fp_rep_a_eligible_status_unit_mfo->setStartRecordNumber($this->StartRec);
			}
		} else {
			$this->SearchWhere = $frm_fp_rep_a_eligible_status_unit_mfo->getSearchWhere();
		}
		...
		// Export data only
		if (in_array($frm_fp_rep_a_eligible_status_unit_mfo->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_fp_rep_a_eligible_status_unit_mfo->Export <> "email")
	...
	function BasicSearchSQL($Keyword) {
		...
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $frm_fp_rep_a_eligible_status_unit_mfo->cu_short_name, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_fp_rep_a_eligible_status_unit_mfo->Status, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_fp_rep_a_eligible_status_unit_mfo->Remarks, $Keyword);
		return $sWhere;
	}
	...
	function BasicSearchWhere() {
		...
		$sSearchKeyword = $frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchKeyword;
		$sSearchType = $frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchType;
		if ($sSearchKeyword <> "") {
		...
		if ($sSearchKeyword <> "") {
			$frm_fp_rep_a_eligible_status_unit_mfo->setSessionBasicSearchKeyword($sSearchKeyword);
			$frm_fp_rep_a_eligible_status_unit_mfo->setSessionBasicSearchType($sSearchType);
		}
		return $sSearchStr;
	}
	...
	function ResetSearchParms() {
		...
		$this->SearchWhere = "";
		$frm_fp_rep_a_eligible_status_unit_mfo->setSearchWhere($this->SearchWhere);
	...
	function ResetBasicSearchParms() {
		global $frm_fp_rep_a_eligible_status_unit_mfo;
		$frm_fp_rep_a_eligible_status_unit_mfo->setSessionBasicSearchKeyword("");
		$frm_fp_rep_a_eligible_status_unit_mfo->setSessionBasicSearchType("");
	}
	...
	function RestoreSearchParms() {
		...
		if ($bRestore) {
			...
			$frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchKeyword = $frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchKeyword();
			$frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchType = $frm_fp_rep_a_eligible_status_unit_mfo->getSessionBasicSearchType();
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name); // cu_unit_name
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep); // mfo_name_rep
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Participated_PI); // Participated PI
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Not_Started_PI); // Not Started PI
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Started_PI); // % Not Started PI
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Status); // Status
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Not_Eligible_PI_Count); // Not Eligible PI Count
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Eligible_PI_Count); // % Not Eligible PI Count
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Eligible_PI_Count); // Eligible PI Count
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->z25_Eligible_PI_Count); // % Eligible PI Count
			$frm_fp_rep_a_eligible_status_unit_mfo->UpdateSort($frm_fp_rep_a_eligible_status_unit_mfo->Remarks); // Remarks
	...
	function ResetCmd() {
		...
		if (@$_GET["cmd"] <> "") {
			...
			if (strtolower($sCmd) == "resetsort") {
				$sOrderBy = "";
				$frm_fp_rep_a_eligible_status_unit_mfo->setSessionOrderBy($sOrderBy);
				$frm_fp_rep_a_eligible_status_unit_mfo->cu_unit_name->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->mfo_name_rep->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Participated_PI->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Not_Started_PI->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Started_PI->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Status->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Not_Eligible_PI_Count->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->z25_Not_Eligible_PI_Count->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Eligible_PI_Count->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->z25_Eligible_PI_Count->setSort("");
				$frm_fp_rep_a_eligible_status_unit_mfo->Remarks->setSort("");
			}
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList('frm_fp_pbb_detail_accomplishment') && $this->ShowOptionLink()) {
			$oListOpt->Body = $Language->Phrase("DetailLink") . $Language->TablePhrase("frm_fp_pbb_detail_accomplishment", "TblCaption");
			$oListOpt->Body = "<a class=\"upRowLink\" href=\"frm_fp_pbb_detail_accomplishmentlist.php?" . UP_TABLE_SHOW_MASTER . "=frm_fp_rep_a_eligible_status_unit_mfo&units_id=" . urlencode(strval($frm_fp_rep_a_eligible_status_unit_mfo->units_id->CurrentValue)) . "&mfo=" . urlencode(strval($frm_fp_rep_a_eligible_status_unit_mfo->mfo->CurrentValue)) . "\">" . $oListOpt->Body . "</a>";
			$links = "";
	...
	function RenderListOptionsExt() {
		global $Security, $Language, $frm_fp_rep_a_eligible_status_unit_mfo;
		$sSqlWrk = "`units_id`=" . up_AdjustSql($frm_fp_rep_a_eligible_status_unit_mfo->units_id->CurrentValue) . "";
		$sSqlWrk = $sSqlWrk . " AND " . "`mfo`=" . up_AdjustSql($frm_fp_rep_a_eligible_status_unit_mfo->mfo->CurrentValue) . "";
		$sSqlWrk = TEAencrypt($sSqlWrk, UP_RANDOM_KEY);
		$sSqlWrk = str_replace("'", "\'", $sSqlWrk);
		$sHyperLinkParm = " href=\"frm_fp_pbb_detail_accomplishmentlist.php?" . UP_TABLE_SHOW_MASTER . "=frm_fp_rep_a_eligible_status_unit_mfo&units_id=" . urlencode(strval($frm_fp_rep_a_eligible_status_unit_mfo->units_id->CurrentValue)) . "&mfo=" . urlencode(strval($frm_fp_rep_a_eligible_status_unit_mfo->mfo->CurrentValue)) . "\"";
		$oListOpt =& $this->ListOptions->Items["detail_frm_fp_pbb_detail_accomplishment"];
		...
		$sHyperLinkParm = " href=\"javascript:void(0);\" name=\"dl%i_frm_fp_rep_a_eligible_status_unit_mfo_frm_fp_pbb_detail_accomplishment\" id=\"dl%i_frm_fp_rep_a_eligible_status_unit_mfo_frm_fp_pbb_detail_accomplishment\" onclick=\"up_AjaxShowDetails2(event, this, 'frm_fp_pbb_detail_accomplishmentpreview.php?f=%s')\"";		
	...
	function LoadBasicSearchValues() {
		global $frm_fp_rep_a_eligible_status_unit_mfo;
		$frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchKeyword = @$_GET[UP_TABLE_BASIC_SEARCH];
		$frm_fp_rep_a_eligible_status_unit_mfo->BasicSearchType = @$_GET[UP_TABLE_BASIC_SEARCH_TYPE];
	}
	...
	function RenderRow() {
		...
		// Initialize URLs
		$this->ViewUrl = $frm_fp_rep_a_eligible_status_unit_mfo->ViewUrl();
		$this->EditUrl = $frm_fp_rep_a_eligible_status_unit_mfo->EditUrl();
		$this->InlineEditUrl = $frm_fp_rep_a_eligible_status_unit_mfo->InlineEditUrl();
		$this->CopyUrl = $frm_fp_rep_a_eligible_status_unit_mfo->CopyUrl();
		$this->InlineCopyUrl = $frm_fp_rep_a_eligible_status_unit_mfo->InlineCopyUrl();
		$this->DeleteUrl = $frm_fp_rep_a_eligible_status_unit_mfo->DeleteUrl();
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_frm_fp_rep_a_eligible_status_unit_mfo\" id=\"emf_frm_fp_rep_a_eligible_status_unit_mfo\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_fp_rep_a_eligible_status_unit_mfo',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_fp_rep_a_eligible_status_unit_mfolist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_fp_rep_a_eligible_status_unit_mfo->Export <> "" ||
			$frm_fp_rep_a_eligible_status_unit_mfo->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_fp_rep_a_eligible_status_unit_mfo->SelectRecordCount();
		} else {
		...
		// Export all
		if ($frm_fp_rep_a_eligible_status_unit_mfo->ExportAll) {
		...
		if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_fp_rep_a_eligible_status_unit_mfo, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_fp_rep_a_eligible_status_unit_mfo->getMasterFilter() <> "" && $frm_fp_rep_a_eligible_status_unit_mfo->getCurrentMasterTable() == "frm_fp_rep_a_eligible_status_unit") {
			global $frm_fp_rep_a_eligible_status_unit;
			$rsmaster = $frm_fp_rep_a_eligible_status_unit->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "xml") {
					$ParentTable = "frm_fp_rep_a_eligible_status_unit";
					$frm_fp_rep_a_eligible_status_unit->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_fp_rep_a_eligible_status_unit_mfo->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_fp_rep_a_eligible_status_unit->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "xml") {
			$frm_fp_rep_a_eligible_status_unit_mfo->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_fp_rep_a_eligible_status_unit_mfo->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_fp_rep_a_eligible_status_unit_mfo->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_fp_rep_a_eligible_status_unit_mfo->ExportReturnUrl());
		} elseif ($frm_fp_rep_a_eligible_status_unit_mfo->Export == "pdf") {
	...
	function SetUpMasterParms() {
		...
		if (@$_GET[UP_TABLE_SHOW_MASTER] <> "") {
			...
			if ($sMasterTblVar == "frm_fp_rep_a_eligible_status_unit") {
				$bValidMaster = TRUE;
				if (@$_GET["units_id"] <> "") {
					$GLOBALS["frm_fp_rep_a_eligible_status_unit"]->units_id->setQueryStringValue($_GET["units_id"]);
					$frm_fp_rep_a_eligible_status_unit_mfo->units_id->setQueryStringValue($GLOBALS["frm_fp_rep_a_eligible_status_unit"]->units_id->QueryStringValue);
					$frm_fp_rep_a_eligible_status_unit_mfo->units_id->setSessionValue($frm_fp_rep_a_eligible_status_unit_mfo->units_id->QueryStringValue);
					if (!is_numeric($GLOBALS["frm_fp_rep_a_eligible_status_unit"]->units_id->QueryStringValue)) $bValidMaster = FALSE;
				} else {
					$bValidMaster = FALSE;
				}
			}
		...
		if ($bValidMaster) {
			...
			$frm_fp_rep_a_eligible_status_unit_mfo->setCurrentMasterTable($sMasterTblVar);
			...
			if ($sMasterTblVar <> "frm_fp_rep_a_eligible_status_unit") {
				if ($frm_fp_rep_a_eligible_status_unit_mfo->units_id->QueryStringValue == "") $frm_fp_rep_a_eligible_status_unit_mfo->units_id->setSessionValue("");
			}
		}
