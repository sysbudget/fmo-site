...
// Create page object
$frm_sam_units_list = new cfrm_sam_units_list();
$Page =& $frm_sam_units_list;

// Page init
$frm_sam_units_list->Page_Init();

// Page main
$frm_sam_units_list->Page_Main();
?>
...
// Create page object
var frm_sam_units_list = new up_Page("frm_sam_units_list");

// page properties
frm_sam_units_list.PageID = "list"; // page ID
frm_sam_units_list.FormID = "ffrm_sam_unitslist"; // form ID
var UP_PAGE_ID = frm_sam_units_list.PageID; // for backward compatibility

// extend page with Form_CustomValidate function
frm_sam_units_list.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
frm_sam_units_list.SelectAllKey = function(elem) {
	up_SelectAll(elem);
	up_ClickAll(elem);
}
<?php if (UP_CLIENT_VALIDATE) { ?>
frm_sam_units_list.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
frm_sam_units_list.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<?php } ?>
<?php if (($frm_sam_units->Export == "") || (UP_EXPORT_MASTER_RECORD && $frm_sam_units->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "frm_sam_cu_executive_officeslist.php";
if ($frm_sam_units_list->DbMasterFilter <> "" && $frm_sam_units->getCurrentMasterTable() == "frm_sam_cu_executive_offices") {
	if ($frm_sam_units_list->MasterRecordExists) {
		if ($frm_sam_units->getCurrentMasterTable() == $frm_sam_units->TableVar) $gsMasterReturnUrl .= "?" . UP_TABLE_SHOW_MASTER . "=";
?>
<p class="upbudgetofficeclass upTitle"><?php echo $Language->Phrase("MasterRecord") ?><?php echo $frm_sam_cu_executive_offices->TableCaption() ?>
&nbsp;&nbsp;<?php $frm_sam_units_list->ExportOptions->Render("body"); ?>
</p>
<?php if ($frm_sam_units->Export == "") { ?>
<p class="upbudgetofficeclass"><a href="<?php echo $gsMasterReturnUrl ?>"><?php echo $Language->Phrase("BackToMasterPage") ?></a></p>
<?php } ?>
<?php include_once "frm_sam_cu_executive_officesmaster.php" ?>
<?php
	}
}
?>
...
<?php
	...
	if ($bSelectLimit) {
		$frm_sam_units_list->TotalRecs = $frm_sam_units->SelectRecordCount();
	} else {
		if ($frm_sam_units_list->Recordset = $frm_sam_units_list->LoadRecordset())
			$frm_sam_units_list->TotalRecs = $frm_sam_units_list->Recordset->RecordCount();
	}
	$frm_sam_units_list->StartRec = 1;
	if ($frm_sam_units_list->DisplayRecs <= 0 || ($frm_sam_units->Export <> "" && $frm_sam_units->ExportAll)) // Display all records
		$frm_sam_units_list->DisplayRecs = $frm_sam_units_list->TotalRecs;
	if (!($frm_sam_units->Export <> "" && $frm_sam_units->ExportAll))
		$frm_sam_units_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$frm_sam_units_list->Recordset = $frm_sam_units_list->LoadRecordset($frm_sam_units_list->StartRec-1, $frm_sam_units_list->DisplayRecs);
?>
...
<?php if ($frm_sam_units->getCurrentMasterTable() == "") { ?>
&nbsp;&nbsp;<?php $frm_sam_units_list->ExportOptions->Render("body"); ?>
<?php } ?>
</p>
<?php if ($Security->CanSearch()) { ?>
<?php if ($frm_sam_units->Export == "" && $frm_sam_units->CurrentAction == "") { ?>
<a href="javascript:up_ToggleSearchPanel(frm_sam_units_list);" style="text-decoration: none;"><img id="frm_sam_units_list_SearchImage" src="phpimages/collapse.gif" alt="" width="9" height="9" border="0"></a><span class="upbudgetofficeclass">&nbsp;<?php echo $Language->Phrase("Search") ?></span><br>
<div id="frm_sam_units_list_SearchPanel">
<form name="ffrm_sam_unitslistsrch" id="ffrm_sam_unitslistsrch" class="upForm" action="<?php echo up_CurrentPage() ?>">
<input type="hidden" id="t" name="t" value="frm_sam_units">
<div class="upBasicSearch">
<div id="xsr_1" class="upCssTableRow">
	<input type="text" name="<?php echo UP_TABLE_BASIC_SEARCH ?>" id="<?php echo UP_TABLE_BASIC_SEARCH ?>" size="20" value="<?php echo up_HtmlEncode($frm_sam_units->getSessionBasicSearchKeyword()) ?>">
	<input type="Submit" name="Submit" id="Submit" value="<?php echo up_BtnCaption($Language->Phrase("QuickSearchBtn")) ?>">&nbsp;
	<a href="<?php echo $frm_sam_units_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>&nbsp;
</div>
<div id="xsr_2" class="upCssTableRow">
	<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value=""<?php if ($frm_sam_units->getSessionBasicSearchType() == "") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>&nbsp;&nbsp;<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($frm_sam_units->getSessionBasicSearchType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>&nbsp;&nbsp;<label><input type="radio" name="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo UP_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($frm_sam_units->getSessionBasicSearchType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</form>
</div>
<?php } ?>
<?php } ?>
<?php $frm_sam_units_list->ShowPageHeader(); ?>
<?php
$frm_sam_units_list->ShowMessage();
?>
<br>
<table cellspacing="0" class="upGrid"><tr><td class="upGridContent">
<?php if ($frm_sam_units->Export == "") { ?>
<div class="upGridUpperPanel">
<?php if ($frm_sam_units->CurrentAction <> "gridadd" && $frm_sam_units->CurrentAction <> "gridedit") { ?>
<form name="uppagerform" id="uppagerform" class="upForm" action="<?php echo up_CurrentPage() ?>">
<table border="0" cellspacing="0" cellpadding="0" class="upPager">
	<tr>
		<td nowrap>
<?php if (!isset($frm_sam_units_list->Pager)) $frm_sam_units_list->Pager = new cPrevNextPager($frm_sam_units_list->StartRec, $frm_sam_units_list->DisplayRecs, $frm_sam_units_list->TotalRecs) ?>
<?php if ($frm_sam_units_list->Pager->RecordCount > 0) { ?>
	<table border="0" cellspacing="0" cellpadding="0"><tr><td><span class="upbudgetofficeclass"><?php echo $Language->Phrase("Page") ?>&nbsp;</span></td>
<!--first page button-->
	<?php if ($frm_sam_units_list->Pager->FirstButton->Enabled) { ?>
	<td><a href="<?php echo $frm_sam_units_list->PageUrl() ?>start=<?php echo $frm_sam_units_list->Pager->FirstButton->Start ?>"><img src="phpimages/first.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/firstdisab.gif" alt="<?php echo $Language->Phrase("PagerFirst") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($frm_sam_units_list->Pager->PrevButton->Enabled) { ?>
	<td><a href="<?php echo $frm_sam_units_list->PageUrl() ?>start=<?php echo $frm_sam_units_list->Pager->PrevButton->Start ?>"><img src="phpimages/prev.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phpimages/prevdisab.gif" alt="<?php echo $Language->Phrase("PagerPrevious") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--current page number-->
	<td><input type="text" name="<?php echo UP_TABLE_PAGE_NO ?>" id="<?php echo UP_TABLE_PAGE_NO ?>" value="<?php echo $frm_sam_units_list->Pager->CurrentPage ?>" size="4"></td>
<!--next page button-->
	<?php if ($frm_sam_units_list->Pager->NextButton->Enabled) { ?>
	<td><a href="<?php echo $frm_sam_units_list->PageUrl() ?>start=<?php echo $frm_sam_units_list->Pager->NextButton->Start ?>"><img src="phpimages/next.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/nextdisab.gif" alt="<?php echo $Language->Phrase("PagerNext") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($frm_sam_units_list->Pager->LastButton->Enabled) { ?>
	<td><a href="<?php echo $frm_sam_units_list->PageUrl() ?>start=<?php echo $frm_sam_units_list->Pager->LastButton->Start ?>"><img src="phpimages/last.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phpimages/lastdisab.gif" alt="<?php echo $Language->Phrase("PagerLast") ?>" width="16" height="16" border="0"></td>
	<?php } ?>
	<td><span class="upbudgetofficeclass">&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $frm_sam_units_list->Pager->PageCount ?></span></td>
	</tr></table>
	</td>	
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td>
	<span class="upbudgetofficeclass"><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $frm_sam_units_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $frm_sam_units_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $frm_sam_units_list->Pager->RecordCount ?></span>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($frm_sam_units_list->SearchWhere == "0=101") { ?>
...
<a class="upGridLink" href="<?php echo $frm_sam_units_list->AddUrl ?>"><?php echo $Language->Phrase("AddLink") ?></a>&nbsp;&nbsp;
<?php } ?>
<?php if ($frm_sam_units_list->TotalRecs > 0) { ?>
<?php if ($Security->CanDelete()) { ?>
<a class="upGridLink" href="" onclick="up_SubmitSelected(document.ffrm_sam_unitslist, '<?php echo $frm_sam_units_list->MultiDeleteUrl ?>');return false;"><?php echo $Language->Phrase("DeleteSelectedLink") ?></a>&nbsp;&nbsp;
<?php } ?>
<?php } ?>
</span>
</div>
<?php } ?>
<form name="ffrm_sam_unitslist" id="ffrm_sam_unitslist" class="upForm" action="" method="post">
...
// Render list options
$frm_sam_units_list->RenderListOptions();

// Render list options (header, left)
$frm_sam_units_list->ListOptions->Render("header", "left");
?>
<?php if ($frm_sam_units->cu_unit_name->Visible) { // cu_unit_name ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->cu_unit_name) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn" style="white-space: nowrap;"><thead><tr><td><?php echo $frm_sam_units->cu_unit_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_sam_units->cu_unit_name->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_sam_units->cu_unit_name->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php if ($frm_sam_units->personnel_count->Visible) { // personnel_count ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->personnel_count) ?>',1);">
...
<?php if ($frm_sam_units->mfo_1->Visible) { // mfo_1 ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->mfo_1) ?>',1);">
...
<?php if ($frm_sam_units->mfo_2->Visible) { // mfo_2 ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->mfo_2) ?>',1);">
...
<?php if ($frm_sam_units->mfo_3->Visible) { // mfo_3 ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->mfo_3) ?>',1);">
...
<?php if ($frm_sam_units->mfo_4->Visible) { // mfo_4 ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->mfo_4) ?>',1);">
...
<?php if ($frm_sam_units->mfo_5->Visible) { // mfo_5 ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->mfo_5) ?>',1);">
...
<?php if ($frm_sam_units->sto->Visible) { // sto ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->sto) ?>',1);">
...
<?php if ($frm_sam_units->gass->Visible) { // gass ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->gass) ?>',1);">
...
<?php if ($frm_sam_units->t_cutOffDate_remarks->Visible) { // t_cutOffDate_remarks ?>
	...
	<?php } else { ?>
		<td><div class="upPointer" onmousedown="up_Sort(event,'<?php echo $frm_sam_units->SortUrl($frm_sam_units->t_cutOffDate_remarks) ?>',1);">
			<table cellspacing="0" class="upTableHeaderBtn"><thead><tr><td><?php echo $frm_sam_units->t_cutOffDate_remarks->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></td><td style="width: 10px;"><?php if ($frm_sam_units->t_cutOffDate_remarks->getSort() == "ASC") { ?><img src="phpimages/sortup.gif" width="10" height="9" border="0"><?php } elseif ($frm_sam_units->t_cutOffDate_remarks->getSort() == "DESC") { ?><img src="phpimages/sortdown.gif" width="10" height="9" border="0"><?php } ?></td></tr></thead></table>
		</div></td>		
	<?php } ?>
<?php } ?>		
<?php
...
$frm_sam_units_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<?php
if ($frm_sam_units->ExportAll && $frm_sam_units->Export <> "") {
	$frm_sam_units_list->StopRec = $frm_sam_units_list->TotalRecs;
} else {

	// Set the last record to display
	if ($frm_sam_units_list->TotalRecs > $frm_sam_units_list->StartRec + $frm_sam_units_list->DisplayRecs - 1)
		$frm_sam_units_list->StopRec = $frm_sam_units_list->StartRec + $frm_sam_units_list->DisplayRecs - 1;
	else
		$frm_sam_units_list->StopRec = $frm_sam_units_list->TotalRecs;
}
$frm_sam_units_list->RecCnt = $frm_sam_units_list->StartRec - 1;
if ($frm_sam_units_list->Recordset && !$frm_sam_units_list->Recordset->EOF) {
	$frm_sam_units_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $frm_sam_units_list->StartRec > 1)
		$frm_sam_units_list->Recordset->Move($frm_sam_units_list->StartRec - 1);
} elseif (!$frm_sam_units->AllowAddDeleteRow && $frm_sam_units_list->StopRec == 0) {
	$frm_sam_units_list->StopRec = $frm_sam_units->GridAddRowCount;
}

// Initialize aggregate
$frm_sam_units->RowType = UP_ROWTYPE_AGGREGATEINIT;
$frm_sam_units->ResetAttrs();
$frm_sam_units_list->RenderRow();
$frm_sam_units_list->RowCnt = 0;
while ($frm_sam_units_list->RecCnt < $frm_sam_units_list->StopRec) {
	$frm_sam_units_list->RecCnt++;
	if (intval($frm_sam_units_list->RecCnt) >= intval($frm_sam_units_list->StartRec)) {
		$frm_sam_units_list->RowCnt++;

		// Set up key count
		$frm_sam_units_list->KeyCount = $frm_sam_units_list->RowIndex;
		...
		if ($frm_sam_units->CurrentAction == "gridadd") {
			...
			$frm_sam_units_list->LoadRowValues($frm_sam_units_list->Recordset); // Load row values
		}
		$frm_sam_units->RowType = UP_ROWTYPE_VIEW; // Render view
		$frm_sam_units->RowAttrs = array('onmouseover'=>'up_MouseOver(event, this);', 'onmouseout'=>'up_MouseOut(event, this);', 'onclick'=>'up_Click(event, this);');
...
$frm_sam_units_list->ListOptions->Render("body", "left");
?>
	<?php if ($frm_sam_units->cu_unit_name->Visible) { // cu_unit_name ?>
...
<a name="<?php echo $frm_sam_units_list->PageObjName . "_row_" . $frm_sam_units_list->RowCnt ?>" id="<?php echo $frm_sam_units_list->PageObjName . "_row_" . $frm_sam_units_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($frm_sam_units->personnel_count->Visible) { // personnel_count ?>
...
$frm_sam_units_list->ListOptions->Render("body", "right");
?>
	</tr>
<?php
	}
	if ($frm_sam_units->CurrentAction <> "gridadd")
		$frm_sam_units_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
...
// Close recordset
if ($frm_sam_units_list->Recordset)
	$frm_sam_units_list->Recordset->Close();
?>
</td></tr></table>
<?php if ($frm_sam_units->Export == "" && $frm_sam_units->CurrentAction == "") { ?>
<?php } ?>
<?php
$frm_sam_units_list->ShowPageFooter();
if (UP_DEBUG_ENABLED)
	echo up_DebugMsg();
?>
<?php if ($frm_sam_units->Export == "") { ?>
...
<?php
$frm_sam_units_list->Page_Terminate();
?>
<?php
...
class cfrm_sam_units_list {
	...
	// Page object name
	var $PageObjName = 'frm_sam_units_list';
	...
	function cfrm_sam_units_list() {
		...
		$this->AddUrl = "frm_sam_unitsadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "frm_sam_unitsdelete.php";
		$this->MultiUpdateUrl = "frm_sam_unitsupdate.php";
	...
	function Page_Init() {
		...
		if (@$_GET["export"] <> "") {
			$frm_sam_units->Export = $_GET["export"];
		} elseif (up_IsHttpPost()) {
			if (@$_POST["exporttype"] <> "")
				$frm_sam_units->Export = $_POST["exporttype"];
		} else {
			$frm_sam_units->setExportReturnUrl(up_CurrentUrl());
		}
		$gsExport = $frm_sam_units->Export; // Get export parameter, used in header
		$gsExportFile = $frm_sam_units->TableVar; // Get export file, used in header
		$Charset = (UP_CHARSET <> "") ? ";charset=" . UP_CHARSET : ""; // Charset used in header
		if ($frm_sam_units->Export == "csv") {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			$frm_sam_units->Recordset_SearchValidated();
		...
		$frm_sam_units->Recordset_Searching($this->SearchWhere);

		// Save search criteria
		if ($this->SearchWhere <> "") {
			if ($sSrchBasic == "")
				$this->ResetBasicSearchParms();
			$frm_sam_units->setSearchWhere($this->SearchWhere); // Save to Session
			if (!$this->RestoreSearch) {
				$this->StartRec = 1; // Reset start record counter
				$frm_sam_units->setStartRecordNumber($this->StartRec);
			}
		} else {
			$this->SearchWhere = $frm_sam_units->getSearchWhere();
		}
		...
		// Export data only
		if (in_array($frm_sam_units->Export, array("html","word","excel","xml","csv","email","pdf"))) {
			$this->ExportData();
			if ($frm_sam_units->Export <> "email")
	...
	function BasicSearchSQL($Keyword) {
		...
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->cu_short_name, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->cu_unit_name, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->unit_name, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->unit_name_short, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->mfo_1, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->mfo_2, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->mfo_3, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->mfo_4, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->mfo_5, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->sto, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->gass, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $frm_sam_units->t_cutOffDate_remarks, $Keyword);
		return $sWhere;
	}
	...
	function BasicSearchWhere() {
		...
		$sSearchKeyword = $frm_sam_units->BasicSearchKeyword;
		$sSearchType = $frm_sam_units->BasicSearchType;
		if ($sSearchKeyword <> "") {
		...
		if ($sSearchKeyword <> "") {
			$frm_sam_units->setSessionBasicSearchKeyword($sSearchKeyword);
			$frm_sam_units->setSessionBasicSearchType($sSearchType);
		}
		return $sSearchStr;
	}
	...
	function ResetSearchParms() {
		...
		$this->SearchWhere = "";
		$frm_sam_units->setSearchWhere($this->SearchWhere);
	...
	function ResetBasicSearchParms() {
		global $frm_sam_units;
		$frm_sam_units->setSessionBasicSearchKeyword("");
		$frm_sam_units->setSessionBasicSearchType("");
	}
	...
	function RestoreSearchParms() {
		...
		if ($bRestore) {
			...
			$frm_sam_units->BasicSearchKeyword = $frm_sam_units->getSessionBasicSearchKeyword();
			$frm_sam_units->BasicSearchType = $frm_sam_units->getSessionBasicSearchType();
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$frm_sam_units->UpdateSort($frm_sam_units->cu_unit_name); // cu_unit_name
			$frm_sam_units->UpdateSort($frm_sam_units->personnel_count); // personnel_count
			$frm_sam_units->UpdateSort($frm_sam_units->mfo_1); // mfo_1
			$frm_sam_units->UpdateSort($frm_sam_units->mfo_2); // mfo_2
			$frm_sam_units->UpdateSort($frm_sam_units->mfo_3); // mfo_3
			$frm_sam_units->UpdateSort($frm_sam_units->mfo_4); // mfo_4
			$frm_sam_units->UpdateSort($frm_sam_units->mfo_5); // mfo_5
			$frm_sam_units->UpdateSort($frm_sam_units->sto); // sto
			$frm_sam_units->UpdateSort($frm_sam_units->gass); // gass
			$frm_sam_units->UpdateSort($frm_sam_units->t_cutOffDate_remarks); // t_cutOffDate_remarks
	...
	function ResetCmd() {
		...
		if (@$_GET["cmd"] <> "") {
			...
			if (strtolower($sCmd) == "resetsort") {
				$sOrderBy = "";
				$frm_sam_units->setSessionOrderBy($sOrderBy);
				$frm_sam_units->cu_unit_name->setSort("");
				$frm_sam_units->personnel_count->setSort("");
				$frm_sam_units->mfo_1->setSort("");
				$frm_sam_units->mfo_2->setSort("");
				$frm_sam_units->mfo_3->setSort("");
				$frm_sam_units->mfo_4->setSort("");
				$frm_sam_units->mfo_5->setSort("");
				$frm_sam_units->sto->setSort("");
				$frm_sam_units->gass->setSort("");
				$frm_sam_units->t_cutOffDate_remarks->setSort("");
			}
	...
	function SetupListOptions() {
		...
		$item->OnLeft = TRUE;
		$item->Header = "<input type=\"checkbox\" name=\"key\" id=\"key\" class=\"upbudgetofficeclass\" onclick=\"frm_sam_units_list.SelectAllKey(this);\">";
		$item->MoveTo(0);
	...
	function RenderListOptions() {
		...
		if ($Security->CanDelete() && $this->ShowOptionLink() && $oListOpt->Visible)
			$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" id=\"key_m[]\" value=\"" . up_HtmlEncode($frm_sam_units->units_id->CurrentValue) . "\" class=\"upbudgetofficeclass\" onclick='up_ClickMultiCheckbox(this);'>";
	...
	function LoadBasicSearchValues() {
		global $frm_sam_units;
		$frm_sam_units->BasicSearchKeyword = @$_GET[UP_TABLE_BASIC_SEARCH];
		$frm_sam_units->BasicSearchType = @$_GET[UP_TABLE_BASIC_SEARCH_TYPE];
	}
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($frm_sam_units->getKey("units_id")) <> "")
			$frm_sam_units->units_id->CurrentValue = $frm_sam_units->getKey("units_id"); // units_id
		else
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		// Initialize URLs
		$this->ViewUrl = $frm_sam_units->ViewUrl();
		$this->EditUrl = $frm_sam_units->EditUrl();
		$this->InlineEditUrl = $frm_sam_units->InlineEditUrl();
		$this->CopyUrl = $frm_sam_units->CopyUrl();
		$this->InlineCopyUrl = $frm_sam_units->InlineCopyUrl();
		$this->DeleteUrl = $frm_sam_units->DeleteUrl();
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a name=\"emf_frm_sam_units\" id=\"emf_frm_sam_units\" href=\"javascript:void(0);\" onclick=\"up_EmailDialogShow({lnk:'emf_frm_sam_units',hdr:upLanguage.Phrase('ExportToEmail'),f:document.ffrm_sam_unitslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;

		// Hide options for export/action
		if ($frm_sam_units->Export <> "" ||
			$frm_sam_units->CurrentAction <> "")
			$this->ExportOptions->HideAllOptions();
	}
	...
	function ExportData() {
		...
		// Load recordset
		if ($bSelectLimit) {
			$this->TotalRecs = $frm_sam_units->SelectRecordCount();
		} else {
		...
		if ($frm_sam_units->Export == "xml") {
			$XmlDoc = new cXMLDocument(UP_XML_ENCODING);
		} else {
			$ExportDoc = new cExportDocument($frm_sam_units, "h");
		}
		$ParentTable = "";

		// Export master record
		if (UP_EXPORT_MASTER_RECORD && $frm_sam_units->getMasterFilter() <> "" && $frm_sam_units->getCurrentMasterTable() == "frm_sam_cu_executive_offices") {
			global $frm_sam_cu_executive_offices;
			$rsmaster = $frm_sam_cu_executive_offices->LoadRs($this->DbMasterFilter); // Load master record
			if ($rsmaster && !$rsmaster->EOF) {
				if ($frm_sam_units->Export == "xml") {
					$ParentTable = "frm_sam_cu_executive_offices";
					$frm_sam_cu_executive_offices->ExportXmlDocument($XmlDoc, '', $rsmaster, 1, 1);
				} else {
					$ExportStyle = $ExportDoc->Style;
					$ExportDoc->ChangeStyle("v"); // Change to vertical
					if ($frm_sam_units->Export <> "csv" || UP_EXPORT_MASTER_RECORD_FOR_CSV) {
						$frm_sam_cu_executive_offices->ExportDocument($ExportDoc, $rsmaster, 1, 1);
		...
		if ($frm_sam_units->Export == "xml") {
			$frm_sam_units->ExportXmlDocument($XmlDoc, ($ParentTable <> ""), $rs, $StartRec, $StopRec, "");
		} else {
			$sHeader = $this->PageHeader;
			$this->Page_DataRendering($sHeader);
			$ExportDoc->Text .= $sHeader;
			$frm_sam_units->ExportDocument($ExportDoc, $rs, $StartRec, $StopRec, "");
			$sFooter = $this->PageFooter;
			$this->Page_DataRendered($sFooter);
		...
		// Write BOM if utf-8
		if ($utf8 && !in_array($frm_sam_units->Export, array("email", "xml")))
			echo "\xEF\xBB\xBF";
		...
		if ($frm_sam_units->Export == "xml") {
			...
			echo $XmlDoc->XML();
		} elseif ($frm_sam_units->Export == "email") {
			$this->ExportEmail($ExportDoc->Text);
			$this->Page_Terminate($frm_sam_units->ExportReturnUrl());
		} elseif ($frm_sam_units->Export == "pdf") {
	...
	function SetUpMasterParms() {
		...
		if (@$_GET[UP_TABLE_SHOW_MASTER] <> "") {
			...
			if ($sMasterTblVar == "frm_sam_cu_executive_offices") {
				...
				if (@$_GET["focal_person_id"] <> "") {
					$GLOBALS["frm_sam_cu_executive_offices"]->focal_person_id->setQueryStringValue($_GET["focal_person_id"]);
					$frm_sam_units->focal_person_id->setQueryStringValue($GLOBALS["frm_sam_cu_executive_offices"]->focal_person_id->QueryStringValue);
					$frm_sam_units->focal_person_id->setSessionValue($frm_sam_units->focal_person_id->QueryStringValue);
					if (!is_numeric($GLOBALS["frm_sam_cu_executive_offices"]->focal_person_id->QueryStringValue)) $bValidMaster = FALSE;
				} else {
					$bValidMaster = FALSE;
				}
			}
		...
		if ($bValidMaster) {
			...
			$frm_sam_units->setCurrentMasterTable($sMasterTblVar);
			...
			if ($sMasterTblVar <> "frm_sam_cu_executive_offices") {
				if ($frm_sam_units->focal_person_id->QueryStringValue == "") $frm_sam_units->focal_person_id->setSessionValue("");
			}
		}
