...
// Page class
//
class cfrm_u_pbb_detail_accomplishment_grid {
	...
	// Page object name
	var $PageObjName = 'frm_u_pbb_detail_accomplishment_grid';
	...
	//
	function cfrm_u_pbb_detail_accomplishment_grid() {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			if ($frm_u_pbb_detail_accomplishment->AllowAddDeleteRow) {
	...
	function ClearInlineMode() {
		global $frm_u_pbb_detail_accomplishment;
		$frm_u_pbb_detail_accomplishment->LastAction = $frm_u_pbb_detail_accomplishment->CurrentAction; // Save last action
		$frm_u_pbb_detail_accomplishment->CurrentAction = ""; // Clear action
	...
	function GridUpdate() {
		global $conn, $Language, $objForm, $gsFormError, $frm_u_pbb_detail_accomplishment;
		$bGridUpdate = TRUE;

		// Get old recordset
		$frm_u_pbb_detail_accomplishment->CurrentFilter = $this->BuildKeyFilter();
				...
				// Validate form and insert/update/delete record
					...
					if ($rowaction == "delete") {
						...
						if ($rowaction == "insert") {
							...
							if ($rowkey <> "") {
								$frm_u_pbb_detail_accomplishment->SendEmail = FALSE; // Do not send email on update success
		...
		if ($bGridUpdate) {
			...
			$frm_u_pbb_detail_accomplishment->EventCancelled = TRUE; // Set event cancelled
			$frm_u_pbb_detail_accomplishment->CurrentAction = "gridedit"; // Stay in Grid Edit mode
		}
		return $bGridUpdate;
	}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$frm_u_pbb_detail_accomplishment->pbb_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($frm_u_pbb_detail_accomplishment->pbb_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function GridInsert() {
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				$addcnt++;
				$frm_u_pbb_detail_accomplishment->SendEmail = FALSE; // Do not send email on insert success
				...
				if ($bGridInsert) {
					...
					$sKey .= $frm_u_pbb_detail_accomplishment->pbb_id->CurrentValue;
		...
		if ($bGridInsert) {
			...
			// Get new recordset
			$frm_u_pbb_detail_accomplishment->CurrentFilter = $sWrkFilter;
			$sSql = $frm_u_pbb_detail_accomplishment->SQL();
			if ($rs = $conn->Execute($sSql)) {
				$rsnew = $rs->GetRows();
				$rs->Close();
			...
			$frm_u_pbb_detail_accomplishment->EventCancelled = TRUE; // Set event cancelled
			$frm_u_pbb_detail_accomplishment->CurrentAction = "gridadd"; // Stay in gridadd mode
		}
		return $bGridInsert;
	}
	...
	// Check if empty row
	function EmptyRow() {
		global $frm_u_pbb_detail_accomplishment, $objForm;
		if ($objForm->HasValue("x_cu_unit_name") && $objForm->HasValue("o_cu_unit_name") && $frm_u_pbb_detail_accomplishment->cu_unit_name->CurrentValue <> $frm_u_pbb_detail_accomplishment->cu_unit_name->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_mfo_name") && $objForm->HasValue("o_mfo_name") && $frm_u_pbb_detail_accomplishment->mfo_name->CurrentValue <> $frm_u_pbb_detail_accomplishment->mfo_name->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_target") && $objForm->HasValue("o_target") && $frm_u_pbb_detail_accomplishment->target->CurrentValue <> $frm_u_pbb_detail_accomplishment->target->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_t_numerator") && $objForm->HasValue("o_t_numerator") && $frm_u_pbb_detail_accomplishment->t_numerator->CurrentValue <> $frm_u_pbb_detail_accomplishment->t_numerator->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_t_denominator") && $objForm->HasValue("o_t_denominator") && $frm_u_pbb_detail_accomplishment->t_denominator->CurrentValue <> $frm_u_pbb_detail_accomplishment->t_denominator->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_t_remarks") && $objForm->HasValue("o_t_remarks") && $frm_u_pbb_detail_accomplishment->t_remarks->CurrentValue <> $frm_u_pbb_detail_accomplishment->t_remarks->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_accomplishment") && $objForm->HasValue("o_accomplishment") && $frm_u_pbb_detail_accomplishment->accomplishment->CurrentValue <> $frm_u_pbb_detail_accomplishment->accomplishment->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_numerator") && $objForm->HasValue("o_a_numerator") && $frm_u_pbb_detail_accomplishment->a_numerator->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_numerator->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_denominator") && $objForm->HasValue("o_a_denominator") && $frm_u_pbb_detail_accomplishment->a_denominator->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_denominator->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_supporting_docs") && $objForm->HasValue("o_a_supporting_docs") && $frm_u_pbb_detail_accomplishment->a_supporting_docs->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_supporting_docs->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_remarks") && $objForm->HasValue("o_a_remarks") && $frm_u_pbb_detail_accomplishment->a_remarks->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_remarks->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_rating_above_90") && $objForm->HasValue("o_a_rating_above_90") && $frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_rating_above_90->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_rating_below_90") && $objForm->HasValue("o_a_rating_below_90") && $frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_rating_below_90->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_supporting_docs_comparison") && $objForm->HasValue("o_a_supporting_docs_comparison") && $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_a_cutOffDate_remarks") && $objForm->HasValue("o_a_cutOffDate_remarks") && $frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->CurrentValue <> $frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->OldValue)
			return FALSE;
		return TRUE;
	}
		...
		if ($frm_u_pbb_detail_accomplishment->AllowAddDeleteRow) {
			if ($frm_u_pbb_detail_accomplishment->CurrentMode == "add" || $frm_u_pbb_detail_accomplishment->CurrentMode == "copy" || $frm_u_pbb_detail_accomplishment->CurrentMode == "edit") {
				...
				if (is_numeric($this->RowIndex) && ($this->RowAction == "" || $this->RowAction == "edit")) { // Do not allow delete existing record
					...
					$oListOpt->Body = "<a class=\"upGridLink\" href=\"javascript:void(0);\" onclick=\"up_DeleteGridRow(this, frm_u_pbb_detail_accomplishment_grid, " . $this->RowIndex . ");\">" . $Language->Phrase("DeleteLink") . "</a>";
				}
			}
		}
		if ($frm_u_pbb_detail_accomplishment->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $frm_u_pbb_detail_accomplishment->pbb_id->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		global $frm_u_pbb_detail_accomplishment;
		$frm_u_pbb_detail_accomplishment->cu_unit_name->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->cu_unit_name->OldValue = $frm_u_pbb_detail_accomplishment->cu_unit_name->CurrentValue;
		$frm_u_pbb_detail_accomplishment->mfo_name->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->mfo_name->OldValue = $frm_u_pbb_detail_accomplishment->mfo_name->CurrentValue;
		$frm_u_pbb_detail_accomplishment->target->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->target->OldValue = $frm_u_pbb_detail_accomplishment->target->CurrentValue;
		$frm_u_pbb_detail_accomplishment->t_numerator->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->t_numerator->OldValue = $frm_u_pbb_detail_accomplishment->t_numerator->CurrentValue;
		$frm_u_pbb_detail_accomplishment->t_denominator->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->t_denominator->OldValue = $frm_u_pbb_detail_accomplishment->t_denominator->CurrentValue;
		$frm_u_pbb_detail_accomplishment->t_remarks->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->t_remarks->OldValue = $frm_u_pbb_detail_accomplishment->t_remarks->CurrentValue;
		$frm_u_pbb_detail_accomplishment->accomplishment->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->accomplishment->OldValue = $frm_u_pbb_detail_accomplishment->accomplishment->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_numerator->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_numerator->OldValue = $frm_u_pbb_detail_accomplishment->a_numerator->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_denominator->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_denominator->OldValue = $frm_u_pbb_detail_accomplishment->a_denominator->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_supporting_docs->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_supporting_docs->OldValue = $frm_u_pbb_detail_accomplishment->a_supporting_docs->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_remarks->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_remarks->OldValue = $frm_u_pbb_detail_accomplishment->a_remarks->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_rating_above_90->OldValue = $frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_rating_below_90->OldValue = $frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->OldValue = $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue;
		$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->CurrentValue = NULL;
		$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->OldValue = $frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$frm_u_pbb_detail_accomplishment->cu_unit_name->setOldValue($objForm->GetValue("o_cu_unit_name"));
		...
		$frm_u_pbb_detail_accomplishment->mfo_name->setOldValue($objForm->GetValue("o_mfo_name"));
		...
		$frm_u_pbb_detail_accomplishment->target->setOldValue($objForm->GetValue("o_target"));
		...
		$frm_u_pbb_detail_accomplishment->t_numerator->setOldValue($objForm->GetValue("o_t_numerator"));
		...
		$frm_u_pbb_detail_accomplishment->t_denominator->setOldValue($objForm->GetValue("o_t_denominator"));
		...
		$frm_u_pbb_detail_accomplishment->t_remarks->setOldValue($objForm->GetValue("o_t_remarks"));
		...
		$frm_u_pbb_detail_accomplishment->accomplishment->setOldValue($objForm->GetValue("o_accomplishment"));
		...
		$frm_u_pbb_detail_accomplishment->a_numerator->setOldValue($objForm->GetValue("o_a_numerator"));
		...
		$frm_u_pbb_detail_accomplishment->a_denominator->setOldValue($objForm->GetValue("o_a_denominator"));
		...
		$frm_u_pbb_detail_accomplishment->a_supporting_docs->setOldValue($objForm->GetValue("o_a_supporting_docs"));
		...
		$frm_u_pbb_detail_accomplishment->a_remarks->setOldValue($objForm->GetValue("o_a_remarks"));
		if (!$frm_u_pbb_detail_accomplishment->a_rating_above_90->FldIsDetailKey) {
			$frm_u_pbb_detail_accomplishment->a_rating_above_90->setFormValue($objForm->GetValue("x_a_rating_above_90"));
		}
		$frm_u_pbb_detail_accomplishment->a_rating_above_90->setOldValue($objForm->GetValue("o_a_rating_above_90"));
		if (!$frm_u_pbb_detail_accomplishment->a_rating_below_90->FldIsDetailKey) {
			$frm_u_pbb_detail_accomplishment->a_rating_below_90->setFormValue($objForm->GetValue("x_a_rating_below_90"));
		}
		$frm_u_pbb_detail_accomplishment->a_rating_below_90->setOldValue($objForm->GetValue("o_a_rating_below_90"));
		if (!$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->FldIsDetailKey) {
			$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->setFormValue($objForm->GetValue("x_a_supporting_docs_comparison"));
		}
		$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->setOldValue($objForm->GetValue("o_a_supporting_docs_comparison"));
		...
		$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->setOldValue($objForm->GetValue("o_a_cutOffDate_remarks"));
		if (!$frm_u_pbb_detail_accomplishment->pbb_id->FldIsDetailKey && $frm_u_pbb_detail_accomplishment->CurrentAction <> "gridadd" && $frm_u_pbb_detail_accomplishment->CurrentAction <> "add")
	...
	function RestoreFormValues() {
		global $objForm, $frm_u_pbb_detail_accomplishment;
		if ($frm_u_pbb_detail_accomplishment->CurrentAction <> "gridadd" && $frm_u_pbb_detail_accomplishment->CurrentAction <> "add")
		...
		$frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue = $frm_u_pbb_detail_accomplishment->a_rating_above_90->FormValue;
		$frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue = $frm_u_pbb_detail_accomplishment->a_rating_below_90->FormValue;
		$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue = $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->FormValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$frm_u_pbb_detail_accomplishment->pbb_id->CurrentValue = strval($arKeys[0]); // pbb_id
			else
				$bValidKey = FALSE;
		}
	...
	function RenderRow() {
		...
		if ($frm_u_pbb_detail_accomplishment->RowType == UP_ROWTYPE_VIEW) { // View row
		...
		} elseif ($frm_u_pbb_detail_accomplishment->RowType == UP_ROWTYPE_ADD) { // Add row

			// cu_unit_name
			$frm_u_pbb_detail_accomplishment->cu_unit_name->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->cu_unit_name->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->cu_unit_name->CurrentValue);

			// mfo_name
			$frm_u_pbb_detail_accomplishment->mfo_name->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->mfo_name->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->mfo_name->CurrentValue);

			// target
			$frm_u_pbb_detail_accomplishment->target->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->target->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->target->CurrentValue);

			// t_numerator
			$frm_u_pbb_detail_accomplishment->t_numerator->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->t_numerator->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->t_numerator->CurrentValue);

			// t_denominator
			$frm_u_pbb_detail_accomplishment->t_denominator->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->t_denominator->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->t_denominator->CurrentValue);
			...
			$frm_u_pbb_detail_accomplishment->accomplishment->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->accomplishment->CurrentValue);

			// a_numerator
			$frm_u_pbb_detail_accomplishment->a_numerator->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_numerator->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_numerator->CurrentValue);

			// a_denominator
			$frm_u_pbb_detail_accomplishment->a_denominator->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_denominator->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_denominator->CurrentValue);

			// a_supporting_docs
			$frm_u_pbb_detail_accomplishment->a_supporting_docs->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_supporting_docs->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_supporting_docs->CurrentValue);
			...
			// a_rating_above_90
			$frm_u_pbb_detail_accomplishment->a_rating_above_90->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_rating_above_90->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue);

			// a_rating_below_90
			$frm_u_pbb_detail_accomplishment->a_rating_below_90->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_rating_below_90->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue);

			// a_supporting_docs_comparison
			$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue);

			// a_cutOffDate_remarks
			$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->EditValue = up_HtmlEncode($frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->CurrentValue);
			...
			// t_remarks
			$frm_u_pbb_detail_accomplishment->t_remarks->EditCustomAttributes = "";
			$frm_u_pbb_detail_accomplishment->t_remarks->EditValue = $frm_u_pbb_detail_accomplishment->t_remarks->CurrentValue;
			$frm_u_pbb_detail_accomplishment->t_remarks->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!UP_SERVER_VALIDATE)
			return ($gsFormError == "");
		if (!up_CheckNumber($frm_u_pbb_detail_accomplishment->a_rating_above_90->FormValue)) {
			up_AddMessage($gsFormError, $frm_u_pbb_detail_accomplishment->a_rating_above_90->FldErrMsg());
		}
		if (!up_CheckNumber($frm_u_pbb_detail_accomplishment->a_rating_below_90->FormValue)) {
			up_AddMessage($gsFormError, $frm_u_pbb_detail_accomplishment->a_rating_below_90->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// a_rating_above_90
			$frm_u_pbb_detail_accomplishment->a_rating_above_90->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue, NULL, $frm_u_pbb_detail_accomplishment->a_rating_above_90->ReadOnly);

			// a_rating_below_90
			$frm_u_pbb_detail_accomplishment->a_rating_below_90->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue, NULL, $frm_u_pbb_detail_accomplishment->a_rating_below_90->ReadOnly);

			// a_supporting_docs_comparison
			$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue, NULL, $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->ReadOnly);
	...
	function AddRow($rsold = NULL) {
		...
		if ($Security->CurrentUserID() <> "" && !$Security->IsAdmin()) { // Non system admin
			$sFilter = $frm_u_pbb_detail_accomplishment->SqlMasterFilter_frm_u_rep_a_eligible_status_unit_mfo();
			if (strval($frm_u_pbb_detail_accomplishment->units_id->CurrentValue) <> "" &&
				$frm_u_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_u_rep_a_eligible_status_unit_mfo") {
				$sFilter = str_replace("@units_id@", up_AdjustSql($frm_u_pbb_detail_accomplishment->units_id->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if (strval($frm_u_pbb_detail_accomplishment->mfo->CurrentValue) <> "" &&
				$frm_u_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_u_rep_a_eligible_status_unit_mfo") {
				$sFilter = str_replace("@mfo@", up_AdjustSql($frm_u_pbb_detail_accomplishment->mfo->CurrentValue), $sFilter);
			} else {
				$sFilter = "";
			}
			if ($sFilter <> "") {
				$rsmaster = $GLOBALS["frm_u_rep_a_eligible_status_unit_mfo"]->LoadRs($sFilter);
				$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
				if (!$this->MasterRecordExists) {
		...
		// Set up foreign key field value from Session
			if ($frm_u_pbb_detail_accomplishment->getCurrentMasterTable() == "frm_u_rep_a_eligible_status_unit_mfo") {
				$frm_u_pbb_detail_accomplishment->units_id->CurrentValue = $frm_u_pbb_detail_accomplishment->units_id->getSessionValue();
				$frm_u_pbb_detail_accomplishment->mfo->CurrentValue = $frm_u_pbb_detail_accomplishment->mfo->getSessionValue();
			}
		$rsnew = array();

		// cu_unit_name
		$frm_u_pbb_detail_accomplishment->cu_unit_name->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->cu_unit_name->CurrentValue, NULL, FALSE);

		// mfo_name
		$frm_u_pbb_detail_accomplishment->mfo_name->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->mfo_name->CurrentValue, NULL, FALSE);

		// target
		$frm_u_pbb_detail_accomplishment->target->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->target->CurrentValue, NULL, FALSE);

		// t_numerator
		$frm_u_pbb_detail_accomplishment->t_numerator->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->t_numerator->CurrentValue, NULL, FALSE);

		// t_denominator
		$frm_u_pbb_detail_accomplishment->t_denominator->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->t_denominator->CurrentValue, NULL, FALSE);

		// t_remarks
		$frm_u_pbb_detail_accomplishment->t_remarks->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->t_remarks->CurrentValue, NULL, FALSE);

		// accomplishment
		$frm_u_pbb_detail_accomplishment->accomplishment->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->accomplishment->CurrentValue, NULL, FALSE);

		// a_numerator
		$frm_u_pbb_detail_accomplishment->a_numerator->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_numerator->CurrentValue, NULL, FALSE);

		// a_denominator
		$frm_u_pbb_detail_accomplishment->a_denominator->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_denominator->CurrentValue, NULL, FALSE);

		// a_supporting_docs
		$frm_u_pbb_detail_accomplishment->a_supporting_docs->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_supporting_docs->CurrentValue, NULL, FALSE);

		// a_remarks
		$frm_u_pbb_detail_accomplishment->a_remarks->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_remarks->CurrentValue, NULL, FALSE);

		// a_rating_above_90
		$frm_u_pbb_detail_accomplishment->a_rating_above_90->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_rating_above_90->CurrentValue, NULL, FALSE);

		// a_rating_below_90
		$frm_u_pbb_detail_accomplishment->a_rating_below_90->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_rating_below_90->CurrentValue, NULL, FALSE);

		// a_supporting_docs_comparison
		$frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_supporting_docs_comparison->CurrentValue, NULL, FALSE);

		// a_cutOffDate_remarks
		$frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->SetDbValueDef($rsnew, $frm_u_pbb_detail_accomplishment->a_cutOffDate_remarks->CurrentValue, NULL, FALSE);

		// units_id
		if ($frm_u_pbb_detail_accomplishment->units_id->getSessionValue() <> "") {
			$rsnew['units_id'] = $frm_u_pbb_detail_accomplishment->units_id->getSessionValue();
		} elseif (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin
			$rsnew['units_id'] = CurrentUserID();
		}

		// mfo
		if ($frm_u_pbb_detail_accomplishment->mfo->getSessionValue() <> "") {
			$rsnew['mfo'] = $frm_u_pbb_detail_accomplishment->mfo->getSessionValue();
		}

		// Call Row Inserting event
		$rs = ($rsold == NULL) ? NULL : $rsold->fields;
		$bInsertRow = $frm_u_pbb_detail_accomplishment->Row_Inserting($rs, $rsnew);
		if ($bInsertRow) {
			$conn->raiseErrorFn = 'up_ErrorFn';
			$AddRow = $conn->Execute($frm_u_pbb_detail_accomplishment->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
			if ($frm_u_pbb_detail_accomplishment->CancelMessage <> "") {
				$this->setFailureMessage($frm_u_pbb_detail_accomplishment->CancelMessage);
		...
		if ($AddRow) {
			$frm_u_pbb_detail_accomplishment->pbb_id->setDbValue($conn->Insert_ID());
			$rsnew['pbb_id'] = $frm_u_pbb_detail_accomplishment->pbb_id->DbValue;
		}
		if ($AddRow) {

			// Call Row Inserted event
			$rs = ($rsold == NULL) ? NULL : $rsold->fields;
			$frm_u_pbb_detail_accomplishment->Row_Inserted($rs, $rsnew);
		}
		return $AddRow;
	}
	...
	function SetUpMasterParms() {
		...
		// Hide foreign keys
		$sMasterTblVar = $frm_u_pbb_detail_accomplishment->getCurrentMasterTable();
		if ($sMasterTblVar == "frm_u_rep_a_eligible_status_unit_mfo") {
			$frm_u_pbb_detail_accomplishment->units_id->Visible = FALSE;
			if ($GLOBALS["frm_u_rep_a_eligible_status_unit_mfo"]->EventCancelled) $frm_u_pbb_detail_accomplishment->EventCancelled = TRUE;
			$frm_u_pbb_detail_accomplishment->mfo->Visible = FALSE;
			if ($GLOBALS["frm_u_rep_a_eligible_status_unit_mfo"]->EventCancelled) $frm_u_pbb_detail_accomplishment->EventCancelled = TRUE;
		}
