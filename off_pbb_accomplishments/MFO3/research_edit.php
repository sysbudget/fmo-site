...
$query = "SELECT mfo3_form_program_id, unit_contributor_id, unit_delivery_id, focal_person_id, cu_id, cu_short_name, unit_delivery_name_cu, unit_contributor_name, research_type_id, research_type_name, mfo3_form_research_title, mfo3_form_research_quarter, mfo3_form_research_completed, mfo3_form_research_duration_in_months_plan, mfo3_form_research_month_year_started, mfo3_form_research_month_year_completed, mfo3_form_research_name_of_authors, mfo3_form_research_completed_within_the_timeframe, mfo3_form_research_attachment, mfo3_form_research_considered_in_gaa FROM tbl_sd_3_mfo3_form_research WHERE mfo3_form_program_id = " . $mfo3_form_program_id;
...
//Extract fields
	...
	if ($end_date == 0 || $end_date == NULL){$end_date = date(time());}

	$considered_gaa_bp = $row['mfo3_form_research_considered_in_gaa'];

	$a = $row['mfo3_form_research_attachment'];
	$attachment = rtrim($a,".pdf");
...
<body onload = "choice1()">
<div class="viewbody">
<h2>MFO3: Research Project - Edit Record</h2>
<a href="research_view_all.php">View All Records</a>
<hr/>

<!-- Edit form here -->
<form name="edit_research_form" id="edit_research_form" method="post" action="research_save_edited.php" onsubmit="return validateForm()" enctype="multipart/form-data">
<table>

<tr>
	<?php
		$array = array($research_type_id, $research_type_name);
		$rsrch_type = implode(",", $array);
	?>
    <td width="155"><div align="right">Research Type</div></td>
    <td width="500"><select name="rsrch_type" size="1" required>
	    <option value="<?php print $rsrch_type; ?>"><?php print $research_type_name; ?></option>
      <?php foreach($array1 as $option1) : ?>
      <option value="<?php print $option1->research_type_id . ',' . $option1->research_type_name; ?>"><?php print $option1->research_type_name; ?></option>
      <?php endforeach; ?>
     </select></td>
</tr>
...
<tr>
    ...
    <td width="500"><textarea name="rsrch_title" cols="50" rows="6" onkeyup="limit(rsrch_title)" onblur="limit(rsrch_title)" required><?php print $rsrch_title; ?></textarea></td>
</tr>
...
<tr>
    <td width="155"><div align="right">Project / Study Duration <br/>(in terms of months)</div></td>
    <td width="500"><input name="proj_duration" type="number" min="0" required id="proj_duration" value="<?php print $proj_duration; ?>" size="11" onkeyup="checkban(proj_duration)" onblur="checkban(proj_duration)"/></td>
</tr>
...
<tr>
    <td width="155"><div align="right">Completed</div></td>
 	<td width="500">
    <input type="radio" name="completed" onchange="choice1(completed_y)" id="completed_y" <?php if (isset($completed) && $completed=="Y") echo "checked";?>  value="Y">Yes
...
<tr>
	...
	<td width="500"><input type="text" name="start_date" id="start_date" size="12" value="<?php print date("m/d/Y",$start_date); ?>" required></td>
</tr>
...
<tr>
	...
	<td width="500"><input type="text" name="end_date" id="end_date" size="12" value="<?php print date("m/d/Y",$end_date); ?>" required></td>
</tr>
...
<tr>
    ...
    <td width="500"><textarea name="author"  cols="50" rows="6" onkeyup="limit(author)" onblur="limit(author)" required><?php print $author; ?></textarea></td>
</tr>
...
<tr>
       ...
       <input type="hidden" name="mfo3_form_program_id" value="<?php print $mfo3_form_program_id; ?>"><button type="submit">Submit</button></div>
	</th>
    </tr>
