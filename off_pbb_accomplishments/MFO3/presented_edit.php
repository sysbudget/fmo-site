...
// get the id from the url
	$mfo3_forma_presented_id = $_GET['id'];
...
mfo3_forma_presented_id 
FROM tbl_sd_3_mfo3_forma_presented 
...
//Extract fields.
	...
	$confe_date = strtotime($row['mfo3_forma_presented_date']);
	$confe_presenter = $row['mfo3_forma_presented_presenter'];
	$conference_forum_type_id = $row['conference_forum_type_id'];
	$conference_forum_type_name = $row['conference_forum_type_name'];
	$confe_venue = $row['mfo3_forma_presented_venue'];	
	$confe_title = $row['mfo3_forma_presented_title_of_the_conference_form'];
	$considered_gaa_bp = $row['mfo3_forma_presented_considered_in_gaa'];

	$a = $row['mfo3_forma_presented_attachment'];
	$confe_attachment = rtrim($a,".pdf");
...
//Generate sample filename
	...
	$confe_attachment = $file_name_sample;
	}
...
<div class="viewbody">
<h2>MFO3: Paper Presented - Edit Record</h2>
<a href="presented_view_all.php">View All Records</a>
<hr/>

<!-- Edit form here -->
<form id="edit_presented_form" method="post" action="presented_save_edited.php" onsubmit="return validateForm()" enctype="multipart/form-data">
...
<tr>
	...
	<td width="500"><input type="text" name="confe_date" id="confe_date" size="12" value="<?php print date("m/d/Y",$confe_date); ?>" required></td>
</tr>
...
<tr>
    ...
    <td width="500"><textarea name="confe_presenter"  cols="50" rows="6" onkeyup="limit(confe_presenter)" onblur="limit(confe_presenter)" required><?php print $confe_presenter; ?></textarea></td>
</tr>
...
<tr>
	<?php
		$array = array($conference_forum_type_id, $conference_forum_type_name);
		$confe_type = implode(",", $array);
	?>
    <td width="155"><div align="right">Type</div></td>
    <td width="500"><select name="confe_type" size="1" required>
	    <option value="<?php print $confe_type; ?>"><?php print $conference_forum_type_name; ?></option>
      <?php foreach($array1 as $option1) : ?>
      <option value="<?php print $option1->conference_forum_type_id . ',' . $option1->conference_forum_type_name; ?>"><?php print $option1->conference_forum_type_name; ?></option>
      <?php endforeach; ?>
     </select></td>
</tr>
    ...
    <tr>
    	<td width="155"><div align="right">Venue</div></td>
	   	<td width="500"><textarea name="confe_venue" cols="50" rows="6" onkeyup="limit(confe_venue)" onblur="limit(confe_venue)" required style='text-align:left'><?php print $confe_venue; ?></textarea></td>
  	</tr>
    ...
    <tr>
    	<td width="155"><div align="right">Title</div></td>
	   	<td width="500"><textarea name="confe_title" cols="50" rows="6" onkeyup="limit(confe_title)" onblur="limit(confe_title)" required style='text-align:left'><?php print $confe_title; ?></textarea></td>
  	</tr>
...
<tr>
    ...
    <td width="500"><textarea name="file_name_display" id="file_name_display" readonly cols="50" rows="6" required><?php print $confe_attachment; ?></textarea></td>
	<input name="file_name_sample" hidden id="file_name_sample" value="<?php print $confe_attachment; ?>" />
</tr>
...
<tr>
       ...
       <input type="hidden" name="mfo3_forma_presented_id" value="<?php print $mfo3_forma_presented_id; ?>"><button type="submit">Submit</button></div>
	</th>
    </tr>
