...
// get the id from the url
	$gass_formcd_apcpi_app_id = $_GET['id'];
...
gass_formcd_apcpi_app_id
FROM tbl_sd_7_gass_formcd_apcpi_app
...
//Extract fields.
	...
	$qtr = $row['gass_formcd_apcpi_app_quarter'];
	$apcpi = $row['gass_formc_apcpi_submitted'];
	$app = $row['gass_formd_app_submitted'];

	$a = $row['gass_formc_apcpi_attachment'];
	$gass_formc_apcpi = rtrim($a,".pdf");
	$b = $row['gass_formd_app_attachment'];
	$gass_formd_app = rtrim($b,".pdf");
...
//Get session values
	...
	$gass_formc_apcpi = $file_name_sample1;
	}
	
	if (empty($gass_formd_app)){
	...
	$gass_formd_app = $file_name_sample2;
...
function validateForm()
{
	...
	{
		alert ("Nothing to submit if both APCPI and APP are not activated");
		return false;
	}
...
<!-- Edit form here -->
<form name="edit_apcpi_app_form" id="edit_apcpi_app_form" method="post" onsubmit="return validateForm()" action="apcpi_app_save_edited.php" enctype="multipart/form-data">
<table>
...
<tr>
    ...
    <input type="radio" onchange="choice1(apcpi)" name="apcpi" id="apcpi_y" <?php if (isset($apcpi) && $apcpi=="Y") echo "checked";?>  value="Y">Yes
   	<input type="radio" onchange="choice1(apcpi)" name="apcpi" id="apcpi_n" <?php if (isset($apcpi) && $apcpi=="N") echo "checked";?>  value="N">No
	</td>
</tr>
...
<tr>
    ...
    <td width="500"><textarea name="file_name_display1" id="file_name_display1" readonly cols="50" rows="6" required><?php print $gass_formc_apcpi; ?></textarea></td>
	<input name="file_name_sample1" hidden id="file_name_sample1" value="<?php print $gass_formc_apcpi; ?>" />
</tr>
...
<tr>
        ...
        <i>(Copy the above prescribed filename to upload the PDF copy of the scanned acknowledgement receipt of complete submission)</i><br/></div>
	   	</td>
</tr>
...
<tr>
    ...
    <input type="radio" onchange="choice2(app)" name="app" id="app_y" <?php if (isset($app) && $app=="Y") echo "checked";?>  value="Y">Yes
   	<input type="radio" onchange="choice2(app)" name="app" id="app_n" <?php if (isset($app) && $app=="N") echo "checked";?>  value="N">No
	</td>
</tr>
...
<tr>
    ...
    <td width="500"><textarea name="file_name_display2" id="file_name_display2" readonly cols="50" rows="6" required><?php print $gass_formd_app; ?></textarea></td>
	<input name="file_name_sample2" hidden id="file_name_sample2" value="<?php print $gass_formd_app; ?>" />
</tr>
...
<tr>
       ...
       <input type="hidden" name="gass_formcd_apcpi_app_id" value="<?php print $gass_formcd_apcpi_app_id; ?>"><button type="submit">Submit</button></div>
	</th>
    </tr>
